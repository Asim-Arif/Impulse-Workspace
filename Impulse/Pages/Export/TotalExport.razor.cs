...
{
    public partial class TotalExport : ComponentBase
    {
        [Inject] private ITotalExportService TotalExportService { get; set; } = null!;
        ...
        private List<TotalExportModel> exportData = new();
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                var fetched = await TotalExportService.GetActiveCustomersAsync();
        ...
        private void SetDatesForRange()
        {
            ...
            switch (selectedRange)
            {
                ...
                case "90Days":
                    ...
                    break;
                case "Custom":
                    // Keeps existing date picker values
                    break;
            }
        }
        ...
        private async Task RefreshDataInternal()
        {
            ...
            try
            {
                exportData = await TotalExportService.GetTotalExportDataAsync(custCodeVal, dateFrom, dateTo);
            ...
            {
                NotificationServiceManager.ShowError("Query Error", "Failed to query export statistics: " + ex.Message);
            }
        }

        private void CalculateSummaryTotals()
        {
            totalAmount = exportData.Sum(row => showCustomAmount ? row.TotalCustomAmt : row.TotalAmt);
        ...
        private async Task PrintReport()
        {
            ...
            try
            {
                ...
                NotificationServiceManager.ShowSuccess("Success", "Total export report dispatched.");
            }
            catch (Exception ex)
            {
