...
{
    public partial class RemunerationandOtherBenefits : ComponentBase, IDisposable
    {
        private RemunerationandOtherBenefitsModel statementmodel = new();
        ...
        private List<RemunerationandOtherBenefitsModel> getDBEntries { get; set; } = new List<RemunerationandOtherBenefitsModel>();
        private RemunerationandOtherBenefitsModel SelectedHead;
        private RemunerationandOtherBenefitsModel CurrentHead =new RemunerationandOtherBenefitsModel();
        ...
        private async Task SaveHead()
        {
            if (IsAdding)
            {
                ...
                else if (CurrentHead.CEAmount == 0)
                {
                    NotificationServiceManager.ShowWarning("Alert", "Invalid Amount.");
                    return;
                }
                CurrentHead.AccGroup_RefID = SelectedSubHead.EntryID;
                await _financialstatementService.SaveRemunerationandOtherBenefitsEntries(CurrentHead);
            }
            ...
            else if (IsEdit)
            {
                ...
                await _financialstatementService.EditRemunerationandOtherBenefitsEntries(CurrentHead,CurrentHead.EntryID);
        ...
        {
            var bank = e.Data as RemunerationandOtherBenefitsModel;
            SelectedHead = bank;

            if (SelectedHead != null)
            {
                CurrentHead = new RemunerationandOtherBenefitsModel
                {
                    ...
                    CEAmount = SelectedHead.CEAmount,
                    DirAmount = SelectedHead.DirAmount,
                    ExeAmount = SelectedHead.ExeAmount
                };
        ...
        private async Task Refreshlist()
        {
            ...
            try
            {
                var EntriesFromDb = await _financialstatementService.GetRemunerationandOtherBenefitsEntries();
        ...
        private async Task DeleteSelected(ItemClickEventArgs e)
        {
            try
            {
                var head = e.Data as RemunerationandOtherBenefitsModel;
                SelectedHead = head;    
                bool bdeleted= await _financialstatementService.DeleteRemunerationandOtherBenefitsEntries(SelectedHead.EntryID);
                if (bdeleted == false)
                {
