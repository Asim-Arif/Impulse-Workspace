...
namespace CIP.Pages.Accounts
{
    public partial class ProfitandLossHeads : ComponentBase, IDisposable
    {
        ...
        private List<ProfitandLossHeadsDetailToLoadModel> getDBEntries { get; set; } = new List<ProfitandLossHeadsDetailToLoadModel>();
        private ProfitandLossHeads_Main_Model SelectedHead;
        private ProfitandLossHeadsDetailToLoadModel SelectedEntry;
        private ProfitandLossHeads_Main_Model CurrentHead =new ProfitandLossHeads_Main_Model();
        ...
        private async Task SaveHead()
        {
            if (IsAdding)
            {
                ...
                CurrentHead.DetailItems = SelectedMainHead.Select(x => new ProfitandLossHeads_Detail_Model
                {
                    Group_RefID = x.EntryID
                }).ToList();

                await _financialstatementService.SaveProfitandLossHeads(CurrentHead);
            ...
            else if (IsEdit)
            {
                ...
                await _financialstatementService.EditProfitandLossHeads(CurrentHead,CurrentHead.EntryID);
        ...
        {
            var main = e.Data as ProfitandLossHeadsDetailToLoadModel;
            SelectedEntry = main;

            if (SelectedEntry != null)
            {
                CurrentHead = new ProfitandLossHeads_Main_Model
                {
        ...
        private async Task Refreshlist()
        {
            ...
            try
            {
                var EntriesFromDb = await _financialstatementService.GetProfitandLossHeadsList();
        ...
        private async Task DeleteSelected(ItemClickEventArgs e)
        {
            try
            {
                ...
                var head = e.Data as ProfitandLossHeadsDetailToLoadModel;
                SelectedEntry = head;    
