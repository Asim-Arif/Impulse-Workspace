...
{
    public partial class BalanceSheetHeads : ComponentBase, IDisposable
    {
        ...
        private List<BalanceSheetHeadsDetailToLoadModel> getDBEntries { get; set; } = new List<BalanceSheetHeadsDetailToLoadModel>();
        private BalanceSheet_Main_Model SelectedHead;
        private BalanceSheetHeadsDetailToLoadModel SelectedEntry;
        private BalanceSheet_Main_Model CurrentHead =new BalanceSheet_Main_Model();
        ...
        public IList<MainHeadsListModel> MainHeads { get; set; } = new List<MainHeadsListModel>();
        private IList<MainHeadsListModel> SelectedMainHead { get; set; } = new List<MainHeadsListModel>();
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                var data = await _financialstatementService.GetAllHeads();
                MainHeads = data?.ToList() ?? new List<MainHeadsListModel>();
        ...
        private async Task SaveHead()
        {
            if (IsAdding)
            {
                ...
                CurrentHead.DetailItems = SelectedMainHead.Select(x => new BalanceSheet_Detail_Model
                {
                    Group_RefID = x.EntryID
                }).ToList();

                await _financialstatementService.SaveBalanceSheetHeads(CurrentHead);
            ...
            else if (IsEdit)
            {
                ...
                await _financialstatementService.EditBalanceSheetHeads(CurrentHead,CurrentHead.EntryID);
        ...
        {
            var main = e.Data as BalanceSheetHeadsDetailToLoadModel;
            SelectedEntry = main;

            if (SelectedEntry != null)
            {
                CurrentHead = new BalanceSheet_Main_Model
                {
        ...
        private void HideModal()
        {
            ...
            //{
            //    SelectedMainHead.GroupName= string.Empty;
            //    SelectedMainHead.EntryID = 0;
            //}
        ...
        private async Task Refreshlist()
        {
            ...
            try
            {
                var EntriesFromDb = await _financialstatementService.GetBalanceSheetHeadsList();
        ...
        private async Task DeleteSelected(ItemClickEventArgs e)
        {
            try
            {
                ...
                var head = e.Data as BalanceSheetHeadsDetailToLoadModel;
                SelectedEntry = head;    
                bool bdeleted= await _financialstatementService.DeleteProfitandLoss(SelectedEntry.RefID);
                if (bdeleted == false)
                {
        ...
        private async Task<IEnumerable<MainHeadsListModel>> SearchMainHeads(string searchText)
        {
        ...
        private async Task SelectedResultChanged_MainHead(IList<MainHeadsListModel?> selectedhead)
        {
            //SelectedMainHead = selectedhead;
            SelectedMainHead = selectedhead ?? new List<MainHeadsListModel>();
            if (SelectedMainHead != null)
            {
