...
{
    public partial class OnlinePaymentVoucher : ComponentBase, IDisposable
    {
        //private bool ShowEmployees = false;
        private string strVendorOrEmployee = string.Empty;
        ...
        private List<GenericDropDownModel> Employees = new List<GenericDropDownModel>();
        private List<GenericDropDownModel> Vendors = new List<GenericDropDownModel>();
        ...
        private GenericDropDownModel? SelectedEmployeeVendor = null;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            paymentTypes = await _voucherService.GetOnlineVoucherPaymentModes();
            if (paymentTypes != null && paymentTypes.Any()) //Select Default 
            {
                SelectedPaymentMethod = paymentTypes.FirstOrDefault(p => p.DropDownValue_Description == "Corporate Cheque") ?? paymentTypes.First();
            }
            ...
            Employees = await _voucherService.GetValuesForDropDown("Employees",  "EmpID", "EmpID +' { ' + Name +' '+ Designation +' ' + ISNULL(BankAccNo,'') +'} '", " ORDER BY EmpID");
            Vendors = await _voucherService.GetValuesForDropDown("Venders", "VendID", "AccTitle +' { ' + ISNULL(BankAccNo,'') +'} '", " ORDER BY AccTitle");
        ...
        private async Task SaveVoucher()
        {
            ...
            try
            {
                ...
                int iOnlineVchrType = 0;
                if (SelectedPaymentMethod.DropDownValue_ID == "0")  //Corporate Cheque
                {
                    iOnlineVchrType = 2;
                    iVendID = 0;
                }
                ...
                {
                    iOnlineVchrType = 1;
                    if (SelectedEmployeeVendor!=null)
                    iVendID = int.Parse(SelectedEmployeeVendor.DropDownValue_ID);
                }
                if (ShowEmployees) 
                {
                    if (SelectedEmployeeVendor != null)
                    {
                        strEmpID = SelectedEmployeeVendor.DropDownValue_ID;
                    }                        
                    iVendID = 0;
                }
                voucherViewModel.Online_Vchr = true;
                voucherViewModel.Online_Vchr_Type = iOnlineVchrType;
                voucherViewModel.VendID = iVendID;
                voucherViewModel.EmpID = strEmpID;
                voucherViewModel.PaymentToEmp = ShowEmployees ? 1 : 0;
        ...
        private void SelectedResultChanged_EmployeeVendor(GenericDropDownModel? selectedEmployeeVendor)
        {            
            SelectedEmployeeVendor = selectedEmployeeVendor;
            //newCIP.CardNo = SelectedCardNo.CardNo;
        }
        private async Task<IEnumerable<GenericDropDownModel>> GetEmployeesVendors(string searchText)
        {
            if (ShowEmployees)
            {
                return await Task.FromResult(
        ...
        public bool ShowEmployees
        {
            ...
            set
            {
                if (_showEmployees != value)
                {
                    _showEmployees = value;
                    HandleTypeChange(); // Call your logic here
                }
            }
        }
        ...
        {
            // 1. Update the label we discussed earlier
            //strVendorOrEmployee = _showEmployees ? "Employee" : "Vendor";

            // 2. CRITICAL: Clear the selected account so you don't save 
            // a Vendor ID into an Employee slot by mistake.
            SelectedEmployeeVendor = null;
