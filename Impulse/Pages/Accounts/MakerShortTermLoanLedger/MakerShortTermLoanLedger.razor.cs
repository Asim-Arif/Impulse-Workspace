...
namespace Impulse.Pages.Accounts.MakerShortTermLoanLedger
{
    public partial class MakerShortTermLoanLedger : ComponentBase
    {
        [Inject] private IMakerShortLoanLedgerService LedgerService { get; set; } = default!;
        ...
        private MakerShortLoanLedgerResultModel LedgerData { get; set; } = new();
        ...
        private async Task RefreshLedgerAsync()
        {
            ...
            {
                LedgerData = new MakerShortLoanLedgerResultModel();
                return;
            }
        ...
        private async Task PrintLedgerAsync()
        {
            ...
            try
            {
                string selectionFormula = $"{{VMakerAdvancesLedger_Short.AccNo}}='{SelectedMaker.AccNo}' AND {{VMakerAdvancesLedger_Short.DT}}>=#{FromDate:yyyy-MM-dd}# AND {{VMakerAdvancesLedger_Short.DT}}<=#{ToDate:yyyy-MM-dd}#";
