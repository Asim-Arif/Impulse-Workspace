...
{
    public partial class AccountsHeadManualEntry : ComponentBase, IDisposable
    {
        private AccountsHeadManualEntryModel statementmodel = new();
        ...
        private List<AccountsHeadManualEntryModel> getDBEntries { get; set; } = new List<AccountsHeadManualEntryModel>();
        private AccountsHeadManualEntryModel SelectedHead;
        private AccountsHeadManualEntryModel CurrentHead =new AccountsHeadManualEntryModel();
        ...
        private async Task SaveHead()
        {
            if (IsAdding)
            {
                ...
                await _financialstatementService.SaveAccountsHeadManualEntries(CurrentHead);
            }
            ...
            else if (IsEdit)
            {
                ...
                await _financialstatementService.EditAccountsHeadManualEntries(CurrentHead,CurrentHead.EntryID);
        ...
        {
            var bank = e.Data as AccountsHeadManualEntryModel;
            SelectedHead = bank;

            if (SelectedHead != null)
            {
                CurrentHead = new AccountsHeadManualEntryModel
                {
        ...
        private async Task Refreshlist()
        {
            ...
            try
            {
                var EntriesFromDb = await _financialstatementService.GetAccountsHeadManualEntries();
        ...
        private async Task DeleteSelected(ItemClickEventArgs e)
        {
            try
            {
                ...
                var head = e.Data as AccountsHeadManualEntryModel;
                SelectedHead = head;    
                bool bdeleted= await _financialstatementService.DeleteAccountsHeadManualEntries(SelectedHead.EntryID);
                if (bdeleted == false)
                {
