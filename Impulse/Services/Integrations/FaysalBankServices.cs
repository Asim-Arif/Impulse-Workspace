...
namespace Impulse.Services.Integrations
{
    public class FaysalBankServices : IFaysalBankServices
    {
        ...
        public FaysalBankServices(HttpClient httpClient, IConfiguration config, IDBHelperService db, IAccountReportingAccess accountReportingAccess, IAuditService auditService, ILogger<FaysalBankServices> logger)
        {
        ...
        public async Task<int> ExportAndUploadExcelForBOD(int entryId) 
        {
            var details = await _accountReportingAccess.GetBODMeetingPaymentDetail(entryId);            
            List<FaysalBankTransaction_Model> bankTransactions = details
                .Select(item => new FaysalBankTransaction_Model
                {
                    // Direct Mappings
                    Bank_Code_Name = item.Bank_Code_Name,
                    Transaction_Type = item.Transaction_Type,
                    Bank_Acc_Title = item.Bank_Acc_Title,
                    ...
                    FolioNo = item.FolioNo.ToString(),
                    OL_PV_RefID = item.OL_PV_RefID

                }).ToList();

            return await ExportAndUploadExcelAsync(bankTransactions);
        }
        public async Task<int> ExportAndUploadExcelForOPV(string strVchrNo)
        {
            //Will do it later

            /*var details = await _accountReportingAccess.GetOnlinePaymentVoucherList(null, null, null, null, null);
            List<FaysalBankTransaction_Model> bankTransactions = details
                .Select(item => new FaysalBankTransaction_Model
                {
                    // Direct Mappings
        ...
        public async Task<int> ExportAndUploadExcelAsync(List<FaysalBankTransaction_Model> details)
        {
            //var details = await _accountReportingAccess.GetBODMeetingPaymentDetail(entryId);
            int entryId=0;
            string strVchrNo = string.Empty;
            if (details[0].OL_PV_RefID == null) //it means it's BOD
            {
                entryId = details[0].OL_PV_RefID;
            }
            else 
            {
                strVchrNo = details[0].VchrNo;
            }
        ...
        public class FaysalBankTransaction_Model
        {
