...
Begin VB.Form frmMakerIssuanceFromSF 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin TabDlg.SSTab SSTab1 
         ...
         Tab(0).Control(0)=   "txtTotal"
         Tab(0).Control(1)=   "LVBal"
         Tab(0).ControlCount=   2
         TabCaption(1)   =   "Vender History"
         TabPicture(1)   =   "frmMakerIssuanceFromSF.frx":0D3E
         Tab(1).ControlEnabled=   0   'False
         Tab(1).Control(0)=   "txtTotalValueHistory"
         Tab(1).Control(1)=   "txtTotalQtyHistory"
         Tab(1).Control(2)=   "txtRate"
         Tab(1).Control(3)=   "LVMaker"
         Tab(1).Control(4)=   "Label5(3)"
   ...
   Begin MSForms.Label Label4_1 
      ...
      Width           =   10785
      BackColor       =   11517387
      Caption         =   $"frmMakerIssuanceFromSF.frx":0E5F
      Size            =   "19024;423"
      BorderColor     =   16512
      BorderStyle     =   1
      FontName        =   "Tahoma"
   ...
   Begin MSForms.Label Label3 
      ...
      Width           =   10860
      BackColor       =   11517387
      Caption         =   $"frmMakerIssuanceFromSF.frx":0F39
      Size            =   "19156;423"
      BorderColor     =   16512
      BorderStyle     =   1
      FontName        =   "Tahoma"
   ...
   Begin MSForms.Label Label2 
      ...
      Width           =   10770
      BackColor       =   11517387
      Caption         =   $"frmMakerIssuanceFromSF.frx":0FFF
      Size            =   "18997;423"
      BorderColor     =   16512
      BorderStyle     =   1
      FontName        =   "Tahoma"
...
'    End If
    ...
    Else
        ...
        Else
            cmbStore.ClearVals
            cmbStore.AddVals con, "DISTINCT StoreName", "VendRcvdDetail INNER JOIN VStockOrderOpening ON VendRcvdDetail.Opening_RefID=VStockOrderOpening.EntryID" _
             , "Store_RefID", "WHERE ItemCode='" & cmbItems.Text & "' AND (Qty-IssdQty)>0 AND VStockOrderOpening.ProcessID=" & cmbProcess.ID '& " AND LotNo='0'"
        End If
    End If
...
Private Sub ShowOtherVendersHistory()
    ...
    Dim ITM As ListItem
    With rs
        .Open "Select Top 10 * From VMakerIssItems Where VendID<>" & lMakerID & " AND ProcessID=" & cmbProcess.ID & " AND ItemCode='" & cmbItems & "' Order By DT Desc,EntryID Desc", con, adOpenForwardOnly, adLockReadOnly
...
Private Sub cmbVender_matched()
    ...
    Dim dRate As Double
    dRate = GetSingleDoubleValue("Rate", "VMakerAssItems", "WHERE VendID=" & lMakerID & " AND ItemID='" & cmbItems.Text & "' AND ProcessID=" & cmbReturnProcess.List(cmbReturnProcess.ListIndex, 1))
    '------------------------------------------
    txtVendRate = dRate
...
Private Sub ShowVenderHistory()
    ...
    With rs
        .Open "Select Top 10 * From VMakerIssItems WHERE VendID=" & lMakerID & " Order By DT Desc,EntryID Desc", con, adOpenForwardOnly, adLockReadOnly
        ...
        'If cmbVender.ID <> 129 Then
            .Open "Select ProcessName,ItemCode,ItemName,Sum(RemQty) As RemQty From VMakerIssItems Where VendID=" & lMakerID & "  AND RemQty>0 Group By ProcessName,ItemCode,ItemName Having Sum(RemQty)>0", con, adOpenForwardOnly, adLockReadOnly
...
Private Sub txtLotNo_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        With rs
            .Open "SELECT TOP 1 LotTransferDetails.EntryID,LotNo,RcvdQty,NextProcessID,ItemCode,ProcessID FROM LotTransferDetails INNER JOIN VendRcvdDetail ON LotTransferDetails.VRD_To_RefID=VendRcvdDetail.EntryID WHERE LotNo='" & txtLotNo & "' AND RcvdQty>IssQty", con, adOpenForwardOnly, adLockReadOnly
            If .EOF = False Then
                bRecFound = True
...
Private Sub txtQtyToAdd_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        ITM.ListSubItems(3).Tag = Val(txtInHand)
...
'    End If
    Call StartTrans(con)
        ...
        For j = 1 To myPOSavedCol.count
            ...
            If myPOObjTemp.iIssueProcess = iProcessID And myPOObjTemp.iReturnProcess = iProcessID And myPOObjTemp.iVendID = lMakerID And myPOObjTemp.strItemCode = cmbItems.Text Then
                SaveIt = False
    ...
    If SaveIt Then
        ...
        "MaximumRcvingsAgainstPO,MasterPONo,SampleProvided,DrawingProvided,ForgingProvided,SteelProvided,Authorized) VALUES(" & lMakerID & ",'" & _
        ...
        If lMakerID = iFactoryMakerID And (strCompany = "Weldon Instruments" Or strCompany = "QEL") Then
...
'            End If
            ...
            End If
            If bLotNoWiseEntry Then
                con.Execute "INSERT INTO StockOrderOpening_Issuance(SOO_RefID,QtyIssd,VID_RefID,LTD_RefID,InHand_B4_Issuance,LotNo_Manual,BatchNo_Manual) VALUES(" & lOpeningEntryID & _
                 "," & lShelfQty & "," & lVID_EntryID & "," & Val(txtLotNo.Tag) & "," & Val(.ListSubItems(3).Tag) & ",'" & .SubItems(5) & "','" & .SubItems(6) & "')"
            Else
                con.Execute "INSERT INTO StockOrderOpening_Issuance(SOO_RefID,QtyIssd,VID_RefID,InHand_B4_Issuance,LotNo_Manual,BatchNo_Manual) VALUES(" & lOpeningEntryID & _
                 "," & lShelfQty & "," & lVID_EntryID & "," & Val(.ListSubItems(3).Tag) & ",'" & .SubItems(5) & "','" & .SubItems(6) & "')"
            End If
        End With
    Next
    ...
    If chkPrint.Value = vbChecked Then
        ...
        Set rpt = rptApp.OpenReport(RptPath & "\IssSlip.rpt")
        Dim frmPrev As New frmPrevRpt
        frmPrev.ShowReport "{VendIssued.EntryID} IN[" & lRefID & "]", rpt
    End If
    ...
    For i = 1 To LV.ListItems.count
        If LV.ListItems(i).Checked Then
            ...
            With ITM
                ...
                ITM.ListSubItems.add(, lMakerID & "'", cmbVender.Text).Tag = lMakerID
