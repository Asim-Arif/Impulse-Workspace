...
Begin VB.Form frmRejection_Receive 
   ClientHeight    =   9870
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   13230
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.ComboBox cmbRange 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmRejection_Receive.frx":0000
         Left            =   4455
         List            =   "frmRejection_Receive.frx":0016
      ...
      Begin TabDlg.SSTab SSTab1 
         ...
         TabPicture(0)   =   "frmRejection_Receive.frx":0066
         ...
         TabPicture(1)   =   "frmRejection_Receive.frx":0082
         ...
         TabPicture(2)   =   "frmRejection_Receive.frx":009E
         ...
         Begin VB.Label Label1 
            ...
            Caption         =   $"frmRejection_Receive.frx":00BA
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   8.25
               Charset         =   0
      ...
      Begin VB.Label Label1 
         ...
         BackColor       =   &H00AFBDCB&
         BackStyle       =   0  'Transparent
         Caption         =   $"frmRejection_Receive.frx":01A2
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin MSComctlLib.ImageList ImageList2 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRejection_Receive.frx":0273
            Key             =   "Down"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRejection_Receive.frx":042F
            Key             =   "Up"
         EndProperty
      EndProperty
   End
   ...
   End
End
Attribute VB_Name = "frmRejection_Receive"
...
Private Sub cmdPrintList_Click()
    ...
    Set rpt = rptApp.OpenReport(RptPath & "\MakerRepair_Auth.rpt")
    Dim f As New frmPrevRpt
    f.ShowReport "", rpt, strSQLReport
End Sub
...
Private Sub cmdUpdate_Click()
    ...
    If (Val(txtStoreQty) + Val(txtLostQty)) > Val(txtBalance) Then
        MsgBox "Invalid Qty.", vbInformation
        Exit Sub
    End If
    If Val(txtStoreQty) < 0 Then
        MsgBox "Invalid Qty.", vbInformation
        Exit Sub
    ElseIf Val(txtLostQty) < 0 Then
        MsgBox "Invalid Qty.", vbInformation
        Exit Sub
    End If
    If (Val(txtStoreQty) + Val(txtLostQty)) = 0 Then
        MsgBox "Invalid Qty.", vbInformation
        Exit Sub
    End If
    ...
    con.Execute "INSERT INTO Rejection_Receiving(VRDWD_RefID,QtyRcvd,QtyLost,Remarks,UserName,MachineName,Shelf_RefID) VALUES(" & Val(LVRepair.SelectedItem.key) & _
     "," & Val(txtStoreQty) & "," & Val(txtLostQty) & ",'" & txtRemarks & "','" & CurrentUserName & "','" & strComputerName & "'," & cmbLocation.ID & ")"
     
    If Val(txtStoreQty) + Val(txtLostQty) = Val(txtBalance) Then
        con.Execute "UPDATE UsersToDoTasks SET Done=1,DoneUserName='" & CurrentUserName & "',DoneMachineName='" & strComputerName & _
         "',DoneEntryDT=getDate() WHERE TaskValue='" & Val(LVRepair.SelectedItem.ListSubItems(1).Tag) & "' AND Type='Receive_Rejection'"
    End If
    con.CommitTrans
...
'    If strCompany = "Weldon Instruments" Or strCompany = "Sunlike" Or strCompany = "QEL" Or (strCompany = "Dr-Frgz" And strSubCompany = "Intl") Or strCompany = "Banzai" Or strCompany = "Towne" Or strCompany = "Tecnimen" Then
...
Private Sub AddRepairDetail(Optional strCondition As String)

    Dim rs As New ADODB.Recordset
    Dim ITM As ListItem, dRate As Double
    
    If strCondition = "" Then
        strCondition = " WHERE HandlingBy=1 AND DT BETWEEN '" & DtFrom.Value & "' AND '" & DtTo.Value & "' AND Qty>(TotalReceived+TotalLost) "
        ...
        If cmbItemCat.ID <> "0" And cmbItemCat.ID <> "" Then
            If cmbItemCat.Functionality = 1 Then
                strCondition = strCondition & " AND CatID IN(" & cmbItemCat.ID & ")"
            Else
                strCondition = strCondition & " AND CatID='" & cmbItemCat.ID & "'"
            End If
        End If
        
        If cmbProcess.ID <> "0" And cmbProcess.ID <> "" Then
            strCondition = strCondition & " AND ProcessID IN(" & cmbProcess.ID & ")"
        End If
        
        If Trim(txtLotNo) <> "" Then
            'strCondition = strCondition & " AND LotNo='" & txtLotNo & "'"
            strCondition = " WHERE LotNo='" & txtLotNo & "'"
        End If
    End If
    'strCondition = strCondition & " AND VendID<>" & iFactoryMakerID
    Dim lForeColor As Long
    Dim j As Integer
    'Dim lTotal As Long
    With rs
        strSQLReport = "SELECT * FROM VVendRcvdDetailWastageDetail_RejectionStore " & strCondition
        
        .Open strSQLReport, con, adOpenForwardOnly, adLockReadOnly
        ...
        Do Until .EOF
            ...
            ITM.ListSubItems.add , , Val(!TotalReceived & "")
            ITM.ListSubItems.add , , Val(!TotalLost & "")
            ITM.ListSubItems.add , , Val(!Qty & "") - Val(!TotalReceived & "") - Val(!TotalLost & "")
            
            ITM.ListSubItems(1).Tag = Val(!VRD_RefID & "")
...
Private Sub Form_Resize()
    ...
    FRcvd.Move (Me.ScaleWidth - FRcvd.Width) / 2, (Me.ScaleHeight - FAcc.Height) / 2
...
Private Sub LVRepair_ColumnClick(ByVal ColumnHeader As MSComctlLib.ColumnHeader)
   
    If LVRepair.SortKey + 1 = ColumnHeader.Index Then
        If LVRepair.SortOrder = lvwAscending Then
            LVRepair.SortOrder = lvwDescending
            'ColumnHeader.Icon = "Down"
        Else
            LVRepair.SortOrder = lvwAscending
            'ColumnHeader.Icon = "Up"
        End If
    Else
        LVRepair.ColumnHeaderIcons = Nothing
        Set LVRepair.ColumnHeaderIcons = ImageList2
        LVRepair.SortKey = ColumnHeader.Index - 1
        LVRepair.SortOrder = lvwAscending
        'ColumnHeader.Icon = "Down"
    End If
    Call RefreshSnos
...
Private Sub LVRepair_DblClick()
    ...
    FAcc.Enabled = False
    txtBalance = Val(LVRepair.SelectedItem.SubItems(9))
    ...
    FRcvd.Visible = True
    txtStoreQty.SetFocus
    
'    'If Val(LVRepair.SelectedItem.ListSubItems(1).Tag) = iFactoryMakerID Then
'        With txtEditRate
'            .Move LVRepair.Left + LVRepair.ColumnHeaders(7).Left, LVRepair.Top + LVRepair.SelectedItem.Top
'            .Text = Val(LVRepair.SelectedItem.SubItems(6))
'            .Visible = True
'            .SelStart = 0
'            .SelLength = Len(.Text)
'            .SetFocus
...
Private Sub txtEditRate_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        LVRepair.SelectedItem.SubItems(6) = Val(txtEditRate)
        LVRepair.SelectedItem.SubItems(8) = Val(txtEditRate) * Val(LVRepair.SelectedItem.SubItems(5))
...
Public Sub ShowForAlert(p_lVRD_RefID As Long)

    lVRD_RefID = p_lVRD_RefID
    Dim strCondition As String
    strCondition = " WHERE VRD_RefID=" & lVRD_RefID & " AND Qty>(TotalReceived+TotalLost)"
    Call AddRepairDetail(strCondition)
    
    Me.Show
'    Dim strLotNo As String
'    strLotNo = GetSingleStringValue("LotNo", "VendRcvdDetail", " WHERE EntryID=" & lVRD_RefID)
'    txtLotNo = strLotNo
