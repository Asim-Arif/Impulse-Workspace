...
Begin VB.Form frmFCustOrderList 
   ClientHeight    =   9885
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   13905
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.ComboBox cmbfield 
            ...
            EndProperty
            Height          =   315
            ItemData        =   "frmFCustOrderList.frx":0000
            Left            =   60
            List            =   "frmFCustOrderList.frx":000D
      ...
      Begin VB.ComboBox cmbRange 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmFCustOrderList.frx":002F
         Left            =   8460
         List            =   "frmFCustOrderList.frx":0048
         Style           =   2  'Dropdown List
         TabIndex        =   15
         Top             =   330
         Width           =   1530
      ...
      Begin MSComctlLib.ImageList ImageList2 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmFCustOrderList.frx":00A7
               Key             =   "Down"
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmFCustOrderList.frx":0263
               Key             =   "Up"
            EndProperty
         EndProperty
      End
      ...
      Begin VB.Label Label1 
         Appearance      =   0  'Flat
         BackColor       =   &H00AFBDCB&
         Caption         =   $"frmFCustOrderList.frx":041F
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmFCustOrderList"
...
Public Sub ShowMe(p_bForProduction As Boolean, Optional p_bSample As Boolean = False, Optional p_iViewType As Integer = 0)

    bSample = p_bSample
    iViewType = p_iViewType
    ...
    If iViewType = 0 Then 'Show All
    ElseIf iViewType = 1 Then 'Non Parts & Stock
        strCondition = " WHERE CustCode NOT IN('Parts','Stock')"
    ElseIf iViewType = 2 Then 'Parts & Stock Only
        strCondition = " WHERE CustCode IN('Parts','Stock')"
    End If
    Call cmbCust.AddVals(con, "CustCode", "ForeignCustomers", "CustCode", strCondition)
    cmbCust.ID = "0"
...
Private Sub cmbRange_Click()
    ...
    Select Case cmbRange.ListIndex
        ...
        Case 6
            DtFrom.Enabled = True
            DtTo.Enabled = True
            If DtFrom.Visible Then DtFrom.SetFocus
            Exit Sub
    End Select
...
Private Sub RefreshLV()
    ...
    If iViewType = 0 Then 'Show All
    ElseIf iViewType = 1 Then 'Non Parts & Stock
        Cond = Cond & " AND CustCode NOT IN('Parts','Stock')"
    ElseIf iViewType = 2 Then 'Parts & Stock Only
        Cond = Cond & " AND CustCode IN('Parts','Stock')"
    End If
    ...
    ElseIf cmbStatus.ListIndex = 1 Then
        ...
        sTmp = " (UnShippedOrderNo IS NULL AND Cancelled=0)"
    ...
    SQLString = "SELECT * FROM VFOrderList " & Cond
    ...
    With rs
        ...
        Do Until .EOF
            ...
            ITM.ListSubItems.add , , ![OrderType] & ""
            ...
            ITM.ListSubItems.add(, , Val(!TotalBalanceQty & "")).ForeColor = vbRed
...
'    End If
    ...
    Call frmFNewCustOrder_Frgz.EditOrder(Left(LV.SelectedItem.key, Len(LV.SelectedItem.key) - 1))
...
'    End If
    ...
    If strCompany = "IAA" Or strCompany = "Kami" Then
        ...
        If lCount > 0 Then
            ...
            strRet = ShowPopUpMenu1("Copy Order", "Delete Order", "&Edit This Order", "-", "&Print Order", "Print Order Slip", "Print Order (Details)", "Print Order (Pictures)", "Print Order Status", "Order Balance Report", "Print Order (Balance)", "-", strPrintOrderListCaption, "-", "Print Order (Balance Quantities)", "-", "Print Balance Order", "Purchase Order Planning", "Print Order Planning Report", "Hub Wise Status", "Unplanned Items", "Print Raw Material Planning", "-", "Stock Order Adjustment", "Un-Cancel/Un-Hold Order")
        Else
            ...
            strRet = ShowPopUpMenu1("Copy Order", "Delete Order", "&Edit This Order", "-", "&Print Order", "Print Order Slip", "Print Order (RM)", "Print Order (Packing)", "Print Order (Details)", "Print Order (Pictures)", "Print Order Status", "Print Order (Balance)" _
             , strPrintOrderListCaption, "-", "Print Order (Balance Quantities)", "-", "Order Balance Report", "Print Balance Order", "-", "Purchase Order Planning", "Print Order Planning Report", "Hub Wise Status", "Unplanned Items", "Print Raw Material Planning", "-", "Stock Order Adjustment", IIf(bForProduction, "-", "Mark As Shipped"), IIf(bForProduction, "-", "Cancel Order"), IIf(bForProduction, "-", "Hold Order"))
        End If
...
'            Call DeleteOrder
        ...
        Case "&Print Order"
            Call PrintOrder
        Case "Print Order Slip"
            Call PrintOrder(, , , , , , , , , 99)
        Case "Print Order (Details)"
            If getDBPassword("Print Order (Details)") = False Then Exit Sub
            Call PrintOrder(False, False, False, False, False, False, False, False, True)
        Case "Print Order (Pictures)"
            Call PrintOrder(, , , , , , , , , 98)
        Case "Print Order Status"
            Call PrintOrder(False, False, False, False, False, False, False, True)
        Case "Print Order (RM)"
            Call PrintOrder(False, False, False, False, False, False, False, True, False, 1)
        Case "Print Order (Packing)"
            Call PrintOrder(False, False, False, False, False, False, False, True, False, 2)
        Case "Print Order (Balance)"
            If getDBPassword("Print Order (Balance)") = False Then Exit Sub
        ...
        Case "Print Raw Material Planning"
            Call PrintRawMaterialPlanning(LV.SelectedItem.SubItems(4))
    End Select
    'Me.PopupMenu mnuPop
    ...
    Else
        ...
        ElseIf bOrderStatus Then
            If iReportType = 1 Then
                Set rpt = rptApp.OpenReport(RptPath & "\ProductionOrder_RM.rpt")
            ElseIf iReportType = 2 Then
                Set rpt = rptApp.OpenReport(RptPath & "\ProductionOrder_Packing.rpt")
            Else
                Set rpt = rptApp.OpenReport(RptPath & "\rptProductionOrder_Status.rpt")
            End If
        ElseIf bOrderDetails Then
            Set rpt = rptApp.OpenReport(RptPath & "\rptProductionOrder_Details.rpt")
        ElseIf bPrintBalance Then
            Set rpt = rptApp.OpenReport(RptPath & "\Production_Order_Balance.rpt")
        Else
            If iReportType = 0 Then
                Set rpt = rptApp.OpenReport(RptPath & "\rptProductionOrderWrangler.rpt")
            ElseIf iReportType = 99 Then
                Set rpt = rptApp.OpenReport(RptPath & "\Print_Order_Slip.rpt")
            ElseIf iReportType = 98 Then
                Set rpt = rptApp.OpenReport(RptPath & "\Print_Order_Pictures.rpt")
            End If
        End If
    End If
...
Private Function DeleteOrder(p_strOrderNo As String) As Boolean
    ...
    con.Execute "DELETE FROM FCustomerOrders WHERE OrderNo='" & p_strOrderNo & "'"
    DeleteOrder = True
