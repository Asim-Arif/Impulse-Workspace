...
Begin VB.Form frmVendRcvPO 
   ...
   End
End
Attribute VB_Name = "frmVendRcvPO"
...
Private Sub cmbOrders_matched()
    ...
    With rs
        .Open "Select * From VVendPOItemsToRcv Where OrderNo='" & cmbOrders.ID & "'", con, adOpenForwardOnly, adLockReadOnly
        ...
        Do Until .EOF
            ...
            ITM.ListSubItems.add(, , ![CompleteItemName] & "").ForeColor = FColor
            ITM.ListSubItems.add(, , Val(![Qty] & "") - Val(![QtyRcvd] & "")).ForeColor = FColor
...
'        End If
'    End If
    cmbVend.AddVals con, "Distinct VenderName", "VVendPOItemsToRcv", "VendID", " Where ProcessID=" & cmbProcess.ID
...
Private Sub cmbVend_matched()
    ...
    cmbOrders.ClearVals
    cmbOrders.AddVals con, "Distinct OrderNo", "VVendPOItemsToRcv", "OrderNo", " Where VendID=" & cmbVend.ID
...
'        FormProcessID = cmbProcess.ID
'        Unload Me
'        Call ShowRcvPOForm(FormProcessID)
'    End If
    ...
    For i = 1 To LV.ListItems.count
            ...
            If Val(LV.ListItems(i).SubItems(3)) > 0 Then
                ...
                With cmd
                    .ActiveConnection = con
                    .CommandType = adCmdStoredProc
                    .CommandText = "SP_InsertVendRcvdDirect"
                    .Parameters("@ProcessID").Value = cmbProcess.ID
                    .Parameters("@ItemCode").Value = LV.ListItems(i).SubItems(1)
                    ...
                    If cmbProcess.ID = 4 Then
                        .Parameters("@ReqAuth").Value = 1
                    Else
                        .Parameters("@ReqAuth").Value = 1 'UserReqAuth
                    End If
                    .Parameters("@VendID").Value = cmbVend.ID
                    If cmbProcess.ID >= 7 Then
                        .Parameters("@LotNo").Value = LV.ListItems(i).ListSubItems(4).Tag
                    End If
                    '.Parameters("@UserID").value = UserID
                    .Execute
                ...
                '-----------------------------------------------------------------
                'Following Block Is Commented Out BCoz Entry Will always require Authorization.
...
'                End If
                ...
                con.Execute "Update VendPurchOrdDetail Set QtyRcvd=QtyRcvd+" & Val(LV.ListItems(i).SubItems(4)) & " Where ID=" & Val(LV.ListItems(i).Tag)
                con.Execute "Insert Into VendPORcvdDetail (OrderRefID,RcvdRefID,Qty) Values(" & LV.ListItems(i).Tag & "," & DetailRefID & "," & Val(LV.ListItems(i).SubItems(4)) & ")"
...
Private Sub txtQty_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        Else
            ...
            End If
        End If
        dRate = Val(con.Execute("Select Rate From VendAssItems Where VendID=" & cmbVend.ID & " AND ItemID='" & LV.SelectedItem.SubItems(1) & "'").Fields(0).Value & "")
        If dRate = 0 Then
...
'        dtpicker1.SetFocus
        If Val(txtQty) = 0 Then
            ...
            If cmbProcess.Tag = "Lot No" Then
                If GotRcvID = 0 Then
                    If cmbProcess.ID = 7 Then
                        GotRcvID = Val(con.Execute("Select Max(Cast(right(RecieptID,Len(RecieptID)-12) As Int)) From VVendRcvdItems Where Convert(varchar,DT,6)='" & Format(DTPicker1, "dd MMM yy") & "' AND ProcessID=7 and UPPER(Left(RecieptID,5))='HSPOL'").Fields(0).Value & "") + 1
                    ElseIf cmbProcess.ID = 12 Then
                        GotRcvID = Val(con.Execute("Select Max(Cast(right(RecieptID,Len(RecieptID)-12) As Int)) From VVendRcvdItems Where Convert(varchar,DT,6)='" & Format(DTPicker1, "dd MMM yy") & "' AND ProcessID=7 and UPPER(Left(RecieptID,5))='HSPAC'").Fields(0).Value & "") + 1
                    End If
                Else
                    GotRcvID = GotRcvID + 1
                End If
                If cmbProcess.ID = 7 Then
                    LV.SelectedItem.SubItems(LV.ColumnHeaders.count - 1) = "HSPOL-" & _
                     Format(DTPicker1, "ddMMyy") & GotRcvID
                Else
                    LV.SelectedItem.SubItems(LV.ColumnHeaders.count - 1) = "HSPAC-" & _
                     Format(DTPicker1, "ddMMyy") & GotRcvID
                End If
            Else
                If GotRcvID = 0 Then
