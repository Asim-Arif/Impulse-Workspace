...
Begin VB.Form frmRMRcvItemsOld 
   ClientHeight    =   7425
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   11040
   ...
   End
End
Attribute VB_Name = "frmRMRcvItemsOld"
...
Private Sub cmbVend_matched()
    ...
    With rs
        .Open "Select * From VRMPOrders Where Qty>RcvdQty and VendID=" & cmbVend.ID, con, adOpenForwardOnly, adLockReadOnly
        ...
        Do Until .EOF
            ...
            itm.ListSubItems.Add(, , ![RMID1] & "").Tag = ![RMID] & ""
            itm.ListSubItems.Add , , ![RMName] & ""
            itm.ListSubItems.Add , , ![Qty] & ""
            itm.ListSubItems.Add , , ![Qty] - ![RcvdQty] & ""
    ...
    For i = 1 To LV.ListItems.Count
        With LV.ListItems(i)
            If Val(.SubItems(6)) > 0 Then
                '-----------------------------------------------------------------
                con.Execute "Update RMPOrderItems Set RcvdQty=RcvdQty+" & _
                 Val(.SubItems(6)) & " Where ID=" & _
                 Val(.Key)
                 
                con.Execute "Update RM Set QtyInStock=IsNull(QtyInStock,0)+" & _
                 Val(.SubItems(6)) & " Where RMID=" & .ListSubItems(2).Tag
                
                con.Execute "Insert Into RMRcvdItems(RefID,OrderNo,RMID,Qty)" & _
                " Values(" & iRefID & ",'" & .SubItems(1) & "'," & _
                .ListSubItems(2).Tag & "," & .SubItems(6) & ")"
