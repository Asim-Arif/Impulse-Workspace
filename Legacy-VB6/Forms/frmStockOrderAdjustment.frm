...
Private Sub cmdOK_Click()
    ...
    For i = 1 To LVAdj.ListItems.count
        ...
        ITM.SubItems(3) = LVAdj.ListItems(i).SubItems(3)
        
        ITM.SubItems(4) = LVAdj.ListItems(i).SubItems(4)
        ITM.ListSubItems(4).Tag = LVAdj.ListItems(i).ListSubItems(4).Tag
    Next
...
Private Sub LV_DblClick()
    ...
    For i = 1 To LVAdjAll.ListItems.count
        If Val(LVAdjAll.ListItems(i).Tag) = Val(LV.SelectedItem.Index) Then
            'Add These...
            Set ITM = LVAdj.ListItems.add(, LVAdjAll.ListItems(i).ListSubItems(2).Tag, LVAdjAll.ListItems(i).Text)
            ...
            ITM.SubItems(3) = LVAdjAll.ListItems(i).SubItems(3)
            
            ITM.SubItems(4) = LVAdjAll.ListItems(i).SubItems(4)
            ITM.ListSubItems(4).Tag = LVAdjAll.ListItems(i).ListSubItems(4).Tag
        End If
    Next
    Call GetAdjTotalQty
...
'        'Call AddToCombo(cmbLocation_QEL, "StoreName + ' ' + RackNo + ' ' + ShelfNo + ' (' + CAST(NetQty AS VARCHAR(50)) + ') {' + Remarks + '}'", "VItemsWithShelfWiseStock", " WHERE ItemID='" & strItemID & "' AND NetQty>0", , "Shelf_RefID,NetQty")
        Call AddToCombo(cmbLocation_QEL, "StoreName + ' ' + RackNo + ' ' + ShelfNo + ' (' + CAST(NetQty AS VARCHAR(50)) + ') {' + LotNo + '} '+ISNULL(Batch_No,'')", "VItemsWithLotWiseStock", " WHERE ItemID='" & strItemID & "' AND NetQty>0 AND ISNULL(Batch_No,'')<>'' ORDER BY Batch_No", , "Shelf_RefID,NetQty,EntryID,LotNo,Batch_No,Mill_Certificate_No")
'    End If
    'Call AddToCombo(cmbLocation_QEL, "Location + ' (' + CAST(NetQty AS VARCHAR(50)) + ')'", "VItemsWithLocationwiseStock", " WHERE ItemID='" & strItemID & "' AND NetQty>0", , "Location,NetQty")
    With PicEdit
...
Public Sub ShowMe(p_strOrderNo As String)
    ...
    With rs
        ...
        Do Until .EOF
            ...
            ITM.ListSubItems.add(, , Val(!InHand & "")).Bold = True
    ...
    cmbItems.ClearVals
    cmbItems.AddVals con, "DISTINCT CompItemID+' - '+ItemName", "VrptOrders_ForProduction", "CompItemID", " WHERE OrderNo='" & strOrderNo & "'"
...
'            Exit Sub
'        End If
        If Val(txtEdit) <= 0 Or Val(txtEdit) > Val(cmbLocation_QEL.List(cmbLocation_QEL.ListIndex, 2)) Then
            MsgBox "Invalid Qty.", vbInformation
            Exit Sub
        End If
        Dim ITM As ListItem
        Set ITM = LVAdj.ListItems.add(, cmbLocation_QEL.List(cmbLocation_QEL.ListIndex, 3) & "'", cmbLocation_QEL.Text)
        ITM.ListSubItems.add(, , Val(txtEdit)).Tag = cmbLocation_QEL.List(cmbLocation_QEL.ListIndex, 1)
        ITM.SubItems(3) = cmbLocation_QEL.List(cmbLocation_QEL.ListIndex, 4) & ""
        
        ITM.SubItems(4) = cmbLocation_QEL.List(cmbLocation_QEL.ListIndex, 5) & ""   'Batch No.
        ITM.ListSubItems(4).Tag = cmbLocation_QEL.List(cmbLocation_QEL.ListIndex, 6) & ""   'Mil Certif. No.
...
Private Function Saved() As Boolean
    ...
    Dim iPos As Integer, iLotPart As Integer
    Dim strItemID As String
    
    Dim strBatchNo As String, strMilNo As String, iBatch_Count As Integer, k As Integer
...
'                lLotNo = lLotNo + 1
                ...
                For j = 1 To LVAdjAll.ListItems.count
                    ...
                    With LVAdjAll.ListItems(j)
                        ...
                        If Val(.Tag) = Val(LV.ListItems(i).Index) And Val(.SubItems(5)) = 0 Then
                            dQty = 0
                            For k = 1 To LVAdjAll.ListItems.count
                                If .SubItems(4) = LVAdjAll.ListItems(k).SubItems(4) And Val(LVAdjAll.ListItems(k).Tag) = Val(LV.ListItems(i).Index) Then 'If same batch no.
                                    dQty = dQty + Val(LVAdjAll.ListItems(k).SubItems(1))
                                    iBatch_Count = iBatch_Count + 1
                                    strBatchNo = LVAdjAll.ListItems(k).SubItems(4)
                                    strMilNo = LVAdjAll.ListItems(k).ListSubItems(4).Tag
                                    LVAdjAll.ListItems(k).SubItems(5) = "1"
                                End If
                            Next
                            
                            If iBatch_Count > 1 Then
                                lLotNo = getNextLotNo
                            Else
                                lLotNo = getNextSubLotNo(.SubItems(3))
                            End If
                            
'                            strBatchNo = GetSingleStringValue("Batch_No", "Lots_List", " WHERE LotNo='" & .SubItems(3) & "'")
'                            strMilNo = GetSingleStringValue("Mill_Certificate_No", "Lots_List", " WHERE LotNo='" & .SubItems(3) & "'")
                            
                            con.Execute "INSERT INTO Lots_List(LotNo,ItemID,Lot_Type,Reference_LotNo,Batch_No,Mill_Certificate_No) VALUES('" & lLotNo & "','" & strItemID & "',4,'','" & strBatchNo & "','" & strMilNo & "')"
        
                            strMsgLots = strMsgLots & " Lot #:" & lLotNo & " Batch #:" & strBatchNo & " Item:" & .Text & vbNewLine
                                ...
                                If .SubItems(4) = LVAdjAll.ListItems(k).SubItems(4) And Val(LVAdjAll.ListItems(k).Tag) = Val(LV.ListItems(i).Index) Then   'If same batch no.
                                    With LVAdjAll.ListItems(k)
                                        ...
                                        con.Execute "INSERT INTO ItemLedgerSimple(ItemID,DT,Description,Qty,Balance,OrderNo,Location) VALUES('" & _
                                            strMaterialID & "','" & DT & "','" & theDesc & "'," & -Val(.SubItems(1)) & "," & dBal & ",'" & strOrderNo & "','" & .ListSubItems(1).Tag & "')"
                                            
                                        Call adjustItemLedger(strMaterialID, DT, -Val(.SubItems(1)))
                                        ...
                                        con.Execute "INSERT INTO IssItemsSimpleDetail(IssNo,ItemID,Qty,InHand,LedgerRefID,OrderNo,Location,Remarks,RCV_ISD_RefID,LotNo_Manual) VALUES('" & _
                                            strIssNo & "','" & strMaterialID & "'," & Val(.SubItems(1)) & "," & getItemBalance(strMaterialID, DT) - Val(.SubItems(1)) & "," & lRefID & _
                                            ",'" & strOrderNo & "','" & .ListSubItems(1).Tag & "',''," & Val(.ListSubItems(2).Tag) & ",'" & lLotNo & "')"
                                         
                                        lIISD_EntryID = GetSingleLongValue("MAX(EntryID)", "IssItemsSimpleDetail")
                                                    
                                        con.Execute "INSERT INTO StockOrderAdjustmentsDetail(SOA_RefID,Shelf_RefID,Qty,RCV_ISD_RefID) VALUES(" & lSOA_EntryID & _
                                         "," & Val(.ListSubItems(1).Tag) & "," & Val(.SubItems(1)) & "," & Val(.ListSubItems(2).Tag) & ")"
                                                     
                                        con.Execute "INSERT INTO IssItemsSimpleDetail_More(IISD_RefID,IssdQty,Shelf_RefID,Remarks) VALUES(" & _
                                         lIISD_EntryID & "," & Val(.SubItems(1)) & "," & Val(.ListSubItems(1).Tag) & ",'')"
                                                
                                        Call UpdateItemLocation(strMaterialID, Val(.ListSubItems(1).Tag), Val(.SubItems(1)))
...
Private Function Saved_Old() As Boolean
                ...
                For j = 1 To LVAdjAll.ListItems.count
                    With LVAdjAll.ListItems(j)
                        If Val(.Tag) = Val(LV.ListItems(i).Index) Then
                            '=======================================
                            'lLotNo = .SubItems(3) 'getNextLotNo()
                            dQty = Val(.SubItems(1))
'                            iPos = InStr(lLotNo, "-")
'                            If iPos > 0 Then
'                                iLotPart = Mid$(lLotNo, iPos + 1)
'                                lLotNo = Left(lLotNo, iPos - 1)
'                            Else
'                                iLotPart = 0
'                            End If
                            'lLotNo = lLotNo & "-" & (iLotPart + 1)
                            lLotNo = getNextSubLotNo(.SubItems(3))
                            ...
                            con.Execute "INSERT INTO StockOrderAdjustments(OrderNo,ItemID,Qty,UserName,MachineName,VID_RefID,Shelf_RefID) VALUES('" & _
                                 strOrderNo & "','" & strItemID & "'," & dQty & ",'" & CurrentUserName & "','" & strComputerName & _
                                 "'," & lVID_EntryID & "," & Val(.ListSubItems(1).Tag) & ")"
                            ...
                            con.Execute "INSERT INTO ItemLedgerSimple(ItemID,DT,Description,Qty,Balance,OrderNo,Location) VALUES('" & _
                             strMaterialID & "','" & DT & "','" & theDesc & "'," & -dQty & "," & dBal & ",'" & strOrderNo & "','" & .ListSubItems(1).Tag & "')"
