...
Begin VB.Form frmAuthorizeIssRcv 
   AutoRedraw      =   -1  'True
   ClientHeight    =   9525
   ClientLeft      =   0
   ClientTop       =   285
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.ComboBox cmbfield 
            Height          =   315
            ItemData        =   "frmAuthorizeIssRcv.frx":0000
            Left            =   60
            List            =   "frmAuthorizeIssRcv.frx":0002
      ...
      Begin TabDlg.SSTab SSTab1 
         ...
         TabPicture(0)   =   "frmAuthorizeIssRcv.frx":0004
         Tab(0).ControlEnabled=   0   'False
         Tab(0).Control(0)=   "LVIss"
         Tab(0).ControlCount=   1
         TabCaption(1)   =   "Authorize Received"
         TabPicture(1)   =   "frmAuthorizeIssRcv.frx":0020
         ...
         Tab(1).Control(3)=   "FraEdit"
         Tab(1).Control(3).Enabled=   0   'False
         Tab(1).Control(4)=   "FraWastageOwn"
         Tab(1).Control(4).Enabled=   0   'False
         Tab(1).ControlCount=   5
         TabCaption(2)   =   "Authorize Raw Material"
         TabPicture(2)   =   "frmAuthorizeIssRcv.frx":003C
         ...
         Begin VB.Frame FraEdit 
            ...
            Begin MSForms.Label lblmine 
               ...
               Width           =   10830
               BackColor       =   11517387
               Caption         =   $"frmAuthorizeIssRcv.frx":0058
               Size            =   "19103;423"
               BorderColor     =   16512
               BorderStyle     =   1
               FontName        =   "Tahoma"
   ...
   End
End
Attribute VB_Name = "frmAuthorizeIssRcv"
...
Private Sub cmdOK_Click()
    ...
    LVRcv.SelectedItem.SubItems(9) = Val(txtWastage)
...
Private Sub UpdateRcvEntries()
    ...
    For i = 1 To LVRcv.ListItems.count
        If LVRcv.ListItems(i).Checked Then
            ...
            lRepair = Val(LVRcv.ListItems(i).SubItems(8))
            lWastage = Val(LVRcv.ListItems(i).SubItems(9))
            lLost = Val(LVRcv.ListItems(i).SubItems(10))
            lReWork = Val(LVRcv.ListItems(i).SubItems(11))
            
            con.Execute "UPDATE VendRcvdDetail SET ReqAuth=0,Wastage=" & lWastage & ",RepairAmt=" & _
             lRepair & ",LostQty=" & lLost & ",ReWorkQty=" & lReWork & " WHERE EntryID=" & Val(LVRcv.ListItems(i).key)
...
'                Else
                    ...
                    .Parameters("@Qty").Value = Val(LVRcv.ListItems(i).SubItems(7))
...
'                                    strVchrNo = objVchr.PostVoucher(False, True)
            ...
            ElseIf strCompany = "QEL" Then
                If (lWastage > 0 Or lRepair > 0) And Val(LVRcv.ListItems(i).ListSubItems(9).Tag) > 0 Then '
                    Dim lmyEntryID As Long, lmyVendID As Long, dmyRate As Double
                    lmyEntryID = Val(LVRcv.ListItems(i).key)
                    lmyVendID = Val(LVRcv.ListItems(i).Tag)
                    dmyRate = GetSingleDoubleValue("Rate", "VVendIssdDetail", " WHERE EntryID=" & lmyEntryID)
                    con.Execute "INSERT INTO MakerRepair(VendID,DT,Rcvd_RefID,Qty,Rate,UserName,MachineName,VendIssdDetail_RefID,RepairAmt) VALUES(" & _
                     lmyVendID & ",'" & GetServerDate(False) & "'," & Val(LVRcv.ListItems(i).key) & "," & lWastage & _
                     "," & dmyRate & ",'" & CurrentUserName & "','" & strComputerName & "'," & lmyEntryID & "," & lRepair & ")"
                End If
            End If
        End If
...
'        .Close
        ...
        .Open "Select * From VVendRcvItemsrpt Where (ReqAuth=1) Order By ProcessID Desc,DT Asc", con, adOpenForwardOnly, adLockReadOnly
...
Private Sub LVRcv_DblClick()
    ...
    If iColNo < 9 Or iColNo = 10 Then Exit Sub
