...
Begin VB.Form frmQMSDocument_Ex_AE 
   ...
   Begin VB.Label lblDocTypes 
      ...
      Height          =   195
      Left            =   2910
      MouseIcon       =   "frmQMSDocument_Ex_AE.frx":0000
   ...
   End
End
Attribute VB_Name = "frmQMSDocument_Ex_AE"
...
Private Function SaveItem() As Boolean
    ...
    If lEntryID = 0 Then
        Sql = CreateInsertSQL("QMS_Documents_Ex")
    Else
        Sql = CreateUpdateSQL("QMS_Documents_Ex")
        Sql = Sql & " WHERE EntryID=" & lEntryID
    End If
    
    con.Execute Sql
    If lEntryID = 0 Then
        lmyEntryID = GetSingleLongValue("MAX(EntryID)", "QMS_Documents_Ex")
        con.Execute "UPDATE QMS_Documents_Ex SET Master_DocumentType='" & strMaster_DocType & "' WHERE EntryID= " & lmyEntryID
    Else
        lmyEntryID = lEntryID
    ...
    If strDelUsers <> "" Then
        ...
        con.Execute "DELETE FROM QMS_Documents_Ex_DistributionList WHERE EntryID IN(" & strDelUsers & ")"
    End If
    Dim i As Integer
    For i = 1 To LV.ListItems.count
        With LV.ListItems(i)
            If Val(.ListSubItems(1).Tag) = 0 Then
                con.Execute "INSERT INTO QMS_Documents_Ex_DistributionList(QMS_RefID,EmpID) VALUES(" & lmyEntryID & ",'" & .Tag & "')"
            End If
        End With
    Next
    con.CommitTrans
...
Private Sub ShowData(p_lEntryID As Long, Optional bCopy As Boolean = False)
    ...
    With rs
        ...
        Me.Caption = !Master_DocumentType & ""
        .Close
        
        Dim ITM As ListItem
        .Open "SELECT * FROM VQMS_Documents_Ex_DistributionList WHERE QMS_RefID=" & p_lEntryID, con, adOpenForwardOnly, adLockReadOnly
        Do Until .EOF
...
Public Sub AddNew(p_strMaster_DocType As String)
    strMaster_DocType = p_strMaster_DocType
    TBox(14) = "" 'GetNewDocumentNo()
    Call AddToCombo(cmbDocumentType, "DocumentType", "QMS_Master_DocumentTypes_DocTypes", " WHERE MasterDocumentType='" & strMaster_DocType & "'")
    Me.Caption = strMaster_DocType
...
Private Sub AttachPDF(p_lEntryID As Long)
    ...
    If strPDFPath <> "" And txtFilePath.Tag = "" Then
        ...
        Rec.Open "SELECT EntryID,AttachmentPDF,AttachedPDFFileName FROM QMS_Documents_Ex WHERE EntryID=" & p_lEntryID, con, adOpenStatic, adLockOptimistic
...
Private Sub lblDocTypes_Click()
    Call ShowQMSDocumentTypes_Ex(strMaster_DocType)
    Call AddToCombo(cmbDocumentType, "DocumentType", "QMS_Master_DocumentTypes_DocTypes", " WHERE MasterDocumentType='" & strMaster_DocType & "'")
End Sub
