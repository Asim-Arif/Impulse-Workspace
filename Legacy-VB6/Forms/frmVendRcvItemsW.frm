...
Begin VB.Form frmVendRcvItemsW 
   ...
   Begin VB.Frame FALL 
      ...
      Begin MSForms.Label Label2 
         ...
         ForeColor       =   4194304
         BackColor       =   12504280
         Caption         =   $"frmVendRcvItemsW.frx":0000
         Size            =   "18177;397"
   ...
   End
End
Attribute VB_Name = "frmVendRcvItemsW"
...
Private Sub cmbVend_matched()
    ...
    With rs
        .Open "Select ItemCode,ItemDescription,CompItemID,Sum(IssQty-RcvdQty) as Quantity From VVendIssItems1 Where IssQty-RcvdQty>0 and VendID=" & cmbVend.ID & " Group By ItemCode,ItemDescription,CompItemID", con, adOpenForwardOnly, adLockReadOnly
    ...
    RcvdID = Val(con.Execute("Select Max(right(RecieptNo,3)) From VendReceived Where DT='" & Format(Date, "dd-MMM-yyyy") & "'").Fields(0).value & "")
        
    RcvdID = "RCV-" & Val(cmbVend.ID) & "-" & _
     Format(Date, "ddMMyy") & "-" & Format(Val(RcvdID) + 1, "000")
...
Private Sub txtWastage_KeyPress(KeyAscii As Integer)
    ...
    On Error GoTo err
    If KeyAscii = 13 Then
        If Val(txtQty) + Val(txtWastage) > Val(LV.SelectedItem.SubItems(3)) Then
            MsgBox "Invalid Quantities!", vbInformation
            Exit Sub
        End If
        LV.SelectedItem.SubItems(4) = Val(txtQty)
        LV.SelectedItem.SubItems(5) = Val(txtWastage)
    ...
    For i = 1 To LV.ListItems.Count
            ...
            If Val(LV.ListItems(i).SubItems(6)) > 0 Then
                ...
                With cmd
                    ...
                    .Parameters("@WasteQty").value = Val(LV.ListItems(i).SubItems(5) & "")
