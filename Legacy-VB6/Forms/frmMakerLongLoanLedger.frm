...
Begin VB.Form frmMakerLongLoanLedger 
   ClientHeight    =   7725
   ClientLeft      =   60
   ClientTop       =   240
   ClientWidth     =   11550
   ...
   Begin VB.Frame fTitle 
      ...
      Begin VB.Label Label1 
         ...
         Caption         =   "Vendor Long Term Loan Ledger"
         BeginProperty Font 
   ...
   Begin VB.Label lbl 
      ...
      Caption         =   $"frmMakerLongLoanLedger.frx":0000
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Image Image1 
      Height          =   375
      Left            =   10200
      Picture         =   "frmMakerLongLoanLedger.frx":00A6
      Top             =   1395
      Width           =   465
   End
   ...
   End
End
Attribute VB_Name = "frmMakerLongLoanLedger"
...
Private Sub cmdPrint_Click()
    ...
    lOpenBalance = GetSingleLongValue("SUM(Amount)", "VMakerAdvancesLedger", " WHERE VendID=" & cmbMaker.ID & " AND DT<'" & DtFrom.Value & "'")
    
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\MakerAdvancesLedger_Long.rpt")
    rpt.FormulaFields.GetItemByName("OpeningBalance").Text = lOpenBalance
    Dim strSelection As String
    strSelection = "{VMakerAdvancesLedger_Short.VendID}=" & cmbMaker.ID & " AND {VMakerAdvancesLedger_Short.DT}=#" & DtFrom.Value & "# TO #" & DtTo.Value & "#"
...
Private Sub RefreshLedger()
    ...
    With rsLedger
        ...
        .Open "SELECT * FROM VMakerAdvancesLedger WHERE VendID=" & cmbMaker.ID & " AND DT BETWEEN '" & DtFrom.Value & "' AND '" & DtTo.Value & "' ORDER BY DT,Amount DESC", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        ...
        Balance = GetSingleLongValue("SUM(Amount)", "VMakerAdvancesLedger", " WHERE VendID=" & cmbMaker.ID & " AND DT<'" & DtFrom & "'")
        ...
        For i = 0 To .RecordCount - 1
            ...
            ITM.Tag = !EntryID & ""
            strDescription = ![Description] & "" & IIf(!Remarks & "" = "", "", " (" & !Remarks & "" & ")")
            ITM.ListSubItems.add(, , strDescription).Tag = ![DT] & ""
            ITM.SubItems(2) = Val(![Amount] & "")
            ITM.SubItems(3) = ![AccVoucherNo] & ""
            Balance = Balance + Val(![Amount] & "")
            ITM.SubItems(4) = Balance
            ITM.SubItems(5) = IIf(Val(![dAmount] & "") = 0, "", ![dAmount])
