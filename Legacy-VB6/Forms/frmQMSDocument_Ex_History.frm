...
Begin VB.Form frmQMSDocument_Ex_History 
   ...
   End
End
Attribute VB_Name = "frmQMSDocument_Ex_History"
...
Private Sub ShowData(p_lEntryID As Long, Optional bCopy As Boolean = False)
    ...
    With rs
        ...
        Call AddToCombo(cmbDocumentType, "DocumentType", "QMS_Master_DocumentTypes_DocTypes", " WHERE MasterDocumentType='" & !Master_DocumentType & "'")
        For Each c In Controls
            If c.Tag <> "" Then
            ...
            End If
        Next
        Me.Caption = !Master_DocumentType & " History"
        .Close
        
        Dim ITM As ListItem
        .Open "SELECT * FROM QMS_Documents_Ex_ChangeRequests WHERE QMS_RefID=" & p_lEntryID & " AND Approved=1", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        Do Until .EOF
...
Private Sub LV_DblClick()
    ...
    Dim lCount As Long
    lCount = GetSingleLongValue("COUNT(*)", "QMS_Documents_Ex_ChangeRequests", " WHERE EntryID=" & Val(LV.SelectedItem.ListSubItems(1).Tag) & " AND " & strFieldName & " IS NOT NULL")
    
    If lCount = 0 Then
        MsgBox "No PDF attached.", vbInformation
        Exit Sub
    Else
        Call ShowTechDrawing("QMS_Documents_Ex_ChangeRequests", strFieldName, " WHERE EntryID=" & Val(LV.SelectedItem.ListSubItems(1).Tag), LV.SelectedItem.SubItems(iColNo - 1), CD1)
    End If
