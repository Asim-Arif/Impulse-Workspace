...
Begin VB.Form frmOverTimeAuthOld 
   BorderStyle     =   0  'None
   ClientHeight    =   8685
   ClientLeft      =   15
   ClientTop       =   15
   ...
   Begin VB.Frame FLV 
      ...
      Begin VB.Frame FButs1 
         ...
         Begin MSForms.CommandButton cmdSave 
            ...
            PicturePosition =   327683
            Size            =   "2302;635"
            Picture         =   "frmOverTimeAuthOld.frx":0000
            Accelerator     =   83
            FontName        =   "Tahoma"
            FontEffects     =   1073741825
            FontHeight      =   165
      ...
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         Caption         =   "Work To Do In This OverTime"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   178
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmOverTimeAuthOld.frx":0112
            Key             =   "Present"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmOverTimeAuthOld.frx":0564
            Key             =   "Absent"
         EndProperty
      EndProperty
   End
   ...
   End
End
Attribute VB_Name = "frmOverTimeAuthOld"
...
Private Sub cmdSave_Click()
    ...
    con.Execute "Delete From OverTimeAuthMain Where DeptID='" & cmbDept.ID & "' AND DT='" & DTAttend.value & "'"
    'con.Execute "Delete From OverTimeAuth Where DeptID='" & cmbDept.ID & "' AND DT='" & DTAttend.value & "'"
...
'    con.Execute "Update AttendanceSheet Set ExtraHrs=null Where DT='" & DTAttend & "'" & DeptCond
    con.Execute "Insert Into OverTimeAuthMain(DeptID,DT,WorkToBeDone,UserName,MachineName) Values('" & _
     cmbDept.ID & "','" & DTAttend.value & "','" & Replace(txtWorkToDo, "'", "''") & "','" & _
     CurrentUserName & "','" & strComputerName & "')"
    
    Dim lRefID As Long
    lRefID = Val(con.Execute("Select Max(EntryID) From OverTimeAuthMain").Fields(0).value & "")
...
'                lTemp = lHrs - (LHrs1 * 60)
                ...
                con.Execute "INSERT INTO OverTimeAuth(RefID,EmpID,Rate,Authority,OTHrs) VALUES(" & _
                 lRefID & ",'" & .Text & "'," & Val(.SubItems(2)) & ",1," & Val(.SubItems(3)) & ")"
...
Private Sub FillList()
    ...
    With cmd
        ...
        .CommandText = "SP_OverTimeDept"
        .Parameters("@DeptID").value = cmbDept.ID
        .Parameters("@AttDate").value = DTAttend.value
        Set rsList = .Execute
    End With
    ...
    With rsList
        '.Open "Select EmpID,DeptID,Name,FName,Rel,StartingSalary From Employees Where DeptID='" & cmbDept.ID & "' AND EmpID In(Select EmpID From AttendanceSheet  Where Attendance=1 AND DT='" & DTAttend.value & "')", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        Do Until .EOF
            Set ITM = LV.ListItems.add(, !EmpId, !EmpId & "")
            ITM.ListSubItems.add , , !Name & " " & !Rel & " " & !FName
            ITM.ListSubItems.add , , (Val(!StartingSalary & "") / iWorkingHrs) * 2
            ITM.ListSubItems.add().ForeColor = vbBlue
            ITM.ListSubItems.add().ForeColor = vbRed
            ITM.ListSubItems.add , , Val(!TotalOTHrs & "")
            ITM.ListSubItems.add , , 12 - Val(!TotalOTHrs & "")
             .MoveNext
        Loop
        .Close
        ...
        txtWorkToDo = ""
        .Open "SELECT * FROM VOverTimeAuth WHERE DT='" & DTAttend & "' AND DeptID='" & cmbDept.ID & "' ORDER BY EmpID", con, adOpenForwardOnly, adLockReadOnly
        
        Do Until .EOF
            txtWorkToDo = !WorkToBeDone & ""
            With LV.ListItems(!EmpId & "")
                .Checked = True
                .SubItems(3) = Val(rsList!OTHrs & "")
                .SubItems(4) = Val(rsList!OTHrs & "") * Val(.SubItems(2))
            End With
            .MoveNext
        Loop
        .Close
