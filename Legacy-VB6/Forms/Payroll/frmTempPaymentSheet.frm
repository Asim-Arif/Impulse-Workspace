...
Begin VB.Form frmTempPaymentSheet 
   BorderStyle     =   0  'None
   ClientHeight    =   7785
   ClientLeft      =   15
   ClientTop       =   195
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmTempPaymentSheet.frx":0000
            Key             =   "Present"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmTempPaymentSheet.frx":0452
            Key             =   "Absent"
         EndProperty
      EndProperty
   End
   ...
   Begin VB.Menu mnupop 
      ...
      End
      Begin VB.Menu mnuPrintWageRecord 
         Caption         =   "Print Wage Record"
      End
      Begin VB.Menu dash 
         Caption         =   "-"
      End
      ...
      End
      Begin VB.Menu mnuPrintWageSlips 
         Caption         =   "Print Wage Slips"
      End
   End
End
Attribute VB_Name = "frmTempPaymentSheet"
...
Private Sub PrintReport(Optional iReport As Integer = 0, Optional Crpt As CrystalReport = Nothing, Optional Summary As Boolean)
    ...
    If iReport = 0 Then
        Set rpt = rptApp.OpenReport(rptPayrollPath & "\rptTempPaymentSheet.rpt")
        Cond = "{VPaySlip.PerPieceRate}=False"
    ElseIf iReport = 1 Then
        Set rpt = rptApp.OpenReport(rptPayrollPath & "\rptTempPayslip.rpt")
        'rpt.SQLQueryString = SQL
        Cond = "{VPaySlip.PerPieceRate}=False"
    ElseIf iReport = 2 Then
        Set rpt = rptApp.OpenReport(rptPayrollPath & "\rptWageSlip.rpt")
        Cond = "Month({EmpPerPieceWorkDone.DT})=" & cmbMonth.ListIndex + 1 & " AND Year({EmpPerPieceWorkDone.DT})=" & cmbYear & " AND {VPaySlip.PerPieceRate}=True"
    ElseIf iReport = 3 Then
        Set rpt = rptApp.OpenReport(rptPayrollPath & "\rptWageRecord.rpt")
        Cond = "{VPaySlip.PerPieceRate}=True "
    End If
...
Private Sub cmdUpdate_Click()
    ...
    If cmdUpdate.Caption = "Make This Sheet Final" Then
        ...
        If getDBPassword("Others") = False Then
            Exit Sub
        End If
        ...
        cmdUpdate.Caption = "Delete Payment Sheet"
        
    ElseIf cmdUpdate.Caption = "Delete Payment Sheet" Then
        
        If MsgBox("Are you Sure To Delete Payment Sheet?", vbQuestion + vbYesNo) = vbNo Then Exit Sub
        If getDBPassword("Others") = False Then
            Exit Sub
        End If
        ...
        con.Execute "Delete From TempMonthlySalaries Where Month(DT)=" & cmbMonth.ListIndex + 1 & " AND Year(DT)=" & cmbYear
        con.Execute "Delete From TempOldAgeMonthly Where Month(DT)=" & cmbMonth.ListIndex + 1 & " AND Year(DT)=" & cmbYear
        con.Execute "Delete From TempSSMonthly Where Month(DT)=" & cmbMonth.ListIndex + 1 & " AND Year(DT)=" & cmbYear
...
Private Sub Form_Load()
    ...
    cmbDept.AddVals con, "Name", "Departments", "DeptID", " Where IsNull(TempDept,0)=1"
...
Private Sub AddEmployees()
    ...
    If cmbDept.Text = "<All Departments>" Then
        cmbEmp.AddVals con, "Name", "VEmp", "EmpID", " Where JOINDate<='" & LastDT & "' AND (DiscontinuedOn>'" & FirstDT & "' OR DiscontinuedOn Is Null) AND TempDept=1"
    Else
        cmbEmp.AddVals con, "Name", "Employees", "EmpID", "Where DeptID='" & cmbDept.ID & "' and JOINDate<='" & LastDT & "' AND (DiscontinuedOn>'" & FirstDT & "' OR DiscontinuedOn Is Null)"
    End If
...
Private Sub RefreshLedger()
    ...
    With rs
        .Open "Select Count(EmpID) From TempMonthlySalaries Where Month(DT)=" & cmbMonth.ListIndex + 1 & " AND Year(DT)=" & cmbYear.Text & " AND IsForSA=0", con, adOpenForwardOnly, adLockReadOnly
        If Val(.Fields(0) & "") = 0 Then
            IsInternalFinal = False
        Else
            IsInternalFinal = True
            'To Save Long term Loan Information....
            rsInternal.Open "Select EmpID,LongTerm,PrevLTLoan From TempMonthlySalaries Where Month(DT)=" & cmbMonth.ListIndex + 1 & " AND Year(DT)=" & cmbYear.Text & " AND IsForSA=0", con, adOpenStatic, adLockReadOnly
        End If
        .Close
    End With
    ...
    JDate = DTPicker1.value 'DateSerial(cmbYear, cmbMonth.List(cmbMonth.ListIndex, 1), TotalSalDays)
        
    'Sql = "Select * from VAdvanceSalary " & Cond & " JoinDate<='" & Format(JDate, "dd-MMM-yyyy") & "' Order By DeptID,EmpID"
    
    cmd.CommandType = adCmdStoredProc
    
    cmd.CommandText = "SP_TempEmpPaymentSheet"
...
'            End If
           ...
           ' itm.SubItems(15) = IIf((Exempt), "0", Format((LateHours * (Rate / Val(!WorkingHrs & ""))), "0.00"))
            ...
            ''''''''''''''''''''''''''''''''''''''''''
                ...
                If IsNull(![Lunch]) = False And EmpSalary <> 0 Then
...
Dim rs As New ADODB.Recordset
With rs
    .Open "Select * from TempMonthlySalaries", con, adOpenStatic, adLockOptimistic
    For i = 1 To LV.ListItems.Count
        .AddNew
...
Private Function GetFakeWorkingHrs(ITM As ListItem) As Double
    ...
    Else
        myRetVal = 0 'Val(ITM.SubItems(9)) / Val(ITM.ListSubItems(6).Tag)
    End If
...
Private Sub mnuPrintWageRecord_Click()
    Call PrintReport(3)
End Sub

Private Sub mnuPrintWageSlips_Click()
    Call PrintReport(2)
End Sub
...
Private Sub InsertRecords()
    ...
    con.Execute "Delete from PrintTempSalary"
    con.Execute "Delete From PrintTempWageRecord"
    
    Dim rs As New ADODB.Recordset
    With rs
        .Open "Select * from PrintTempSalary", con, adOpenStatic, adLockOptimistic
        For i = 1 To LV.ListItems.Count
            .AddNew
            ![EmpId] = LV.ListItems(i).Text & ""
            'Temp Marked Commented"' AND Month(DT)=" & cmbMonth.ListIndex + 1 & " AND Year(DT)=" & cmbYear & _
            'Insert Work
            con.Execute "Insert Into PrintTempWageRecord Select EmpID,CustCode,Country,ItemID" & _
             ",Rate,Sum(Qty) As TotalQty From EmpPerPieceWorkDone Where EmpID='" & !EmpId & _
             "' AND DT='" & DTPicker1.value & "'" & _
             " Group By EmpID,CustCode,Country,ItemID,Rate"
            ...
            ![MaternityLeaves] = Val(ArrLeaves(5))
            ![HrsPerDay] = Val(LV.ListItems(i).ListSubItems(3).Tag)
            ![LunchAmt] = Val(LV.ListItems(i).ListSubItems(25))
...
Private Function FinalizedSalarySheetLoaded() As Boolean
    ...
    With rs
        .Open "Select * From VTempMonthlySalaries " & Cond, con, adOpenForwardOnly, adLockReadOnly
