...
Begin VB.Form SalarySheetExt 
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "SalarySheetExt.frx":0000
            Key             =   "Present"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "SalarySheetExt.frx":0452
            Key             =   "Absent"
         EndProperty
      EndProperty
   End
   Begin VB.Frame FLV 
      ...
      Begin VB.Frame fButs1 
         ...
         Begin MSForms.CommandButton cmdSave 
            ...
            Width           =   2160
            ForeColor       =   0
            Caption         =   "Make This Sheet Fianal"
   ...
   Begin VB.Menu mnupop 
      ...
      Begin VB.Menu mnuSalSheet 
         Caption         =   "Print External Salary Sheet"
      End
      Begin VB.Menu lnRegister 
         Caption         =   "-"
      End
      Begin VB.Menu mnuPrintAttRegister 
         Caption         =   "Print AttendanceRegister"
      End
      Begin VB.Menu ln1 
         Caption         =   "-"
      End
   ...
   End
End
Attribute VB_Name = "SalarySheetExt"
...
With Crpt
    ...
    '.DataFiles(0) = DatabasePath
      .Connect = ConnectStr
      .Formulas(0) = "Fromto='" & cmbMonth.Tag & "-" & cmbYear.Tag & IIf(chkExternal = vbChecked, "", " ( For Factory Use Only )") & "'"
...
Me.PopUpMenu mnupop, 2, cmdPrint.Left + FLV.Left + fButs1.Left, fButs1.Top + cmdPrint.Top + 150 + cmdPrint.Height
...
Else
    ...
    For i = 1 To LV.ListItems.Count
        ...
        con.Execute "Update MonthlySalaries Set Payed=" & setVal & ",NetSalary=" & Val(LV.ListItems(i).SubItems(19)) + Val(LV.ListItems(i).SubItems(18)) & _
...
Private Sub cmbdept_matched()
    ...
    If cmbDept.Text = "<All Departments>" Then
        cmbEmp.AddVals con, "Name", "Employees", "EmpID", "WHERE Social=1"
    Else
        cmbEmp.AddVals con, "Name", "Employees", "EmpID", "Where DeptID='" & cmbDept.ID & "' AND Social=1"
    End If
    cmbEmp.ID = 0
...
Private Function IsFinal() As Boolean

Dim rec As New ADODB.Recordset
Dim ITM As ListItem, Total As Double

LV.ListItems.Clear

With rec
    rec.CursorLocation = adUseClient
    rec.Open "SELECT * FROM VMonthlyExtSalaries WHERE MONTH(DT)=" & cmbMonth.ListIndex + 1 & " AND YEAR(DT)=" & cmbYear & "", con, adOpenKeyset, adLockReadOnly
    If rec.RecordCount > 0 Then
        IsFinal = True
        cmdSave.Visible = False
        For i = 1 To rec.RecordCount
            Set ITM = LV.ListItems.Add(, , ![EmpId] & "")
            ITM.ListSubItems.Add , , rec!Name & ""
            ITM.ListSubItems.Add , , rec!Salary & ""
            ITM.ListSubItems.Add , , rec!Rate & ""
            ITM.ListSubItems.Add , , rec!AbsentDays & ""
            ITM.ListSubItems.Add , , rec!AbsentAmt & ""
            ITM.ListSubItems.Add , , rec!SalDays & ""
            ITM.ListSubItems.Add , , rec!SalAmt & ""
            ITM.ListSubItems.Add , , rec!OverTimeHrs & ""
            ITM.ListSubItems.Add , , rec!OverTimeAmt & ""
            ITM.ListSubItems.Add , , rec!LateHrs & ""
            ITM.ListSubItems.Add , , rec!LateAmt & ""
            ITM.ListSubItems.Add , , rec!Total & ""
            ITM.ListSubItems.Add , , rec!TAX & ""
            ITM.ListSubItems.Add , , rec!TAmt & ""
            ITM.ListSubItems.Add , , rec!NetTotal & ""
            ITM.ListSubItems.Add , , rec!SLoan & ""
            ITM.ListSubItems.Add , , rec!LongLoan & ""
            ITM.ListSubItems.Add , , rec!AdvSal & ""
            ITM.ListSubItems.Add , , rec!SSec & ""
            ITM.ListSubItems.Add , , rec!Balance & ""
...
Dim GrandTotal As Double
    ...
    'Dim cmd As New ADODB.Command
        ...
        Dim EmpExtSal As Double, TotalSal As Double, SingleExtSal As Double
        
        'Will Select The Sum Of All The Persons Where Social=0 and They Are Also Ommited
        TotalSal = Val(con.Execute("SELECT TotalExtSal FROM FExtSal('" & JDate & "')").Fields(0) & "")
        TotalSal = TotalSal - Val(con.Execute("SELECT TotalExtSal FROM FExtAdvSal('" & JDate & "')").Fields(0) & "")
    ...
    With cmd
        ...
        .CommandText = "VExternalSalary"
        
        Set .ActiveConnection = con
        .Parameters.Refresh
        .Parameters("@ParDate").Type = adDBTimeStamp
    ...
    With rsCheck
        ...
        SingleExtSal = 0 'Round(TotalSal / .RecordCount, 2)
    ...
    With rsCheck
        ...
        LV.Visible = False
        TotalSalDays = DateAdd("m", 1, SDate) - SDate
        GrandTotal = ShowVenders(TotalSal, TotalSalDays)
        
        If .RecordCount = 0 Then LV.Visible = True: LV.ListItems.Clear: Exit Sub
...
'            End If
            ...
            ITM.SubItems(2) = EmpExtSal
            Rate = Format(Val(EmpExtSal) / LastDay, "0.00")
            ITM.SubItems(3) = Rate
            
            On Error Resume Next
            PossOverTime = TotalSalDays - TotalSundays(cmbMonth.ListIndex + 1, cmbYear)
            Holidays = Val(con.Execute("SELECT COUNT(DT) FROM Holidays WHERE MONTH(DT)= " & cmbMonth.ListIndex + 1 & " AND YEAR(DT)=" & cmbYear & "").Fields(0) & "")
            On Error GoTo 0
            
            PossOverTime = (PossOverTime - Holidays) * OverTimeHours
                
                
            
            
            If Round(NetSalary / Rate) > NetSalary / Rate Then
                SalDays = Int(NetSalary / Rate) + 0.5
            Else
                SalDays = Int(NetSalary / Rate)
            End If
...
'            End If
            ...
            EmpSalary = Round(SalDays * Rate)
            ITM.SubItems(7) = EmpSalary
            
            
            Dim OverTimeAmt As Double, OverTimePerHr As Double, CalcOverTime As Integer
            
            Randomize
            CalcOverTime = ((SalDays - Int(Rnd * (5 - (1 - 1)) + 1)) * 40 / 100) * 6
            TotalSal = TotalSal + NetSalary - EmpSalary
            If TotalSal <= 0 Then CalcOverTime = 0
            
            OverTimePerHr = Rate / OverTimeHours
            
            'Will Increase The Adjustment Amount By The Salary Difference


            
            If CalcOverTime * OverTimePerHr > TotalSal Then
                CalcOverTime = Round(TotalSal / OverTimePerHr, 2)
                TotalSal = 0
            'Else
                'OverTimeHours = (!OverTime) + (OverTimeAmt / OverTimePerHr) + CalcOverTime
            End If
            
            'Will Zero Over Time If It Is Less Than Zero
            If CalcOverTime < 0 Then CalcOverTime = 0
            
            OverTimeHours = Val(![OverTime] & "") + CalcOverTime
            OverTimeAmt = Round(OverTimeHours * OverTimePerHr, 2)
            TotalSal = IIf(TotalSal = 0, 0, TotalSal - OverTimeAmt)
            
            'Will Calculate The OverTime Of Employee
            If OverTimeHours > PossOverTime Then
                MsgBox "OverTime Hours For Employee " & !Name & " {" & !EmpId & "} Are Not Possible" & _
                    vbNewLine & "Possbile OverTime Hours Are :" & PossOverTime, vbInformation
            End If
            
            ITM.SubItems(8) = getHrsMin(OverTimeHours)
            ITM.SubItems(9) = OverTimeAmt
            ...
            TaxSal = Round(Rate * TotalSalDays)
            ...
            ITM.SubItems(20) = Round((Total - Val(!AdvSalpaid & "") - SocialAmt - LongLoan) / 10, 0) * 10
...
NEXT_EMPLOYEE:
        ...
        If TotalSal > 0 Then
            MsgBox "Rs. " & TotalSal & " Could Not Be Adjusted. To Adjust This Amount Please Increase Vendes.", vbInformation
        End If
...
Private Sub Computed(Emp As String, NetSalary As Double, ByRef IsComputed As Boolean, ByRef TaxPer As Double, ByRef AbsentDays As Integer, ByRef OverTimeHours As Double, ByRef NTime As Integer, ByRef LTime As Double, ByRef LongLoan As Double, ByRef DT As String, ByRef Payed As Boolean, ByRef Transfered As Boolean, ToDate As Date, ByRef EmpSal As Double, Optional EmpExtSal As Double, Optional IsActive As Boolean)
...
Private Function ShowVenders(TotalSal As Double, TotalDays) As Double

If TotalDays = 0 Then Exit Function

Dim rec As New ADODB.Recordset, AmtAdj As Double
Dim Absents As Integer, Presents As Integer, Salary As Double, Rate As Double, EmpSalary As Double
Dim ITM As ListItem

With rec
    .CursorLocation = adUseClient
    .Open "SELECT * FROM Employees WHERE Vender=1", con, adOpenKeyset, adLockReadOnly
    For i = 1 To .RecordCount
        Salary = Val(!Salary & "")
        Rate = Round(Salary / TotalDays, 2)
        Presents = Int(Rnd * (TotalDays - (20 - 1)) + 20)
        Absents = TotalDays - Presents
        EmpSalary = Round(Rate * Presents, 2)
        
        Set ITM = LV.ListItems.Add(, , !EmpId & "")
        ITM.ListSubItems.Add , , !Name
        ITM.ListSubItems.Add , , Salary
        ITM.ListSubItems.Add , , Rate
        ITM.ListSubItems.Add , , Absents
        ITM.ListSubItems.Add , , Round(Absents * Rate, 2)
        ITM.ListSubItems.Add , , Presents
        ITM.ListSubItems.Add , , EmpSalary
        ...
        ITM.ListSubItems.Add , , "0"
        ITM.ListSubItems.Add , , "0"
        ITM.ListSubItems.Add , , Round(EmpSalary)
        AmtAdj = AmtAdj + Round(EmpSalary)
        .MoveNext
    Next
    TotalSal = TotalSal - AmtAdj
...
If MsgBox("This Will Final External Salary Sheet. After This You Cant Cant Change This Sheet." & vbNewLine & "Do You Want To Continue", vbInformation + vbYesNo) = vbYes Then
    For i = 1 To LV.ListItems.Count
        With LV.ListItems(i)
            con.Execute "INSERT INTO MonthlyExtSalaries VALUES('" & .Text & "','" & DT _
            & "'," & .ListSubItems(2) & "," & .ListSubItems(3) & "," & .ListSubItems(4) _
            & "," & .ListSubItems(5) & "," & .ListSubItems(6) & "," & .ListSubItems(7) _
            & ",'" & .ListSubItems(8) & "'," & .ListSubItems(9) & ",'" & .ListSubItems(10) _
            & "'," & .ListSubItems(11) & "," & .ListSubItems(12) & "," & .ListSubItems(13) _
            & "," & .ListSubItems(14) & "," & .ListSubItems(15) & "," & .ListSubItems(16) _
            & "," & .ListSubItems(17) & "," & .ListSubItems(18) & "," & .ListSubItems(19) _
            & "," & .ListSubItems(20) & ")"
...
Private Sub LV_DblClick()
    ...
    Else
        ...
        LV.SelectedItem.SubItems(19) = Val(LV.SelectedItem.SubItems(19)) + Val(LV.SelectedItem.SubItems(17)) - Abs(Val(Amt))
...
Private Sub mnuPrintAttRegister_Click()
...
Dim AbsentDays As Double, SalDays As Double
Dim TotalDays As Double, DateArr() As Date
Dim Holidays() As Integer

Dim ToDate As Date
   
   'Will Delete The Previos Attendance Register Entries
   con.Execute "Delete From PrintAttReg"
   
   'Will Insert All The Employee's ID(That Are In the List) Into The Table
    For k = 1 To LV.ListItems.Count
      con.Execute "Insert into PrintAttReg (EmpID) values ('" & LV.ListItems(k) & "')"
    Next
    
    
   'Will Update All The Days To Present
   TotalDays = Val(LV.ListItems(1).ListSubItems(4)) + Val(LV.ListItems(1).ListSubItems(6))
    Dim P As String
    P = "P" & vbNewLine & "P"
    For i = 1 To TotalDays
        con.Execute "UPDATE PrintAttReg SET d" & i & "='" & P & "'"
    Next
    
    
    'This Will Insert The Sunday Values
    DateArr = SundayDates(cmbMonth.ListIndex + 1, Val(cmbYear))
    For i = LBound(DateArr) + 1 To UBound(DateArr) - 1
            con.Execute "Update PrintAttReg Set d" & Day(DateArr(i)) & "='S" & vbNewLine & "S'"
    Next
    
    
    'This Will Insert The Holidays Values
    If rsEmp.State = 1 Then rsEmp.Close
    
    rsEmp.CursorLocation = adUseClient
    rsEmp.Open "select Day(DT) From Holidays Where Month(Dt)=" & cmbMonth.List(cmbMonth.ListIndex, 1) & " and  Year(dt)=" & cmbYear, con, adOpenKeyset, adLockReadOnly
    Dim Index As Integer
    
    If rsEmp.RecordCount > 0 Then
        ReDim Holidays(rsEmp.RecordCount - 1)
    
        For i = 0 To rsEmp.RecordCount - 1
            Holidays(i) = rsEmp.Fields(0)
            con.Execute "Update PrintAttReg Set d" & rsEmp.Fields(0) & "='H" & vbNewLine & "H'"
            rsEmp.MoveNext
        Next
    End If

'This Will Insert The Absents Values Randomly into The Table

    Dim AbsentArr As Collection

For k = 1 To LV.ListItems.Count

    Set AbsentArr = New Collection
    
    AbsentDays = Val(LV.ListItems(k).ListSubItems(4))
    SalDays = Val(LV.ListItems(k).ListSubItems(6))
   
    Dim ExitNow As Boolean, AbsentsInserted As Double, ADay As Integer
    
    P = "A" & vbNewLine & "A"
    
        Do Until AbsentDays <= 0
Repeat:
            Randomize
            
            'Int(Rnd * (iMax - (iMin - 1)) + (iMin))
            ADay = Int(Rnd * (TotalDays - (1 - 1)) + (1))
            
            For i = LBound(DateArr) + 1 To UBound(DateArr) - 1
                If ADay = Day(DateArr(i)) Then
                    GoTo Repeat
                End If
            Next
            On Error Resume Next
            For i = LBound(Holidays) To UBound(Holidays)
                If ADay = Holidays(i) Then
                    If err.Number = 0 Then GoTo Repeat
                End If
            Next
            
            AbsentArr.Add ADay, ADay & ""
            If err.Number = 457 Then GoTo Repeat
            
            con.Execute "UPDATE PrintAttReg SET d" & ADay & "='" & P _
                & "' WHERE EmpID='" & LV.ListItems(k) & "'"
...
'    Do Until .EOF
'        Con.Execute "Update PrintAttReg Set d" & ![AttDay] & "='" & ![FirstAtt] & vbNewLine & ![SecondAtt] & "' Where EmpID='" & ![EmpId] & "'"
'        .MoveNext
'    Loop
 

With CR2
   .ReportFileName = RptPRPath & "\AttRegister.rpt"
   .Formulas(0) = "ForMonth='" & cmbMonth & ", " & cmbYear & "'"
   .SelectionFormula = IIf(cmbDept.ID = "0", "", "{VPrintAttreg.deptID}='" & cmbDept.ID & "'")
