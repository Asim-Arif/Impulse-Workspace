...
Begin VB.Form PrintEmpJoinLeave 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Employees Joined/Discontinued"
   ClientHeight    =   2160
   ClientLeft      =   2760
   ClientTop       =   3750
   ClientWidth     =   4605
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   60
      Width           =   2805
      Begin MSForms.CheckBox chkTemp 
         Height          =   300
         Left            =   120
         TabIndex        =   8
         Top             =   1590
   ...
   Begin VB.Image Image2 
      Height          =   375
      Left            =   3300
      Picture         =   "PrintEmpJoinLeave.frx":0000
      Top             =   1200
      Width           =   465
   End
   ...
   End
End
Attribute VB_Name = "PrintEmpJoinLeave"
...
Private Sub cmdPrint_Click()
    ...
    If chkTemp.value = True Then
        Cond = IIf(cmbDept.ID = "0", "", "{VOutStanding.DeptID}='" & cmbDept.ID & "' AND ") & " ({Departments.TempDept}=True) and ((month({VEmp.DiscontinuedOn})=" & cmbMonth.List(cmbMonth.ListIndex, 1) & " and Year({VEmp.DiscontinuedOn})=" & cmbYear & ")" & _
         " OR (month({VEmp.JoinDate})=" & cmbMonth.List(cmbMonth.ListIndex, 1) & " and Year({VEmp.JoinDate})=" & cmbYear & "))"
    Else
        Cond = IIf(cmbDept.ID = "0", "", "{VOutStanding.DeptID}='" & cmbDept.ID & "' and ") & " ({Departments.TempDept}=False) AND ((month({VEmp.DiscontinuedOn})=" & cmbMonth.List(cmbMonth.ListIndex, 1) & " and Year({VEmp.DiscontinuedOn})=" & cmbYear & ")" & _
         " OR (month({VEmp.JoinDate})=" & cmbMonth.List(cmbMonth.ListIndex, 1) & " and Year({VEmp.JoinDate})=" & cmbYear & "))"
    End If
    Screen.MousePointer = vbHourglass

    Dim f As New frmPrevRpt, rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(rptPayrollPath & "\rptEmpJoinLeave.rpt")
