...
Begin VB.Form EmpPieceRateLedger 
   BorderStyle     =   0  'None
   ClientHeight    =   6900
   ClientLeft      =   0
   ClientTop       =   180
   ...
   Begin MSForms.Label Label2 
      ...
      Caption         =   " Year                Month                      Department                         Employee                               "
      Size            =   "12091;423"
      BorderColor     =   16512
      BorderStyle     =   1
      FontName        =   "Tahoma"
   ...
   Begin VB.Menu mnupop 
      Caption         =   "mnupop"
      Begin VB.Menu mnuDelWork 
         Caption         =   "Delete Work"
      End
   End
End
Attribute VB_Name = "EmpPieceRateLedger"
...
'        Next
        ...
        For i = 1 To LV.ListItems.Count
            ...
            With LV.ListItems(i)
                ...
                con.Execute "Insert into  PrintEmpLedger (DT,Status,InTime,OutTime,SecondInTime,SecondOutTime,Hrs,LateHours,OverTime,ShortLeave,EmpID,PayableHrs)" & _
                 "Values ('" & DT & "','" & .SubItems(2) & "','" & .SubItems(3) & "','" & .SubItems(4) & "','" & SecondIn & "','" & SecondOut & "','" _
                 & getHrsMin(dHrs, True) & "','" & .SubItems(6) & "','" & .SubItems(7) & "','" & .SubItems(8) & "','" & cmbEmp.ID & "','" & getHrsMin(dPHrs, True) & "')"
...
'''''''''''''''''''''''''' Print The Ledger For Production Employees '''''''''''''''''''''''''''''''''
    ...
    For Each FormulaField In FormulaFields
        ...
        ElseIf FormulaField.Name = "{@TotalPayableHrs}" Then
            FormulaField.Text = "'" & getHrsMin(GetTotalPHrs) & "'"
...
''''''''''''''''''''''''''''''''''''' To Get All The Time Intervals ''''''''''
                        ...
                        rsTime.Open "SELECT * FROM VEmpPerPieceWorkDone WHERE EmpID='" & cmbEmp.ID & "' AND DT='" & !DT & "'", con, adOpenForwardOnly, adLockReadOnly
                        If rsTime.RecordCount > 0 Then
                            ITM.ListSubItems.add(, , rsTime![ArticleNo] & "").ForeColor = TheColor
                            ITM.ListSubItems(3).Tag = Val(rsTime!EntryID & "")
                            ITM.ListSubItems.add(, , rsTime![CustItemName] & "").ForeColor = TheColor
                            ITM.ListSubItems.add(, , Val(rsTime!Qty & "")).ForeColor = TheColor
                            ITM.ListSubItems.add(, , Val(rsTime!Rate & "")).ForeColor = TheColor
                            ITM.ListSubItems.add(, , Val(rsTime!Rate & "") * Val(rsTime!Qty & "")).ForeColor = TheColor
                            rsTime.MoveNext
                        Else
                            ITM.ListSubItems.add , , ""
                            ITM.ListSubItems.add , , ""
...
'                            ITM.SubItems(5) = getHrsMin(Val(rsTime!Hrs & ""))
                            ITM.ListSubItems.add , , ""
                            ITM.ListSubItems.add , , ""
                            ITM.ListSubItems.add , , rsTime!ArticleNo & ""
                            ITM.ListSubItems(3).Tag = Val(rsTime!EntryID & "")
                            ITM.ListSubItems.add , , rsTime!CustItemName & ""
                            ITM.ListSubItems.add , , Val(rsTime!Qty & "")
                            ITM.ListSubItems.add , , Val(rsTime!Rate & "")
                            ITM.ListSubItems.add , , Val(rsTime!Rate & "") * Val(rsTime!Qty & "")
...
Private Sub LV_DblClick()
    ...
    If Val(LV.SelectedItem.ListSubItems(3).Tag) = 0 Then Exit Sub
    If Val(LV.SelectedItem.SubItems(5)) = 0 Then Exit Sub
    
    With txtQty
        .Move FLV.Left + LV.Left + LV.ColumnHeaders(6).Left, LV.Top + LV.SelectedItem.Top
...
Private Function GetTotalPHrs() As Double
    ...
    Else
        For i = 1 To LV2.ListItems.Count
            Total = Total + Val(LV2.ListItems(i).ListSubItems(3).Tag)
        Next
    End If
...
Private Sub mnuDelWork_Click()
    
    On Error GoTo err
    
    Dim DT As Date
    DT = DateSerial(cmbYear, cmbMonth.ListIndex + 2, 0)
    Dim rs As New ADODB.Recordset
    Dim bFinal As Boolean
    With rs
        .Open "Select Count(*) From TempMonthlySalaries Where DT='" & DT & "'", con, adOpenForwardOnly, adLockReadOnly
        If .Fields(0) > 0 Then
            bFinal = True
        Else
            bFinal = False
    ...
    If bFinal Then
        MsgBox "Payment Sheet of " & cmbYear & " - " & cmbMonth & " is Final. Can't Delete.", vbInformation
        Exit Sub
    End If
    
    If MsgBox("Are you sure to Delete this ?", vbQuestion + vbYesNo) = vbYes Then
        con.Execute "Delete From EmpPerPieceWorkDone Where EntryID=" & Val(LV.SelectedItem.ListSubItems(3).Tag)
...
Private Sub txtQty_KeyPress(KeyAscii As Integer)
    ...
    ElseIf KeyAscii = 13 Then
        If Val(txtQty) > 0 Then
            con.Execute "Update EmpPerPieceWorkDone Set Qty=" & Val(txtQty) & " Where EntryID=" & Val(LV.SelectedItem.ListSubItems(3).Tag)
