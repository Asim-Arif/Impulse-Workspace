...
Begin VB.Form frmEmployees_ST_Sheet 
   Caption         =   "Over Time"
   ClientHeight    =   9210
   ClientLeft      =   60
   ClientTop       =   450
   ...
   Begin VB.Frame FLV 
      ...
      Begin VB.Frame FraBank 
         ...
         Begin VB.CommandButton cmdBank 
            ...
            Height          =   315
            Left            =   3645
            Picture         =   "frmEmployees_ST_Sheet.frx":0000
            Style           =   1  'Graphical
            TabIndex        =   25
            Top             =   480
            Width           =   360
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmEmployees_ST_Sheet.frx":0372
            Key             =   "Present"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmEmployees_ST_Sheet.frx":07C4
            Key             =   "Absent"
         EndProperty
      EndProperty
   End
   ...
   End
End
Attribute VB_Name = "frmEmployees_ST_Sheet"
...
Private Sub cmdPost_Click(Index As Integer)
    ...
    If cmbDept.ID <> "0" Then
        MsgBox "Please Select All Departments to Post", vbInformation
        Exit Sub
    End If
    
    Dim lCount As Long
    lCount = GetSingleLongValue("COUNT(*)", "Employees_ST_Sheet_Posting", " WHERE DeptID='" & cmbDept.ID & "' AND DT='" & DTAttend.Tag & "' AND Posting_Type=" & Index)
...
'    lBankAmt = GetSingleLongValue("SUM(Approved)", "VEmployees_ST_Sheet", " WHERE DeptID='" & cmbDept.ID & "' AND DT='" & DTAttend.Tag & "' AND EmpID IN(SELECT EmpID FROM Employees WHERE BankPymt=1)")
    lCashAmt = GetSingleLongValue("SUM(Approved)", "VEmployees_ST_Sheet", " WHERE DT='" & DTAttend.Tag & "' AND EmpID IN(SELECT EmpID FROM Employees WHERE BankPymt=0)")
    lBankAmt = GetSingleLongValue("SUM(Approved)", "VEmployees_ST_Sheet", " WHERE DT='" & DTAttend.Tag & "' AND EmpID IN(SELECT EmpID FROM Employees WHERE BankPymt=1)")
'
    Call StartTrans(con)
...
'    strBankVchrNo = SaveTransForCheque(lBankAmt)
    ...
    con.Execute "INSERT INTO Employees_ST_Sheet_Posting(DT,CashVchrNo,BankVchrNo,UserName,MachineName,ChqNo,ChqDT,DeptID,Posting_Type) VALUES('" & DTAttend.Tag & _
     "','" & strCashVchrNo & "','" & strBankVchrNo & "','" & CurrentUserName & "','" & strComputerName & "','" & Tbox(2) & "','" & DTChq.Value & "','" & cmbDept.ID & "'," & Index & ")"
    
    con.CommitTrans
    
    MsgBox "Posted.", vbInformation
...
Private Function PostCashVoucher(lAmt As Long) As String
    ...
    objVchr.AddVoucherHead "15-010-03001", -Val(lAmt), strDescription
...
Private Sub PrintFilled(Optional bAllDepartments As Boolean = False)
    ...
    If bAllDepartments = False And cmbDept.ID <> "0" Then
        strSelection = strSelection & " AND {VEmp.DeptID}='" & cmbDept.ID & "'"
    End If
