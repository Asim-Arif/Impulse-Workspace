...
Begin VB.Form frmMiscGateOutList 
   ClientHeight    =   6795
   ClientLeft      =   1110
   ClientTop       =   345
   ClientWidth     =   10365
   ...
   Begin VB.ComboBox cmbRange 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "frmMiscGateOutList.frx":0000
      Left            =   45
      List            =   "frmMiscGateOutList.frx":0016
   ...
   Begin MSForms.CommandButton cmdClose 
      ...
      PicturePosition =   327683
      Size            =   "2752;688"
      Picture         =   "frmMiscGateOutList.frx":006D
      Accelerator     =   67
      FontName        =   "Tahoma"
      FontEffects     =   1073741825
      FontHeight      =   165
   ...
   End
End
Attribute VB_Name = "frmMiscGateOutList"
...
Public Sub mnuAdd_Click()
   ...
   Load frmMiscGateOutAE
   
   frmMiscGateOutAE.ShowMe
...
Private Sub mnuEdit_Click()
    ...
    Load frmMiscGateOutAE
    Call frmMiscGateOutAE.ShowMe(Val(LV.SelectedItem.Tag))
...
Private Sub mnuPrintDetail_Click()
    ...
    Set rpt = rptApp.OpenReport(RptPath & "\MiscGateOutList.rpt")
    
    rpt.FormulaFields.GetItemByName("DateRange").Text = "'" & Format(DtFrom.Value, "dd-MMM-yyyy") & " to " & Format(DtTo.Value, "dd-MMM-yyyy") & "'"
    
    Dim f As New frmPrevRpt
    f.ShowReport "{ItemIssuanceMisc.OnlyDT}=#" & DtFrom.Value & "# TO #" & DtTo.Value & "#", rpt
...
Private Sub mnuPrintEntry_Click()
    If LV.ListItems.count = 0 Then Exit Sub
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\MiscGateOut.rpt")
...
Private Sub RefreshList()
    ...
    With rs
        .Open "SELECT * FROM VMiscEntries_GateOut WHERE OnlyDT BETWEEN '" & DtFrom.Value & "' AND '" & DtTo.Value & "' ORDER BY DT", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        Do Until .EOF
