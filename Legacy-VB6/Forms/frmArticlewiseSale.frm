...
Begin VB.Form frmArticlewiseSale 
   ClientHeight    =   8415
   ClientLeft      =   120
   ClientTop       =   120
   ClientWidth     =   12660
   ...
   Begin VB.Frame FAll 
      ...
      Begin VB.Label Label1 
         ...
         BackStyle       =   0  'Transparent
         BorderStyle     =   1  'Fixed Single
         Caption         =   $"frmArticlewiseSale.frx":0000
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmArticlewiseSale"
...
Private Sub RefreshLV()
    ...
    If cmbArticle.ID <> "0" Then
        Cond = " WHERE ItemID='" & cmbArticle.ID & "'"
    End If
    ...
    LV.ListItems.Clear
    PrintSQL = "SELECT CustCode,Country,CustomInvoice,DT,Qty,ItemID,ItemName FROM VArticlewiseSales" & Cond & " ORDER BY DT"
    With rs
...
Private Sub cmdPrint_Click()
    ...
    strRet = ShowPopUpMenu(FAll.Left + cmdPrint.Left, cmdPrint.Top + FAll.Top, "Print &Sales Report", "Print Sales Report With &Detail")
    ...
    Select Case strRet
        ...
        Case "Print Sales Report With &Detail"
...
Private Sub PrintReport(Optional iReportType As Integer = 0)
    ...
    ElseIf iReportType = 1 Then
        Set rpt = rptApp.OpenReport(RptPath & "\rptArticlewiseSales_Value.rpt")
    End If
    ...
    strSelection = "1=1"
    If cmbArticle.ID <> "0" Then
        strSelection = strSelection & " AND {VArticlewiseSales.ItemID}='" & cmbArticle.ID & "'"
    End If
    
    If cmbCust.ID <> "0" Then
        strSelection = strSelection & " AND {VArticlewiseSales.CustCode}='" & cmbCust.ID & "'"
    End If
    
    If cmbCountry.ID <> "0" Then
        strSelection = strSelection & " AND {VArticlewiseSales.Country}='" & cmbCountry.ID & "'"
    End If
    
    If chkDT.Value = vbChecked Then
        strSelection = strSelection & " AND {VArticlewiseSales.DT}=#" & DTFrom & "# TO #" & DTTo & "#"
    End If
...
Private Sub cmdPrintAllArticles_Click()
    ...
    f.ShowReport "{VArticlewiseSales.DT}=#" & DTFrom.Value & "# TO #" & DTTo.Value & "#", rpt
...
Private Sub Form_Load()
    ...
    If strCompany = "IAA" Or strCompany = "Kami" Then
        cmdPrintAllArticles.Visible = False
    End If
