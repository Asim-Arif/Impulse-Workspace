...
Begin VB.Form frmReWorkIssuance 
   ClientHeight    =   9390
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11970
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.ComboBox cmbRange 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmReWorkIssuance.frx":0000
         Left            =   135
         List            =   "frmReWorkIssuance.frx":0013
   ...
   End
End
Attribute VB_Name = "frmReWorkIssuance"
...
Private Sub cmbArticle_matched()
    ...
    strCondition = "WHERE OnlyDT BETWEEN '" & DtFrom & "' AND '" & DtTo & "' AND ItemCode='" & cmbArticle.ID & "' AND Qty>IssQty "
    If Trim(txtLotNo) <> "" Then
        strCondition = strCondition & " AND LotNo='" & Trim(txtLotNo) & "'"
    End If
    ...
    With rs
        .Open "SELECT * FROM VVendRcvdDetailReWorkDetail " & strCondition & " ORDER BY EntryID", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        'LV1.ListItems.Clear
        Do Until .EOF
            ...
            ITM.ListSubItems.add(, , !VenderName & "(" & !VendID1 & ")").Tag = Val(!VendID & "")
            ITM.ListSubItems.add(, , !RepairType).Tag = Val(!Repair_RefID & "")
            ITM.ListSubItems.add , , Val(!Qty & "") - Val(!IssQty & "")
...
Private Sub GetLV1Total()
    ...
    For i = 1 To LV1.ListItems.count
        lTotal = lTotal + Val(LV1.ListItems(i).SubItems(5))
    Next
...
Private Sub cmdSave_Click()
    ...
    dRate = Val(LV1.ListItems(1).SubItems(6))
    strOrderNo = LV1.ListItems(1).SubItems(2)
    iRepairType = (LV1.ListItems(1).ListSubItems(4).Tag)
    
    Dim i As Integer
    For i = 1 To LV1.ListItems.count
        lQty = lQty + Val(LV1.ListItems(i).SubItems(5))
    Next
    lProcessID = GetSingleLongValue("TOP 1 ProcessID", "RepairTypeProcesses", " WHERE Repair_RefID=" & Val(LV1.ListItems(1).ListSubItems(4).Tag) & " ORDER BY SeqNo")
    ...
    'IssID = "ISU-" & Format(Date, "ddMMyy") & lMakerIssNo
    ...
    strIssIDDetail = "ISU-" & _
     Format(Date, "ddMMyy") & (Val(strIssIDDetail) + 1)
         
    Dim lLotNo As String, strRefLotNo As String
    lLotNo = getNextLotNo
    Dim strBatchNo As String, strMilNo As String
    For i = 1 To LV1.ListItems.count
        With LV1.ListItems(i)
            strRefLotNo = .SubItems(1)
        End With
    Next
    strBatchNo = GetSingleStringValue("Batch_No", "Lots_List", " WHERE LotNo='" & strRefLotNo & "'")
    strMilNo = GetSingleStringValue("Mill_Certificate_No", "Lots_List", " WHERE LotNo='" & strRefLotNo & "'")
    ...
    con.Execute "INSERT INTO Lots_List(LotNo,ItemID,Lot_Type,Reference_LotNo,Batch_No,Mill_Certificate_No) VALUES('" & lLotNo & "','" & strItemID & "',5,'" & strRefLotNo & "','" & strBatchNo & "','" & strMilNo & "')"
...
'    End If
    ...
    con.Execute "INSERT INTO VendIssdDetail(RefID,RecieptID,ItemCode" & _
     ...
     strIssIDDetail & "','" & strItemID & "'," & dRate & "," & lQty & ",0,'" & strOrderNo & "'," & lProcessID & ",'" & _
     Date & "',1,0,'" & lLotNo & "',1," & iRepairType & ")"
...
Private Sub LV_DblClick()
    ...
    ITM.ListSubItems.add , , Val(LV.SelectedItem.SubItems(5))
    ITM.ListSubItems.add , , Val(LV.SelectedItem.SubItems(6))
    
    LV.ListItems.Remove LV.SelectedItem.Index
    
    If LV1.ListItems.count = 1 Then
        Dim lProcessID As Long, strItemID As String, lRepairType As Long
        lRepairType = Val(LV1.ListItems(1).ListSubItems(4).Tag)
        strItemID = cmbArticle.ID
...
Private Sub LV1_DblClick()
    ...
    With txtRate
        .Move LV1.Left + LV1.ColumnHeaders(7).Left, LV1.Top + LV1.SelectedItem.Top, LV1.ColumnHeaders(7).Width
        .Text = Val(LV1.SelectedItem.SubItems(6))
        .Visible = True
        .SelStart = 0
        .SelLength = Len(.Text)
        .SetFocus
...
Private Sub txtRate_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If Val(txtRate) < 0 Then
            ...
            Exit Sub
        End If
        LV1.SelectedItem.SubItems(6) = Val(txtRate)
