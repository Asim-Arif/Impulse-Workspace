...
Private Sub cmdAdd_Click()
    ...
    If GotRcvID = 0 Then
        If cmbProcess.ID >= 7 Then 'Lot Process
            GotRcvID = Val(con.Execute("Select Max(Cast(right(RecieptID,Len(RecieptID)-10) As Int)) From VVendRcvdItems Where Convert(varchar,DT,6)='" & Format(DTPicker2, "dd MMM yy") & "' and UPPER(Left(RecieptID,2))='HS'").Fields(0).value & "") + 1
        Else
...
Private Function Saved() As Boolean
    ...
    con.Execute "Insert Into VendReceived(VendID,DT,RecieptID," & _
     "ProcessID,UserID) Values(0,'" & ServerDate & " " & ServerTime & "','" & txtRecNo & "'," & _
    ...
    For i = 1 To LV.ListItems.Count
            ...
            If Val(LV.ListItems(i).SubItems(3)) > 0 Then
                ...
                With cmd
                    .ActiveConnection = con
                    .CommandType = adCmdStoredProc
                    .CommandText = "SP_AddStock"
                    .Parameters("@ItemCode").value = LV.ListItems(i).SubItems(1)
                    .Parameters("@ProcessID").value = cmbProcess.ID
                    .Parameters("@Qty").value = Val(LV.ListItems(i).SubItems(3) & "")
