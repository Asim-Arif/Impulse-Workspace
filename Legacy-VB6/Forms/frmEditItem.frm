...
Begin VB.Form frmEditItem 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmEditItem.frx":0000
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "Frame1"
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "Other Info."
      TabPicture(1)   =   "frmEditItem.frx":001C
      Tab(1).ControlEnabled=   -1  'True
      Tab(1).Control(0)=   "Frame2"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).ControlCount=   1
      TabCaption(2)   =   "Barcode"
      TabPicture(2)   =   "frmEditItem.frx":0038
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "Label2(1)"
      Tab(2).Control(1)=   "Label2(0)"
      Tab(2).Control(2)=   "Label2(2)"
      Tab(2).Control(3)=   "CD2"
      Tab(2).Control(4)=   "CDBarcode"
      Tab(2).Control(5)=   "cmdOpenBarcode"
      Tab(2).Control(6)=   "cmdBrowseBarcode(1)"
      Tab(2).Control(7)=   "txtFilePath"
      Tab(2).Control(8)=   "cmdBrowseBarcode(0)"
      Tab(2).Control(9)=   "txtProgramPath"
      Tab(2).Control(10)=   "txtPDFPath"
      Tab(2).Control(11)=   "cmdBrowsePDF"
      ...
      Begin VB.Frame Frame2 
         ...
         Top             =   525
         Width           =   6315
         Begin MSForms.ComboBox cmbMD_Group 
            Height          =   315
            Left            =   135
            TabIndex        =   107
            Tag             =   "MD_Group"
         ...
         Begin VB.Label label1 
            AutoSize        =   -1  'True
            Caption         =   "Medical Devices Marketing Authorization"
            ForeColor       =   &H00400000&
            Height          =   195
            Index           =   38
            Left            =   165
...
Public Sub EditItem(TempstrEdit As String)
    ...
    Dim c As Control
    With rs
        .Open "SELECT * FROM FCustomerCatalog WHERE CustCode+Country+CompItemID='" & strEdit & "'", con, adOpenForwardOnly, adLockReadOnly
    ...
    bAllowSave = GetSingleBooleanValue("EditCustCatalog", "Users", " WHERE UserName='" & CurrentUserName & "'")
...
Private Sub FillCmbs()
    ...
    Call AddToCombo(cmbMD_Group, "DISTINCT MD_Group", "FCustomerCatalog", " WHERE ISNULL(MD_Group,'')<>''")
    '----------------------------------------------------------------
End Sub
...
Private Function SaveItem() As Boolean
    ...
    Dim lCount As Long
    lCount = GetSingleLongValue("COUNT(ItemID)", "FCustomerCatalog", " WHERE ItemID='" & Tbox(1) & "' AND CustCode='" & Tbox(2) & "' AND Country='" & Tbox(5) & "' AND CustCode+Country+CompItemID<>'" & strEdit & "'")
    ...
    SQL = CreateUpdateSQL("FCustomerCatalog")
    SQL = SQL & " WHERE CustCode+Country+CompItemID='" & strEdit & "'"
    
    con.Execute SQL
    con.Execute "UPDATE FCustomerCatalog SET BarcodeFile='" & Replace(txtFilePath, "'", "''") & _
     "' WHERE CustCode+Country+CompItemID='" & strEdit & "'"
...
Private Sub AttachPDF(p_strEdit As String)
    ...
    If strPDFPath <> "" Then
        ...
        Rec.Open "SELECT EntryID,ArtworkPDF FROM FCustomerCatalog WHERE CustCode+Country+CompItemID='" & p_strEdit & "'", con, adOpenStatic, adLockOptimistic
