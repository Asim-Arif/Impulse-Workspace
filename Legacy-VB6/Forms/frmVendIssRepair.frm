...
Begin VB.Form frmVendIssRepair 
   ClientHeight    =   9810
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   11805
   ...
   Begin VB.Frame FALL 
      ...
      Begin MSForms.Label Label2 
         ...
         ForeColor       =   4194304
         BackColor       =   12504280
         Caption         =   $"frmVendIssRepair.frx":0000
         Size            =   "18177;397"
   ...
   End
End
Attribute VB_Name = "frmVendIssRepair"
...
Private Sub cmbProcess_matched()
    ...
    Call cmbVend.AddVals(con, "VenderName", "VVenders", "VendID", " Where (PhaseID=" & cmbProcess.ID & " And VendID In(Select VendID From VVendIssRepair1)) Or (RepairVend=1 And (Select Top 1 IsNull(VendID,0) From VVendIssRepair2 Where VendID Is Null)=0)")
...
Private Sub cmbVend_matched()
    ...
    Dim Sql As String
    'First Check If Vender Is a Repair Vender Or Not
    With rs
        .Open "Select RepairVend From Venders Where VendID=" & cmbVend.ID, con, adOpenForwardOnly, adLockReadOnly
        If ![RepairVend] = 0 Then
            Sql = "Select ItemCode,ItemName,RepairQty,ItemRate From VVendIssRepair1 Where VendID=" & cmbVend.ID
        Else
            Sql = "Select ItemCode,ItemName,RepairQty,ItemRate From VVendIssRepair1 Where VendID Is Null AND ItemCode In(Select ItemID From VendAssItems Where VendID=" & cmbVend.ID & ")"
        End If
        .Close
    End With
    ...
    With rs
        ...
        Do Until .EOF
            ...
            ITM.ListSubItems.Add(, , ![RepairQty] & "").Tag = ![ItemRate] & ""
    ...
    If KeyAscii = 13 Then
        ...
        If Val(txtQty) = 0 Then
            ...
            If LV.ListItems.Count > 0 Then
                For i = LV.SelectedItem.Index To LV.ListItems.Count
                    TempID = Val(Right(LV.ListItems(i).SubItems(2), 3)) - 1
                    If TempID > 1 Then
                        LV.ListItems(i).SubItems(2) = "ISR-" & _
        ...
        If GotIssID = 0 Then
            GotIssID = Val(con.Execute("Select Max(right(RecieptID,Len(RecieptID)-10)) From VVendIssItems Where Convert(varchar,DT,6)='" & Format(ServerDate, "dd MMM yy") & "'").Fields(0).value & "")
            GotIssID = GotIssID + 1
        Else
            GotIssID = GotIssID + 1
        End If
        'IssID = "ISS-" & Val(LVMaker.SelectedItem.ListSubItems(1).Key) & "-" & _
         ...
         IssID = "ISR-" & _
         Format(ServerDate, "ddMMyy") & GotIssID
         
        LV.SelectedItem.SubItems(LV.ColumnHeaders.Count - 1) = IssID
        'Call AdjustQty
        Pic.Visible = False
...
Private Function Saved() As Boolean
    ...
    'con.Execute "Insert Into VendIssued(VendID,DT) Values(" & _
     cmbVend.ID & ",'" & ServerDate & " " & ServerTime & "')"
    
    
     con.Execute "Insert Into VendIssued(VendID,DT,RecieptID,UserID,ProcessID) Values(" & _
        cmbVend.ID & ",'" & DTPicker1 & " " & ServerTime & "',''," & _
    ...
    For i = 1 To LV.ListItems.Count
        If Val(LV.ListItems(i).SubItems(4)) > 0 Then
            ...
            '    con.Execute "Insert Into VendRepairDetail(VendID,DT,ItemCode,ProcessID,IssQty) Values(" & _
            '     cmbVend.ID & ",'" & DTPicker1 & "','" & .SubItems(1) & "'," & cmbProcess.ID & _
            '     "," & Val(.SubItems(4)) & ")"
            'End With
            With cmd
                ...
                .Execute
            End With
            OrderNo = "Repair"  'Temporary...
            
            With LV.ListItems(i)
                con.Execute "Insert Into VendIssdDetail(RefID,RecieptID,ItemCode" & _
                 ",Rate,IssQty,ReqAuth) Values('" & iRefID & "','" & .SubItems(5) & _
                 "','" & .SubItems(1) & "'," & _
                 Val(.ListSubItems(3).Tag) & "," & _
                 Val(.SubItems(4)) & "," & UserReqAuth & ")"
            End With
