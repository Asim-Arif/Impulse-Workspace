...
Begin VB.Form frmMakerBillingList 
   ClientHeight    =   8385
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11970
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.ComboBox cmbRange 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmMakerBillingList.frx":0000
         Left            =   5895
         List            =   "frmMakerBillingList.frx":0016
      ...
      Begin VB.Label Label1 
         ...
         Caption         =   $"frmMakerBillingList.frx":0066
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin MSComctlLib.ImageList ImageList2 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmMakerBillingList.frx":0120
            Key             =   "Down"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmMakerBillingList.frx":02DC
            Key             =   "Up"
         EndProperty
      EndProperty
   End
   Begin VB.Menu mnuPop 
      ...
      Begin VB.Menu mnuPrintSummary 
         ...
         Visible         =   0   'False
      End
      Begin VB.Menu mnuPrintMakerActivitySummary 
         Caption         =   "Maker Activity Summary"
         Visible         =   0   'False
      End
   ...
   End
End
Attribute VB_Name = "frmMakerBillingList"
...
Private Sub cmdPrint_Click()
    ...
    strSelection = "{VMakerPostedBillsList.DT}=#" & DtFrom.Value & "# TO #" & DtTo.Value & "#"
    'strSelection = strSelection & " AND {VMakerPostedBillsList.Posted}=TRUE"
    If cmbCust.ID <> "0" Then
        strSelection = strSelection & " AND {VMakerPostedBillsList.VendID}=" & cmbCust.ID
    End If
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\MakerBillsList.rpt")
    Dim f As New frmPrevRpt
...
Private Sub RefreshLV()
    ...
    'strCondition = strCondition & " AND Posted=1"
    If cmbCust.ID <> "0" Then
    ...
    With rs
        ...
        Do Until .EOF
            ...
            ITM.ListSubItems.add , , Round(Val(!NetPayable & ""), 2)
            ITM.ListSubItems.add , , Round(Val(!TotalPaidAmount & ""), 2)
            ITM.ListSubItems.add , , Round(Val(!NetPayable & ""), 2) - Round(Val(!TotalPaidAmount & ""), 2)
            'ITM.ListSubItems.add , , !Payment_VchrNo & ""
            .MoveNEXT
        Loop
        .Close
    End With
...
Private Sub mnuPrintSummary_Click()
    ...
    Dim strCondition As String
    strCondition = "{MakerPostedBills.DT}=#" & DtFrom.Value & "# TO #" & DtTo.Value & "#"
    strCondition = strCondition & " AND {MakerPostedBills.Posted}=TRUE"
    If cmbCust.ID <> "0" Then
        strCondition = strCondition & " AND {MakerPostedBills.VendID}=" & cmbCust.ID
    End If
    
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\MakerPostedBills_Summary.rpt")
...
Private Sub mnuPaymentVoucher_Click()
    ...
    If LV.ListItems.count = 0 Then Exit Sub
    If Val(LV.SelectedItem.SubItems(7)) <= 0 Then
        MsgBox "Already Paid.", vbInformation
        Exit Sub
    End If
    
'    If LV.SelectedItem.SubItems(6) <> "" Then
'        MsgBox "Already Paid.", vbInformation
'        Exit Sub
'    End If
    Dim f As New frmMakerPV_Adv
    Load f
    Call f.ShowMe(Val(LV.SelectedItem.key))
...
Private Sub mnuPrintBill_Click()
    Call PrintMakerPostedBill(LV.SelectedItem.SubItems(2))
End Sub

Private Sub mnuPrintMakerActivitySummary_Click()
    If LV.ListItems.count = 0 Then Exit Sub
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\MakerActivitySummary.rpt")
    rpt.ParameterFields.GetItemByName("@EntryID").SetCurrentValue Val(LV.SelectedItem.key)
    rpt.FormulaFields.GetItemByName("Posted").Text = True
    Dim f As New frmPrevRpt
