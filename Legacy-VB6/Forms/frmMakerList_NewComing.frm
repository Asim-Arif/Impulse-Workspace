...
Begin VB.Form frmMakerList_NewComing 
   AutoRedraw      =   -1  'True
   ClientHeight    =   6795
   ClientLeft      =   1110
   ClientTop       =   345
   ...
   Begin VB.Menu mnupop 
      ...
      End
      Begin VB.Menu mnuTranserToRegularMakerList 
         Caption         =   "Transfer to Regular Maker List"
      End
   ...
   End
End
Attribute VB_Name = "frmMakerList_NewComing"
...
'    '  mnuDelete.Enabled = Not OutOfFinancialYear
    ...
    cmbVendName.AddVals con, "'{' + ISNULL(MakerID1,'') + '} ' + MakerName", "VMakers_NewComing", "MakerID", " "
...
Public Sub mnuAdd_Click()
    ...
    Dim f As New frmNewMaker_NewComing
    With f
        .add = True
        .Show 1
    End With
...
Private Function GetVendBalance() As Double
    ...
    With cmd
        ...
        .Parameters("@MakerID").Value = Val(LV.SelectedItem.key)
    ...
    With rsLedger
        ...
        If .EOF Then
            GetVendBalance = 0
        Else
            Call Computed(![MakerID], IsComputed, STLoan, LongLoan, DT, IsPayed, IsTransfered, ServerDate, VendWorkDone)
...
Private Sub Computed(MakerID As Long, ByRef IsComputed As Boolean, ByRef STLoan As Double, ByRef LongLoan As Double, ByRef DT As String, ByRef Payed As Boolean, ByRef Transfered As Boolean, ToDate As Date, ByRef VendWorkDone As Double)
    ...
    With cmd
        ...
        .Parameters("@MakerID").Value = MakerID
...
Private Sub mnuedit_Click()
    ...
    Load frmNewMaker_NewComing
    Call frmNewMaker_NewComing.EditVender(Val(LV.SelectedItem.key))
...
Private Sub mnuPrintMakerList_Click()
    ...
    Set rpt = rptApp.OpenReport(RptPath & "\MakerList_NewComing.rpt")
    Dim f As New frmPrevRpt
    f.ShowReport "", rpt
...
Private Sub RefreshList()
    ...
    With rs
        ...
        ActiveCondition = " Where MakerID<>-1 "
        ActiveCondition = IIf(mnuShowAll.Checked, "", ActiveCondition & " AND IsNull(Active,1)=1 ")
        .CursorLocation = adUseClient
        .Open "SELECT MakerID,MakerID1,MakerName,VendType,Phone1,Phone2,Fax1,Active,AccNo,MakerType FROM VMakers_NewComing " & ActiveCondition & " Order By MakerName", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        ...
        Do Until .EOF
            ...
            TempID = ![MakerID]
            Set ITM = LV.ListItems.add(, TempID & "'", ![MakerID1] & "")
            'Set ITM = LV.ListItems.Add(, ![MakerID] & .AbsolutePosition & "'", ![AccNo], , TheTag)
            ...
            ITM.ListSubItems.add(, , ![MakerName] & "").ToolTipText = ![MakerName] & ""
...
Private Sub mnuTranserToRegularMakerList_Click()
    If LV.ListItems.count = 0 Then Exit Sub
    
    Dim strMakerID As String, strAccNo As String, lNM_VendID As Long, strAccTitle As String
    lNM_VendID = Val(LV.SelectedItem.key)
    strAccTitle = LV.SelectedItem.SubItems(3)
    strAccNo = con.Execute("SELECT dbo.GetNextAccno('" & strAccTitle & "','24','24-004-004',0)").Fields(0).Value & ""
        
    strMakerID = GetSingleLongValue("MAX(CAST(RIGHT(VendID1,4) AS INT))", "Makers", "WHERE ISNUMERIC(RIGHT(VendID1,4))=1") + 1
    strMakerID = Format(strMakerID, "0000")
    strMakerID = "CONT-" & strMakerID
        
    Call StartTrans(con)
    
    con.Execute "INSERT INTO Accounts(AccNo,AccTitle,SubAccOf,Type,OpenBal,opendate,Balance,UserName,ComputerName) VALUES('" & strAccNo & "','" & strAccTitle & _
     "','24-004-004','24',0,'" & Date & "',0,'" & CurrentUserName & "','" & strComputerName & "')"
    
    
    con.Execute "INSERT INTO dbo.Makers(VendID1,VenderName,Phone1,Phone2,Fax1,Address,ContactPerson,CPhone,CEmail,Mobile,NICNo,NTNNo,VendPic,VendThumb,RefBy,RefByFName,RefByPhone1," & _
     "RefByPhone2,RefByAddress,TimeLimit,MaxLimit,AccNo,VendType,PhaseID,SubVendType,Planter,Snaffer,Stamper,Experience,OtherCompany1,OtherCompany2,OpenBal,Active,OwnRepair,RepairVend," & _
     "RepairDedRate,MakerNameUrdu,BankAccNo,MakerType,AuthRequired,ExcessQtyPercentage,MaximumRcvingsAgainstPO,ShowRateOnPO,PaymentTerms) SELECT '" & strMakerID & "',MakerName,Phone1," & _
     "Phone2,Fax1,Address,ContactPerson,CPhone,CEmail,Mobile,NICNo,NTNNo,VendPic,VendThumb,RefBy,RefByFName,RefByPhone1,RefByPhone2,RefByAddress,TimeLimit,MaxLimit,'" & strAccNo & "'," & _
     "VendType,PhaseID,SubVendType,Planter,Snaffer,Stamper,Experience,OtherCompany1,OtherCompany2,OpenBal,Active,OwnRepair,RepairVend,RepairDedRate,MakerNameUrdu,BankAccNo,MakerType," & _
     "AuthRequired,ExcessQtyPercentage,MaximumRcvingsAgainstPO,ShowRateOnPO,PaymentTerms FROM dbo.Makers_NewComing WHERE MakerID=" & lNM_VendID
