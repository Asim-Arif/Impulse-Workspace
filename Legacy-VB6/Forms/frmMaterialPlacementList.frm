...
Begin VB.Form frmMaterialPlacementList 
   ClientHeight    =   8385
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11970
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.Frame fTitle 
         ...
         Begin VB.Label Label1 
            ...
            Caption         =   "Material Placement List"
            BeginProperty Font 
      ...
      Begin VB.ComboBox cmbRange 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmMaterialPlacementList.frx":0000
         Left            =   5895
         List            =   "frmMaterialPlacementList.frx":0016
      ...
      Begin VB.Label Label1 
         ...
         BackColor       =   &H00AFBDCB&
         BackStyle       =   0  'Transparent
         Caption         =   $"frmMaterialPlacementList.frx":0066
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin MSComctlLib.ImageList ImageList2 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmMaterialPlacementList.frx":0120
            Key             =   "Down"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmMaterialPlacementList.frx":02DC
            Key             =   "Up"
         EndProperty
      EndProperty
   End
   ...
   End
End
Attribute VB_Name = "frmMaterialPlacementList"
...
Private Sub cmdPrint_Click()
    ...
    Set rpt = rptApp.OpenReport(RptPath & "\VendorPostedBillsList.rpt")
    
    Dim strSelection As String
    
    Dim strCondition As String
    strSelection = " {VVenderPostedBillsList.DT}=#" & DtFrom.Value & "# TO #" & DtTo.Value & "#"
    If cmbCust.ID <> "0" Then
        strSelection = strSelection & " AND {VVenderPostedBillsList.AccNo}='" & cmbCust.ID & "'"
    End If
    strSelection = strSelection & " AND {VVenderPostedBillsList.SampleBilling}=" & bSample
...
Private Sub Form_Load()
    ...
    'Call cmbCust.AddVals(con, "AccTitle", "VVendersANDMakers", "AccNo")
    Call cmbCust.AddVals(con, "AccTitle", "VVenders", "AccNo")
    cmbCust.ID = "0"
    
    cmbRM.AddItem "<All>", "0"
    cmbRM.AddVals con, "RMID1 + ' {'+RMName+'}'", "VRM", "RMID1", " ORDER BY RMName"
    cmbRM.ID = "0"
...
Private Sub RefreshLV()
    ...
    If cmbCust.MatchFound = False Then
        MsgBox "Please Select Vender.", vbInformation
        Exit Sub
    End If
    Dim rs As New ADODB.Recordset, ITM As ListItem
    Dim strCondition As String
    strCondition = " WHERE RcvDate BETWEEN '" & DtFrom.Value & "' AND '" & DtTo.Value & "'"
    If cmbCust.ID <> "0" Then
        strCondition = strCondition & " AND AccNo='" & cmbCust.ID & "'"
    End If
    If cmbRM.ID <> "0" Then
        strCondition = strCondition & " AND RMID1='" & cmbRM.ID & "'"
    End If
    
    With rs
        .Open "SELECT * FROM VMaterialLocationWiseStatus " & strCondition & " ORDER BY EntryID", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        Do Until .EOF
...
Private Sub mnuDeleteBill_Click()
    If LV.ListItems.count = 0 Then Exit Sub
    If getDBPassword("Delete_Vendor_Bill") = False Then Exit Sub
    con.Execute "DELETE FROM VenderPostedBills WHERE EntryID=" & Val(LV.SelectedItem.key)
    Call RefreshLV
End Sub
...
Private Sub mnuPrintBill_Click()
    ...
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\MaterialPlacement_Slip.rpt")
    Dim f As New frmPrevRpt
    f.ShowReport "{VMaterialLocationWiseStatus.EntryID}=" & Val(LV.SelectedItem.Tag), rpt
