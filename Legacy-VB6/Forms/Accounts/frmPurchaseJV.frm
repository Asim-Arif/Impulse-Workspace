...
Begin VB.Form frmPurchaseJV 
   ...
   Begin VB.Frame FALL 
      ...
      Begin VB.PictureBox Picture1 
         ...
         Begin VB.Frame Frame6 
            ...
            Begin MSForms.CommandButton cmdCancel 
               ...
               Size            =   "2937;635"
               Picture         =   "frmPurchaseJV.frx":0000
               Accelerator     =   67
               FontName        =   "Tahoma"
            ...
            Begin MSForms.CommandButton cmdSClear 
               ...
               PicturePosition =   327683
               Size            =   "2937;635"
               Picture         =   "frmPurchaseJV.frx":0112
               Accelerator     =   78
               FontName        =   "Tahoma"
               FontEffects     =   1073741825
               FontHeight      =   165
            ...
            Begin MSForms.CommandButton cmdSClose 
               ...
               PicturePosition =   327683
               Size            =   "2937;635"
               Picture         =   "frmPurchaseJV.frx":0464
               Accelerator     =   83
               FontName        =   "Tahoma"
               FontEffects     =   1073741825
               FontHeight      =   165
         ...
         Begin VB.CommandButton cmdSave 
            ...
            Height          =   555
            Left            =   9690
            Picture         =   "frmPurchaseJV.frx":05C2
            Style           =   1  'Graphical
            TabIndex        =   8
         ...
         Begin VB.CommandButton cmdDelete 
            ...
            Height          =   285
            Left            =   9690
            Picture         =   "frmPurchaseJV.frx":0AF4
            Style           =   1  'Graphical
            TabIndex        =   10
         ...
         Begin VB.Label Label1 
            ...
            BackColor       =   &H00C5D1DC&
            BorderStyle     =   1  'Fixed Single
            Caption         =   $"frmPurchaseJV.frx":1026
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   8.25
               Charset         =   0
      ...
      Begin VB.Label Label1 
         ...
         BackColor       =   &H00C5D1DC&
         BorderStyle     =   1  'Fixed Single
         Caption         =   " Vender Acc. #                Vender Acc. Title                                                        Description"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmPurchaseJV"
...
If DontDropDown Then Exit Sub
    If cmbVendAccNo.MatchFound Then Call cmbVendAccNo_Click Else If cmbVendAccNo <> "" Then cmbVendAccNo.DropDown
End Sub
...
End Sub
Private Sub cmbVendAccTitle_DropButtonClick()
   List2.Visible = False
End Sub
...
Private Sub cmbVendAccTitle_KeyUp(KeyCode As MSForms.ReturnInteger, Shift As Integer)
 If DontShowList Then DontShowList = False Else ShowList List2, cmbVendAccTitle, cmbVendAccTitle
End Sub
...
rsList.Open "Select Sno,TotalAmount,AmountLeft  from VAccRec where (AccNo='" & cmbVendAccNo & "') and (VDate<='" & DTPicker1 & "') and ((AmountLeft>0) or (AmountLeft is null))", con, adOpenForwardOnly, adLockReadOnly
rsList.Sort = "Sno"
...
For i = 1 To LVPymt.ListItems.Count
    ...
    LVPymt.ListItems(i).SubItems(1) = IIf(SaleAmt > Val(LVPymt.ListItems(i)), Val(LVPymt.ListItems(i)), SaleAmt)
...
If lblBalance.Visible Then
    lblBalance.Caption = Format(getBalance(cmbVendAccNo, DTPicker1), "#,##0.00 DR;#,##0.00 CR;0.00 ")
    lblBalance.ToolTipText = "Balance of '" & cmbVendAccTitle & "' As Of " & Format(DTPicker1.Value, "dd/MM/yyyy")
End If
...
Private Sub cmdSClear_Click()
    ...
    If SaveTrans Then
        ...
        If MsgBox("Do You Want To Enter The Payment ?", vbQuestion + vbYesNo) = vbYes Then
            With frmCashPV
...
PermissionDenied = Not UserHasAccess("PurchaseVoucher")
...
Call FindInCombo(CmbAccTitle, "Purchase of Hardware")
...
Private Sub FillAccCmbs()
    ...
    With AccRec
        ...
        .Open "Select AccNo,AccTitle from Accounts as Acc1 Where (AccNo Like '41-001%')  AND (Parent=0) AND Active=1", con, adOpenForwardOnly, adLockReadOnly
    ...
    Call AddToCombo(cmbVendAccNo, "AccNo", "VVenders", " Where Active=1 Order By Accno")
    Call AddToCombo(cmbVendAccTitle, "AccTitle", "VVenders", " Where Active=1 Order By Accno")
...
ElseIf Not cmbVendAccNo.MatchFound Then
    MsgBox "Vender Account Is Not Selected Or Invalid.", vbInformation
    Exit Function
End If
...
'''''''''''''' Start Adding Voucher ''''''''''''
    ...
    con.Execute "insert into Vouchers Values(" & INextSno & ",'" & _
        Format(DTPicker1.Value, "dd-MMM-yyyy") & "','" & Label1(4) & "','" & cmbVendAccNo & _
        "','" & Tbox(0) & "',0," & Val(lblTotal) & "," & Bal & ",'',0)"
    
    
          
   Call adjustLedger(cmbVendAccNo, DTPicker1.Value, INextSno, -Val(lblTotal))
    ...
    For i = 1 To LVPymt.ListItems.Count
        If LVPymt.ListItems(i).Checked Then con.Execute "Insert into VoucherDetail Values (" & SnoToUse & "," & Val(LVPymt.ListItems(i).key) & "," & Val(LVPymt.ListItems(i).SubItems(1)) & " )"
    Next
...
'''''''''''''' Start Adding Voucher ''''''''''''
    ...
    Bal = getFakeBalance(cmbVendAccNo, DTPicker1.Value)
    Bal = Bal - Val(lblTotal)
    ...
    con.Execute "insert into FakeVouchers Values(" & INextSno & ",'" & _
        Format(DTPicker1.Value, "dd-MMM-yyyy") & "','" & VNo & "','" & cmbVendAccNo & _
        "','" & Tbox(0) & "',0," & Val(lblTotal) & "," & Bal & ",'',0)"
    
    
          
   Call adjustFakeLedger(cmbVendAccNo, DTPicker1.Value, INextSno, -Val(lblTotal))
