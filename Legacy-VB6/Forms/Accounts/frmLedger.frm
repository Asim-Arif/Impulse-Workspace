...
Begin VB.Form frmLedger 
   ...
   Begin VB.Frame FraFind 
      ...
      Begin VB.ComboBox cmbfield 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmLedger.frx":02A4
         Left            =   60
         List            =   "frmLedger.frx":02BA
      ...
      Begin MSComctlLib.ImageList ImageList1 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmLedger.frx":02F2
               Key             =   "PND"
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmLedger.frx":074A
               Key             =   "CAN"
            EndProperty
            BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmLedger.frx":0B9C
               Key             =   "CLR"
            EndProperty
            BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmLedger.frx":0FF0
               Key             =   "DUE"
            EndProperty
            BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmLedger.frx":1448
               Key             =   "BNC"
            EndProperty
         EndProperty
      End
...
'    End With
    ...
    Set rpt = rptApp.OpenReport(RptPath & "\Ledger.rpt")
    rpt.FormulaFields.GetItemByName("Company").Text = "'" & strCompany & "'"
    rpt.FormulaFields.GetItemByName("FromTo").Text = "'From " & Format(DtFrom.Tag, "dd-MMM-yyyy") & " To " & Format(DtTo.Tag, "dd-MMM-yyyy") & "'"
    rpt.FormulaFields.GetItemByName("SubReport").Text = LVCheque.Visible
    rpt.FormulaFields.GetItemByName("Address").Text = "'" & COMPANYADDRESS & "'"
    
    rpt.FormulaFields.GetItemByName("OpeningBalance").Text = Val(Replace(LV.ListItems(1).SubItems(5), ",", ""))
    rpt.FormulaFields.GetItemByName("OpenCrDr").Text = "'" & Right(LV.ListItems(1).SubItems(5), 2) & "'"
    
    If LVCheque.Visible Then
        Dim rptSub As CRAXDDRT.Report
        Set rptSub = rpt.OpenSubreport("ChqLedger")
        rptSub.RecordSelectionFormula = "{VChqLedger.BankAccNo}='" & cmbAccNo & "' AND ({VChqLedger.CDate}=#" & DtFrom.Tag & "# TO #" & DtTo.Tag & "#)"
    End If
    Dim frm As New frmPrevRpt
    frm.ShowReport "{VLedger.AccNo}='" & cmbAccNo & "' AND ({VLedger.VDate}=#" & DtFrom.Tag & "# TO #" & DtTo.Tag & "#)", rpt
...
Private Sub Form_Load()
    ...
    If Month(Date) > 6 Then
        DtFrom = DateSerial(year(Date), 7, 1)
    Else
        DtFrom = DateSerial(year(Date) - 1, 7, 1)
    End If
    
    DtTo = GetSingleDateValue("MAX(VDate)", "Vouchers")
    ...
    If GetSingleLongValue("COUNT(*)", "VoucherTypesToAuthorize", " WHERE WaitForAuthorization=1") > 0 Then
        fraUnAuth.Visible = True
    Else
        fraUnAuth.Visible = False
    End If
...
Private Sub Form_Resize()
    ...
    LVCheque.Move 50, FDrag.Top + FDrag.Height, Me.ScaleWidth - 100, Me.ScaleHeight - (FAcc.Height + fraUnAuth.Height + LV.Height + 230)
    FDrag.ZOrder 1
...
Private Sub Label2_Click(Index As Integer)
    If Index = 0 Or Index = 1 Then
        If cmbAccNo.MatchFound Then
            Dim f As New frmVouchersForAuthorization
            Load f
            f.ShowForAccount (cmbAccNo.Text)
        End If
        '"SELECT * From VLedger1ForAuthorization WHERE Authorized=0 AND"
    End If
End Sub
    ...
    With rsLedger
        ...
        For i = 0 To .RecordCount - 1
            ...
            If !ChequeNo & "" = "" Then
                ...
                ITM.ListSubItems.add , , "-"
            Else
                ITM.ListSubItems.add , , !ChequeNo & ""
                ITM.ListSubItems.add , , Format(!ChequeDate & "", "dd-MMM-yyyy")
            End If
            ITM.ListSubItems.add , , !Handed_Over_To & ""
...
'            End If
    ...
    If LV.ListItems.count > 1 Then
        ...
        For i = 1 To ITM.ListSubItems.count
            ITM.ListSubItems(i).ForeColor = vbBlack
        Next
    End If
    ...
    txtUnAuthDebit = GetSingleLongValue("SUM(Debit)", "VouchersForAuthorization", " WHERE AccNo='" & cmbAccNo & "' AND Authorized=0")
    txtUnAuthCredit = GetSingleLongValue("SUM(Credit)", "VouchersForAuthorization", " WHERE AccNo='" & cmbAccNo & "' AND Authorized=0")
    txtLastBalance = Balance 'Val(LV.ListItems(LV.ListItems.count - 1).list
    If Right(txtLastBalance, 2) = "CR" Then
        txtLastBalance.Tag = -Val(Replace(txtLastBalance, ",", ""))
    Else
        txtLastBalance.Tag = Val(Replace(txtLastBalance, ",", ""))
    End If
    Dim dNetBalance As Double
    dNetBalance = Val(txtLastBalance.Tag) + Val(txtUnAuthDebit) - Val(txtUnAuthCredit)
...
Private Sub mnuBalanceTag_Click()
    ...
    On Error GoTo err
    con.Execute "INSERT INTO BalanceTags(Sno,UserName) VALUES(" & Val(LV.SelectedItem.key) & ",'" & CurrentUserName & "')"
    With LV.ListItems.add(LV.SelectedItem.Index + 1, "TAG" & Val(LV.SelectedItem.key))
        .ListSubItems.add
        .ListSubItems.add(, , "<  Balance OK  > (" & CurrentUserName & " " & Format(Now, "dd-MMM-yyyy HH:nn") & ")").ForeColor = RGB(0, 125, 0)
    End With
   ...
   .Open "Select * From BalanceTags Where Sno in(" & SnoList & ")", con, adOpenForwardOnly, adLockReadOnly
      For i = 0 To .RecordCount - 1
         ...
         With LV.ListItems.add(idx, "TAG" & ![SNo])
            .ListSubItems.add
            .ListSubItems.add(, , "<  Balance OK  > (" & rs!UserName & " " & Format(rs!DTEntry, "dd-MMM-yyyy HH:nn") & ")").ForeColor = RGB(0, 125, 0)
            '.ListSubItems.add(, , "<  Balance OK  > (" & !UserName & " " & Format(!DTEntry, "dd-MMM-yyyy HH:nn") & ")").ForeColor = RGB(0, 125, 0)
         End With
         .MoveNEXT
      Next
   .Close
