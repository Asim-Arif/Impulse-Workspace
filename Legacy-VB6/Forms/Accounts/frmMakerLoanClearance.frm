...
Begin VB.Form frmMakerLoanClearance 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Maker Loan Clearance"
   ClientHeight    =   4620
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6705
   ...
   End
End
Attribute VB_Name = "frmMakerLoanClearance"
...
Private Function Saved() As Boolean
    ...
    ElseIf Val(txtLTToClear) = 0 And Val(txtSTToClear) = 0 Then
        MsgBox "Invalid Amounts to Clear.", vbInformation
        Exit Function
    End If
    
    Dim lSTEntryID As Long, lLTEntryID As Long
    lSTEntryID = 0
    lLTEntryID = 0
    ...
    If Val(txtSTToClear) > 0 Then
        con.Execute "INSERT INTO MakerPostedBills_STDeductions(MPB_RefID,Amount,AccNo) VALUES(0," & Val(txtSTToClear) & ",'" & cmbAccName.ID & "')"
        lSTEntryID = GetSingleLongValue("MAX(EntryID)", "MakerPostedBills_STDeductions")
    End If
    
    If Val(txtLTToClear) > 0 Then
        con.Execute "INSERT INTO MakerAmtCleared(AccNo,DT,AmtClrd) VALUES('" & cmbAccName.ID & "','" & DTPicker1.Value & "'," & Val(txtLTToClear) & ")"
        lLTEntryID = GetSingleLongValue("MAX(EntryID)", "MakerAmtCleared")
    End If
    
    con.Execute "INSERT INTO MakerLoanClearance_Manual(DT,AccNo,MPB_STD_EntryID,MAC_EntryID,UserName,MachineName,Remarks) " & _
