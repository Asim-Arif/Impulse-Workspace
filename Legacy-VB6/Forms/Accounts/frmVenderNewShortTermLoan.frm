...
Begin VB.Form frmVenderNewShortTermLoan 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Vender Short Term Loan"
   ClientHeight    =   6720
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6705
   ...
   Begin VB.Frame FraBank 
      ...
      Begin VB.CommandButton cmdBank 
         ...
         Height          =   315
         Left            =   6030
         Picture         =   "frmVenderNewShortTermLoan.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   20
         Top             =   555
         Width           =   360
   ...
   End
End
Attribute VB_Name = "frmVenderNewShortTermLoan"
...
Private Function Saved() As Boolean
    ...
    ElseIf Val(txtAmt) = 0 Then
        ...
        Exit Function
    ElseIf chkPayByCheque.Value = vbUnchecked And cmbCashInHand.MatchFound = False Then
    ...
    SQL = "INSERT INTO MakerAdvancesShort(AccNo,DT,Description,Amount,AmountCleared,AccVoucherNo) VALUES('" & _
     cmbAccNo.id & "','" & DTPicker1 & "','" & txtDesc & "'," & Val(txtAmt) & ",0,'" & strVchrNo & "')"
...
Private Sub cmbAccName_matched()
    ...
    With rs
        .Open "SELECT VendID FROM Venders WHERE AccNo='" & cmbAccName.id & "'", con, adOpenForwardOnly, adLockReadOnly
        
        lVenderID = Val(!VendID & "")
        .Close
        
        
        .Open "SELECT SUM(Amount-AmountCleared) AS TotalDeductionAmt FROM MakerAdvancesShort WHERE AccNo='" & cmbAccNo.id & "'", con, adOpenForwardOnly, adLockReadOnly
...
Public Sub ShowMe(Optional p_lRequestEntryID As Long = 0)
    
    
    
    With cmbAccNo
        .ClearVals
        .AddVals con, "AccNo", "VVenders", "AccNo", " ORDER BY AccNo"
    End With
        
    With cmbAccName
        .ClearVals
        .AddVals con, "AccTitle", "VVenders", "AccNo", " ORDER BY AccNo"
    End With
