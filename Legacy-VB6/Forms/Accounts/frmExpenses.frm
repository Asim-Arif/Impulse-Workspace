...
Begin VB.Form frmExpenses 
   ...
   Begin VB.Frame FALL 
      ...
      Top             =   -30
      Width           =   11835
      Begin VB.TextBox txtHandedOver 
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Private Sub cmbACCTitle_Click()
    ...
    dBalance = getBalance(cmbAccNo.Text, DTPicker1)
    lblPartyBalance = Format(dBalance, "#,##0.00 DR;#,##0.00 CR;#,##0.00      ")
End Sub
...
Private Sub DTPicker1_Change()
    ...
    txtVchrNo = getNextVchrNo(DTPicker1.Value, IIf(chkBank = vbUnchecked, "CPV", "BPV"))
    Call ShowBals
...
'    End If
   ...
   '  Check Whether Expense is Charged To Bank Or To Cash in Hand
    ...
    If chkBank.Value = vbChecked Then
        ...
        con.Execute "Insert Into Cheque ( SNo, CDate, AccNo, Description, Amount, BankID, chequeno, ChqBookNo, chequeType, chequeDate, Posted, Payment, Bounced,Payee,BankAccNo) Values(" & _
        INextSno & ",'" & Format(DTPicker1.Value, "dd-MMM-yyyy") & "','" & _
         CreditAcc & "','" & TBox(4) & "'," & Label1(1).Caption & "," & lblBankAcc.Tag & ",'" & _
         txtChqNo & "'," & txtChqNo.Tag & ",'" & cmbCheckType & "','" & DTPicker2 & "',1,1,0,'" & txtPayee & "','" & GetID(lblBankAcc) & "')"
    ...
    Else
        con.Execute "INSERT INTO VouchersForAuthorization(VDate,VchrNo,Accno,Description,Debit,Credit,balance,DpstSlip,CSNo) VALUES('" & Format(DTPicker1.Value, "dd-MMM-yyyy") & "','" & _
             Label1(4) & "','" & CreditAcc & "','" & theDesc & "',0," & Label1(1) & ",0,''," & CSNo & ")"
    ...
    For i = 1 To LView.ListItems.count
        ...
        Else
            con.Execute "INSERT INTO VouchersForAuthorization(VDate,VchrNo,Accno,Description,Debit,Credit,balance,DpstSlip,CSNo) VALUES('" & Format(DTPicker1.Value, "dd-MMM-yyyy") & "','" & _
             Label1(4) & "','" & LView.ListItems(i).Text & "','" & LView.ListItems(i).ListSubItems(2).Text & "'," & _
             Val(LView.ListItems(i).ListSubItems(3).Text) & ",0,0,0," & CSNo & ")"
        End If
    ...
    If bWaitForAuth = False Then
        con.Execute "INSERT INTO VoucherInfo(VchrNo,UserName,MachineName,DT,Notes,Payee,Handed_Over_To) VALUES('" & Label1(4) & "','" & CurrentUserName & "','" & strComputerName & "','" & GetServerDate(True) & "','" & TBox(4) & "','" & txtPayee & "','" & txtHandedOver & "')"
    Else
        con.Execute "INSERT INTO VouchersForAuthorizationInfo(VchrNo,UserName,MachineName,DT,PostedThroughJVForm,Notes,Payee) VALUES('" & Label1(4) & "','" & CurrentUserName & "','" & strComputerName & "','" & GetServerDate(True) & "',0,'" & TBox(4) & "','" & txtPayee & "')"
    End If
