...
Private Sub cmdSClear_Click()
    If SaveTrans Then
        ...
        If MsgBox("Do You Want To Enter The Amount Received ?", vbQuestion + vbYesNo) = vbYes Then
            With frmCashRV
...
PermissionDenied = Not UserHasAccess("SaleVoucher")
Me.KeyPreview = True
...
Private Sub FillAccCmbs()
    ...
    With AccRec
        ...
        .Open "Select AccNo,AccTitle from Accounts as Acc1 Where (AccNo Like '31%') AND (AccNo not Like '" & SRTACC & "%') AND (Parent=0) AND Active=1 ", con, adOpenForwardOnly, adLockReadOnly
...
Private Function SaveTrans() As Boolean
    ...
    If LView.ListItems.Count = 0 Then
        ...
        Exit Function
    ElseIf Not cmbCustAccNo.MatchFound Then
       MsgBox "Customer Account Is Not Selected Or Invalid.", vbInformation
       Exit Function
    End If
...
VNo = getNextVchrNo(DTPicker1.Value, "JV")
   ...
   INextSno = INextSno + 1
    ...
    For i = 1 To LVPymt.ListItems.Count
        If LVPymt.ListItems(i).Checked Then con.Execute "Insert into VoucherDetail Values (" & Val(LVPymt.ListItems(i).key) & "," & SnoToUse & "," & Val(LVPymt.ListItems(i).SubItems(1)) & ")"
    Next
...
VNo = getNextFakeVchrNo(DTPicker1.Value, "JV")
   ...
   Call adjustFakeLedger(cmbCustAccNo, DTPicker1.Value, INextSno, Val(lblTotal))
    ...
    'Print If To Be Printed
        If chkPrint = vbChecked And FakeAccounts Then
            Call PlaySaveSound
            Call PrintVoucher(frmMain.cr1, VNo, True)
        End If
