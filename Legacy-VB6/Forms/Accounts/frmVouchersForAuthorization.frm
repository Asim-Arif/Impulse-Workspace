...
Begin VB.Form frmVouchersForAuthorization 
   ClientHeight    =   8175
   ClientLeft      =   60
   ClientTop       =   240
   ClientWidth     =   11880
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVouchersForAuthorization.frx":0000
            Key             =   "PND"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVouchersForAuthorization.frx":0458
            Key             =   "CLR"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVouchersForAuthorization.frx":08AC
            Key             =   "DUE"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVouchersForAuthorization.frx":0D04
            Key             =   "BNC"
         EndProperty
      EndProperty
   End
   ...
   Begin VB.Frame FAcc 
      ...
      Begin VB.ComboBox cmbRange 
         Height          =   315
         ItemData        =   "frmVouchersForAuthorization.frx":1158
         Left            =   4275
         List            =   "frmVouchersForAuthorization.frx":116B
      ...
      Begin VB.Label Label1 
         ...
         BackColor       =   &H00AFBDCB&
         BackStyle       =   0  'Transparent
         Caption         =   $"frmVouchersForAuthorization.frx":11AD
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Menu mnupop 
      ...
      End
      Begin VB.Menu mnuAuthorizeThisVoucher 
         Caption         =   "Authorize This Voucher"
      End
   ...
   End
End
Attribute VB_Name = "frmVouchersForAuthorization"
...
Public Sub ShowForAccount(p_strAccNo As String)
    strAccNo = p_strAccNo
    Loaded = True
    Me.Show
    Call RefreshLedger
...
Private Sub Form_Resize()
    ...
    fraUnAuth.Top = cmdPrint.Top - 900 + cmdPrint.Height
    ...
    If LVCheque.Visible Then
        ...
        FDrag.Visible = True
    Else
        TheHeight = Me.ScaleHeight - fraUnAuth.Height - 200
        FDrag.Visible = False
    End If
        ...
        For j = 0 To iCount
            If strAccNo = "" Then
                If cmbRange.ListIndex = 0 Then
                    SQL = "SELECT * FROM VLedger1ForAuthorization WHERE Authorized=0 AND VDate = '" & DtFrom & "' AND VchrNo Like '%" & Chks(j).Tag & "%' ORDER BY VDate,VchrNo,Debit DESC"
                ElseIf j = 4 Then
                    SQL = "SELECT * FROM VLedger1ForAuthorization WHERE Authorized=0 AND (VDate between '" & DtFrom & "' AND '" & DtTo & "') AND (VchrNo Like '%" & Chks(j).Tag & "%' OR LEFT(VchrNo,2)='RV') ORDER BY VDate,VchrNo,Debit DESC"
                Else
                    SQL = "SELECT * FROM VLedger1ForAuthorization WHERE Authorized=0 AND (VDate between '" & DtFrom & "' AND '" & DtTo & "') AND (VchrNo Like '%" & Chks(j).Tag & "%') ORDER BY VDate,VchrNo,Debit DESC"
                End If
            Else
                SQL = "SELECT * FROM VLedger1ForAuthorization WHERE Authorized=0 AND VchrNo IN(SELECT VchrNo FROM VLedger1ForAuthorization WHERE AccNo='" & strAccNo & "') ORDER BY VDate,VchrNo,Debit DESC"
            End If
            ...
            For i = 0 To .RecordCount - 1
                ...
                ITM.ListSubItems.add(, , ![AccTitle] & "").Tag = !AccNo & ""
...
Private Sub mnuAuthorizeThisVoucher_Click()

    On Error GoTo err
    
    Dim bAccess As Boolean
    bAccess = GetSingleBooleanValue("AuthorizeVouchers", "Users", " WHERE UserID=" & UserID)
    
    If bAccess = False Then
        MsgBox "You don't have Access.", vbCritical
        Exit Sub
    End If
    
    Dim myDT As Date, strVchrNo As String
    strVchrNo = LV.SelectedItem.SubItems(1)
    'Check if It's the First in UnAuthorized List...
'    Dim lVchrEntryID As Long, lEntryID As Long
'    lEntryID = GetSingleLongValue("MIN(SNo)", "VouchersForAuthorization", " WHERE Authorized=0")
'    lVchrEntryID = GetSingleLongValue("MIN(SNo)", "VouchersForAuthorization", " WHERE VchrNo='" & strVchrNo & "' AND Authorized=0")
'    If lVchrEntryID <> lEntryID Then
'        MsgBox "This is not the First Un-Auth Voucher.", vbInformation
'        Exit Sub
'    End If
    ...
    With rs
        .Open "SELECT * FROM VouchersForAuthorization WHERE VchrNo='" & LV.SelectedItem.SubItems(1) & "' ORDER BY SNo", con, adOpenForwardOnly, adLockReadOnly
        ReDim strAccNo(.RecordCount, 5)
        myDT = !Vdate
        strVchrNo = !VchrNo & ""
        iIndex = 0
        Do Until .EOF
            strAccNo(iIndex, 0) = !AccNo & ""
            strAccNo(iIndex, 1) = !Description & ""
            strAccNo(iIndex, 2) = Val(!Debit & "")
            strAccNo(iIndex, 3) = Val(!Credit & "")
            strAccNo(iIndex, 4) = Val(!CSNo & "")
            iIndex = iIndex + 1
            .MoveNEXT
        Loop
        .Close
    ...
    objVchr.VoucherDate = myDT
    
    For iIndex = 0 To UBound(strAccNo) - 1
        objVchr.AddVoucherHead strAccNo(iIndex, 0), Val(strAccNo(iIndex, 2)) - Val(strAccNo(iIndex, 3)), strAccNo(iIndex, 1), Val(strAccNo(iIndex, 4))
    Next
    
    Call objVchr.PostVoucher(False, True, strVchrNo)
    con.Execute "UPDATE VouchersForAuthorization SET Authorized=1 WHERE VchrNo='" & strVchrNo & "'"
    con.Execute "UPDATE VoucherInfo SET VoucherInfo.Notes=T1.Notes,VoucherInfo.Payee=T1.Payee FROM VoucherInfo INNER JOIN VouchersForAuthorizationInfo T1 ON VoucherInfo.VchrNo=T1.VchrNo"
...
Private Sub mnuCatTrans_Click()

    Screen.MousePointer = vbHourglass
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\TransRpt.rpt")
    ...
    If Opts(0) Then
        'Transaction Report
        Dim VoucherTypes As String
        VoucherTypes = "'" & IIf(Chks(0) = vbChecked, Chks(0).Tag, "") & IIf(Chks(1) = vbChecked, Chks(1).Tag, "") & IIf(Chks(2) = vbChecked, Chks(2).Tag, "") & IIf(Chks(3) = vbChecked, Chks(3).Tag, "") & "'"
        strSelection = "({VLedger.VDate}=date(" & Format(DtFrom, "yyyy,MM,dd") & ") TO date(" & Format(DtTo, "yyyy,MM,dd") & ")) and (left({Vledger.VchrNo},2) in " & VoucherTypes & ")"
    ElseIf Opts(1) Then
        'Transaction Report
        Dim TransTypes As String
        strSelection = "({VLedger.VDate}=date(" & Format(DtFrom, "yyyy,MM,dd") & ") TO date(" & Format(DtTo, "yyyy,MM,dd") & ")) and ({Vledger.AccNo} like '" & CashInHand & "*' Or {Vledger.AccNo} like " & IIf(Chks1(0) = vbChecked, "'15-002*'", "''") & ")"
    End If
     
    Dim rptSUB As CRAXDDRT.Report
    Set rptSUB = rpt.OpenSubreport("ChqLedger")
    If Opts(0) Then
        '  Transaction Report
        rptSUB.RecordSelectionFormula = "({VChqLedger.cDate}=date(" & Format(DtFrom, "yyyy,MM,dd") & ") TO date(" & Format(DtTo, "yyyy,MM,dd") & ")) AND ({vChqledger.Payment}=" & CBool(Chks(1) <> vbChecked) & " OR   {vChqledger.Payment}<>" & CBool(Chks(2) <> vbChecked) & ")"
    ElseIf Opts(1) Then
        '  Cash book Report
        rptSUB.RecordSelectionFormula = "({VChqLedger.cDate}=date(" & Format(DtFrom, "yyyy,MM,dd") & ") TO date(" & Format(DtTo, "yyyy,MM,dd") & ")) and UpperCase(trim({VChqLedger.chequeType}))<>'CANCELED'"
    End If
...
Private Sub PrintCashnBankTransaction()
    ...
    Set rpt = rptApp.OpenReport(RptPath & "\TransRptCashnBank.rpt")
    rpt.FormulaFields.GetItemByName("Company").Text = "'" & strCompany & "'"
    rpt.FormulaFields.GetItemByName("FromTo").Text = "'" & Format(DtFrom, "dd-MMM-yyyy") & " To " & Format(DtTo, "dd-MMM-yyyy") & "'"
    rpt.FormulaFields.GetItemByName("Address").Text = "'" & COMPANYADDRESS & "'"
    ...
    rptCashEntries.RecordSelectionFormula = "{VLedger.VDate}=#" & DtFrom & "# TO #" & DtTo & "# AND {VLedger.Accno}='" & CashInHand & "'"
    ...
    rptCashBalances.RecordSelectionFormula = "{Accounts.AccNo}='" & CashInHand & "' OR {Accounts.SubAccOf}='15-015'"
...
Private Sub mnuDailyTrans_Click()
        
     
    If Opts(0) = False Then
        Call PrintCashnBankTransaction
        Exit Sub
    End If
    Screen.MousePointer = vbHourglass
    Dim rpt As CRAXDDRT.Report
    Set rpt = rptApp.OpenReport(RptPath & "\TransRpt1.rpt")
    Dim strSelection As String
    rpt.FormulaFields.GetItemByName("Company").Text = "'" & strCompany & "'"
    rpt.FormulaFields.GetItemByName("FromTo").Text = "'" & Format(DtFrom, "dd-MMM-yyyy") & " To " & Format(DtTo, "dd-MMM-yyyy") & "'"
    rpt.FormulaFields.GetItemByName("Address").Text = "'" & COMPANYADDRESS & "'"
...
'    frmDelVchr.ShowMe LV.SelectedItem.SubItems(1)
    If MsgBox("Are you sure ?", vbQuestion + vbYesNo) = vbYes Then
        Call DeleteVoucherUnAuthorized(LV.SelectedItem.SubItems(1))
        Call cmdRefresh_Click
    End If
...
Private Sub mnuPreviewVoucher_Click()
    ...
    Call PrintVoucher(cr1, LV.SelectedItem.SubItems(1), , False, True)
...
Private Sub mnuPrintCheque_Click()
    ...
    Load f
    Call f.ShowMe(LV.SelectedItem.Tag, True)
End Sub
...
Private Sub mnuprintOnblank_Click()
    ...
    If LV.ListItems.count = 0 Then Exit Sub
    Call PrintVoucher(cr1, LV.SelectedItem.SubItems(1), , True, True)
...
Private Sub Opts_Click(Index As Integer)
   PicOpts(0).Visible = Opts(0)
   PicOpts(1).Visible = Opts(1)
   Call cmdRefresh_Click
End Sub
...
For j = 0 To 1
   Dim VType As String
   'if Showing Cash Book Report then Show All Payment Or Receipt Not Catagorised
   If Opts(1) And j = 1 Then Screen.MousePointer = vbDefault: Exit Sub
   If Opts(0) Then If j = 0 Then VType = "RV" Else VType = "PV"
   
   Dim BankCondition As String
   
   'Check For Bank Accounts To Show Or Not
   If Opts(1) Then
      If Chks1(1) = vbChecked Then
         BankCondition = " Or AccNo Like '15-002%'"
      Else
         BankCondition = ""
      End If
   ElseIf Opts(0) Then
   ...
   End If
      ...
      SQL = "select * from VLedger where (VDate between '" & DtFrom & "' and '" & DtTo & "') and (Accno like '" & CashInHand & "%' " & BankCondition & ") And Vchrno Like '%" & VType & "%'  Order By VDate,SNo"
   
   
   
    If .State = 1 Then .Close
       .Open SQL, con, adOpenForwardOnly, adLockReadOnly
       If .RecordCount = 0 Then GoTo EndOfLoop
   Dim trans As String
   
   ''''''''''''''    Show Heading Only For Transaction Report
   ''''''''''''''    Not For Cash Book Report
   
   If Opts(0) Then
      If j = 0 Then trans = "Receipt Transaction" Else trans = "Payment Transaction"
     
      LV.ListItems.add
      With LV.ListItems.add().ListSubItems
       .add
       .add
       .add(3, , trans).Bold = True
...
RSTOTAL.Open "Select Sum(Debit) as TotalDebit,Sum(Credit) as TotalCredit From VLedger where (VDate between '" & DtFrom & "' and '" & DtTo & "') and (Accno like '" & CashInHand & "%' " & BankCondition & ") And Vchrno Like '" & VType & "%'", con, adOpenForwardOnly, adLockReadOnly


Dim TotalCaption As String
If VType = "RV" Then
   TotalCaption = "                      Total Receipt :"
ElseIf VType = "PV" Then
   TotalCaption = "                      Total Payment :"
End If
...
With LV.ListItems.add().ListSubItems
   ...
   .add(, , Format(RSTOTAL![TotalDebit], "#,##0.00")).Bold = True
   .add(, , Format(RSTOTAL![TotalCredit], "#,##0.00")).Bold = True
End With
...
'"(VDate" & Format(DtFrom.value, "(yyyy,MM,dd)") & " to Date" & Format(DtTo.value, "(yyyy,MM,dd)") & ") and ({ledgerrpt.acno}='" & Trim(Text16.Text) & "')"
'Now Fill The Other List With Cheques
If Chks1(1).Value = vbChecked Then
   Call RefreshChequeLedger(False)
   Call RefreshChequeLedger(True)
End If
Screen.MousePointer = vbDefault
End Sub
...
Private Sub txtDesc_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        If Trim(txtDesc = "") Then
            ...
            Exit Sub
        End If
        con.Execute "UPDATE VouchersForAuthorization SET [Description]='" & txtDesc & "' WHERE Sno=" & Val(LV.SelectedItem.key)
