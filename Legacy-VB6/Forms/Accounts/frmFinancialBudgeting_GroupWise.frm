...
Begin VB.Form frmFinancialBudgeting_GroupWise 
   ClientHeight    =   8520
   ClientLeft      =   120
   ClientTop       =   180
   ClientWidth     =   13905
   ...
   End
End
Attribute VB_Name = "frmFinancialBudgeting_GroupWise"
...
Private Sub cmbFinYear_Click()
    ...
    With rs
        .Open "SELECT * FROM GroupsForExpenseReport ORDER BY EntryID", con, adOpenForwardOnly, adLockReadOnly
        ...
        .Open "SELECT * FROM FinancialBudgeting_GroupWise WHERE FinancialYear='" & cmbFinYear.List(cmbFinYear.ListIndex, 1) & "'", con, adOpenForwardOnly, adLockReadOnly
        
        Do Until .EOF
            On Error Resume Next
            Set ITM = LV.ListItems(!GFER_RefID & "'")
...
Private Sub txtEdit_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        con.Execute "UPDATE FinancialBudgeting_GroupWise SET " & LV.ColumnHeaders(iColNo).Tag & "=" & Val(txtEdit) & _
         " WHERE FinancialYear='" & strFinYear & "' AND GFER_RefID=" & Val(LV.SelectedItem.key), iRecAffect
        If iRecAffect = 0 Then
            con.Execute "INSERT INTO FinancialBudgeting_GroupWise(FinancialYear,GFER_RefID," & LV.ColumnHeaders(iColNo).Tag & _
             ") VALUES('" & strFinYear & "','" & Val(LV.SelectedItem.key) & "'," & Val(txtEdit) & ")"
        End If
