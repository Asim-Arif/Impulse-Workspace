...
Begin VB.Form frmMakerNewShortTermLoan 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Maker Short Term Loan...."
   ClientHeight    =   8340
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6705
   ...
   Begin ComboList.Usercontrol1 cmbAccNo_All 
      Height          =   285
      Left            =   1920
      TabIndex        =   42
      Top             =   4395
   ...
   Begin ComboList.Usercontrol1 cmbAccName 
      ...
      ListIndex       =   -1
   End
   Begin ComboList.Usercontrol1 cmbMasterPONo 
      Height          =   360
      Left            =   1920
      TabIndex        =   31
      Top             =   3300
   ...
   Begin VB.Frame FraBank 
      ...
      Begin VB.CommandButton cmdBank 
         ...
         Height          =   315
         Left            =   6030
         Picture         =   "frmMakerNewShortTermLoan.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   20
         Top             =   555
         Width           =   360
   ...
   Begin MSForms.TextBox txtLTDed 
      ...
      ParagraphAlign  =   3
   End
   Begin MSForms.TextBox txtPOValue 
      Height          =   360
      Left            =   5025
      TabIndex        =   38
      Top             =   3300
   ...
   Begin VB.Label Label1 
      ...
      Width           =   840
   End
   Begin MSForms.TextBox txtSTTaken 
      Height          =   360
      Left            =   3465
      TabIndex        =   36
      Top             =   3900
   ...
   Begin VB.Label Label1 
      ...
      Width           =   855
   End
   Begin MSForms.TextBox txtLTTaken 
      Height          =   360
      Left            =   1920
      TabIndex        =   34
      Top             =   3900
   ...
   End
End
Attribute VB_Name = "frmMakerNewShortTermLoan"
...
Private Function Saved() As Boolean
    ...
    Saved = False
    'On Error GoTo ERR
    Dim SQL As String, bPOMaker As Boolean
    ...
    bPOMaker = False 'GetSingleBooleanValue("POMaker", "Makers", " WHERE AccNo='" & cmbAccNo.ID & "'")
    
    If bPOMaker Then
    
        If cmbMasterPONo.MatchFound = False Then
            MsgBox "Please Select PO.", vbInformation
            Exit Function
        End If
        
        Dim lPOValue As Long, lLTTaken As Long, lSTTaken As Long, lNewAdvance As Long, lPOLimit As Long
        lPOValue = Val(txtPOValue)
        lLTTaken = Val(txtLTTaken)
        lSTTaken = Val(txtSTTaken)
        lNewAdvance = Val(txtAmt)
        lPOLimit = lPOValue / 2
        
        If (lLTTaken + lSTTaken + lNewAdvance) > lPOLimit Then
            If MsgBox("Allowing more than 50% Advance is against Company Policy. You want to Proceed ?", vbQuestion + vbYesNo) = vbNo Then
                Exit Function
            End If
        End If
    ...
    If chkPost.Value = vbChecked Then
        ...
        If chkPayByCheque.Value = vbUnchecked And cmbCashInHand.MatchFound = False And chkOtherAccount.Value = vbUnchecked Then
            MsgBox "Please Select Cash Account.", vbInformation
            Exit Function
        ElseIf chkPayByCheque.Value = vbUnchecked And chkOtherAccount.Value = vbChecked And cmbAccNo_All.MatchFound = False Then
        ...
        If chkPayByCheque.Value = vbUnchecked Then
            ...
            If chkOtherAccount.Value = vbUnchecked Then
...
'        If cmbCheckType.ListIndex = 0 Then
'
'            Dim myDT As Date, lAmt As Long, strBankAccNo As String, strPartyAccNo As String, strDescription As String
'            myDT = DTPicker1.Value
'            lAmt = Val(txtAmt)
'            strDescription = "W/H Tax On Cash Chq # " & TBox(2)
'            strBankAccNo = GetID(lblBankAcc)
'            strPartyAccNo = cmbAccNo.ID
'            Call AddWHTaxOnCashWithdrawalVoucher(myDT, lAmt, strBankAccNo, strPartyAccNo, strDescription)
'
'        End If
'    End If
    
    SQL = "INSERT INTO MakerAdvancesShort(AccNo,DT,Description,Amount,AmountCleared,AccVoucherNo,MasterPONo) VALUES('" & _
     cmbAccNo.ID & "','" & DTPicker1 & "','" & txtDesc & "'," & Val(txtAmt) & ",0,'" & strVchrNo & "','" & cmbMasterPONo.ID & "')"
...
Private Function SaveTransForCheque() As String
    ...
    theDesc = "Short Term Advance Paid To " & cmbAccName.Text
...
Private Function SavePaymentTrans(ChqSNo As Double) As String
    ...
    If cmbMasterPONo.MatchFound Then
        theDesc = theDesc & vbNewLine & " Against PO #" & cmbMasterPONo.ID & ", PO Value :" & Val(txtPOValue) & " , Already L.T Taken :" & Val(txtLTTaken) & " , Already S.T Taken : " & Val(txtSTTaken)
    End If
...
Private Function AddVoucher() As String
    ...
    strDesc = "Short Term Advance Paid To " & cmbAccName.Text
    If cmbMasterPONo.MatchFound Then
        strDesc = strDesc & vbNewLine & " Against PO #" & cmbMasterPONo.ID & ", PO Value :" & Val(txtPOValue) & " , Already L.T Taken :" & Val(txtLTTaken) & " , Already S.T Taken : " & Val(txtSTTaken)
    End If
    
    Dim strDebitAccNo As String, strMakerAccNo As String
    If chkOtherAccount.Value = vbChecked Then
        strDebitAccNo = cmbAccNo_All.ID
    Else
        strDebitAccNo = cmbCashInHand.List(cmbCashInHand.ListIndex, 1)
    End If
    
    strMakerAccNo = cmbAccNo.Text
    Dim dValue As Double
    dValue = Val(txtAmt)
    Dim objVchr As New clsVouchers
    objVchr.VoucherDate = DTPicker1.Value 'GetServerDate
    ...
    'objVchr.LastPaymentAccNo = strMakerAccNo
    
    objVchr.AddVoucherHead strMakerAccNo, dValue, strDesc
    objVchr.AddVoucherHead strDebitAccNo, -dValue, strDesc
...
Private Sub chkOtherAccount_Click()
   ...
   If chkOtherAccount.Value = vbChecked Then
        cmbAccNo_All.Visible = True
        cmbCashInHand.Visible = False
        If cmbAccNo_All.Visible Then
            cmbAccNo_All.SetFocus
        End If
    Else
        cmbAccNo_All.Visible = False
        cmbCashInHand.Visible = True
        If cmbCashInHand.Visible Then
...
Private Sub cmbAccName_matched()
    ...
    With rs
        .Open "SELECT VendID FROM Makers WHERE AccNo='" & cmbAccName.ID & "'", con, adOpenForwardOnly, adLockReadOnly
        If .EOF = False Then
        ...
        If lVenderID = 0 Then
            bVendor = True
            lVenderID = GetSingleLongValue("VendID", "Venders", " WHERE AccNo='" & cmbAccName.ID & "'")
        End If
        
        .Open "SELECT SUM(Amount-AmountCleared) AS TotalDeductionAmt FROM VMakerAdvancesShortDeduction WHERE AccNo='" & cmbAccNo.ID & "'", con, adOpenForwardOnly, adLockReadOnly
        txtBal.Text = Val(!TotalDeductionAmt & "")
        .Close
    ...
    cmbMasterPONo.ClearVals
    If bVendor Then
        cmbMasterPONo.AddVals con, "OrderNo", "VendOrders", "VendOrders.OrderNo", " WHERE VendID='" & cmbAccNo.ID & "' AND DT>='12/18/15'"
    Else
        cmbMasterPONo.AddVals con, "DISTINCT MasterPONo", "VendIssued", "MasterPONo", " WHERE VendID=" & lVenderID & " AND DT>='12/18/15' AND MasterPONo IS NOT NULL"
    End If
...
Private Sub cmbAccNo_matched()
    
    cmbAccName.ID = cmbAccNo.ID
    Dim rs As New ADODB.Recordset
    With rs
        .Open "SELECT VendID FROM Makers WHERE AccNo='" & cmbAccNo.ID & "'", con, adOpenForwardOnly, adLockReadOnly
        If .EOF = False Then
            lVenderID = !VendID
        End If
        .Close
...
Private Sub cmbMasterPONo_matched()
    
    If bVendor Then
        txtPOValue = GetSingleLongValue("SUM(Qty*Rate)", "VendOrderDetail", " WHERE OrderNo='" & cmbMasterPONo.ID & "'")
    Else
        txtPOValue = GetSingleLongValue("SUM(IssQty*Rate)", "VendIssdDetail INNER JOIN VendIssued ON VendIssued.EntryID=VendIssdDetail.RefID", " WHERE MasterPONo='" & cmbMasterPONo.ID & "'")
    End If
    txtLTTaken = GetSingleLongValue("SUM(Amount)", "MakerAdvances", " WHERE MasterPONo='" & cmbMasterPONo.ID & "'")
    txtSTTaken = GetSingleLongValue("SUM(Amount)", "MakerAdvancesShort", " WHERE MasterPONo='" & cmbMasterPONo.ID & "'")
...
Private Sub cmdNext_Click()
    ...
    If Saved Then
        ...
        Dim strVend_AccNo As String, myDT As Date, bShowOtherAccounts As Boolean, strAccNo As String
        strVend_AccNo = cmbAccNo.ID
        myDT = DTPicker1.Value
        
        If chkOtherAccount.Value = vbChecked Then
            bShowOtherAccounts = True
            strAccNo = cmbAccNo_All.ID
        Else
            bShowOtherAccounts = False
            strAccNo = cmbCashInHand.List(cmbCashInHand.ListIndex, 1)
        End If
        ...
        Call f.ShowMe(0, strVend_AccNo, myDT, bShowOtherAccounts, strAccNo)
...
Public Sub ShowMe(Optional p_lRequestEntryID As Long = 0, Optional strMakerAccNo As String = "", Optional myDT As Date = #1/1/1900#, Optional bShowOtherAccounts As Boolean = False, Optional strAccNo As String = "")
...
'    If strMakerAccNo <> "" Then
'        cmbAccNo.ID = strMakerAccNo
'    End If
    ...
    cmbAccNo_All.ListHeight = 1600
    cmbAccNo_All.AddVals con, "'{' + AccNo  + '} ' + AccTitle", "VActiveAccounts", "AccNo"
        
    If myDT <> #1/1/1900# Then
    
        Dim i As Integer
        DTPicker1.Value = myDT
        
        If bShowOtherAccounts Then
            chkOtherAccount.Value = vbChecked
            cmbAccNo_All.ID = strAccNo
        Else
            chkOtherAccount.Value = vbUnchecked
            For i = 0 To cmbCashInHand.ListCount - 1
                If cmbCashInHand.List(i, 1) = strAccNo Then
                    cmbCashInHand.ListIndex = i
                    Exit For
                End If
            Next
        End If
