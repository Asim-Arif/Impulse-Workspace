...
Begin VB.Form frmNewCustomPListM 
   ClientHeight    =   9375
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   11820
   ...
   EndProperty
   Icon            =   "frmNewCustomPListM.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame FAll 
      ...
      Begin VB.CommandButton cmdNextCrtn 
         ...
         Height          =   525
         Left            =   75
         Picture         =   "frmNewCustomPListM.frx":0442
         TabIndex        =   31
         Top             =   1350
         UseMaskColor    =   -1  'True
         Visible         =   0   'False
      ...
      Begin VB.Frame FNew 
         ...
         Begin VB.CommandButton cmdAddItem 
            ...
            Height          =   555
            Left            =   10485
            Picture         =   "frmNewCustomPListM.frx":0974
            Style           =   1  'Graphical
            TabIndex        =   11
         ...
         Begin VB.Label Label1 
            ...
            Caption         =   $"frmNewCustomPListM.frx":0EA6
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   8.25
               Charset         =   0
      ...
      Begin VB.CommandButton cmdDelete 
         ...
         Height          =   285
         Left            =   11115
         Picture         =   "frmNewCustomPListM.frx":0F55
         Style           =   1  'Graphical
         TabIndex        =   13
   ...
   End
End
Attribute VB_Name = "frmNewCustomPListM"
...
Private Sub cmbCustomInvoice_Click()
    ...
    With rs
        '.Open "SELECT CustomInvoiceItemsEntryID,ItemID,Description,Qty,OrderNo From VrptCustomInvoiceDetail WHERE CustomInvoice='" & cmbCustomInvoice.Text & "'", con, adOpenForwardOnly, adLockReadOnly
        .Open "SELECT ItemID,Description,SUM(Qty) AS Qty,MAX(CustomInvoiceItemsEntryID) AS CustomInvoiceItemsEntryID FROM VrptCustomInvoiceDetail WHERE CustomInvoice='" & cmbCustomInvoice.Text & "' GROUP BY ItemID,Description", con, adOpenForwardOnly, adLockReadOnly
        cmbItemID.Clear
        cmbItemDesc.Clear
        Do Until .EOF
...
'    Next i
    ...
    For i = 1 To LV.ListItems.count
        With LV.ListItems(i)
            'con.Execute "Insert Into CustomPList(CartonFrom" & _
             ",Qty,PInvoiceID) Values(" & _
             Val(.Text) & "," & Val(.SubItems(1)) & _
             "," & Val(.ListSubItems(3).Tag) & ")"
             
             '''' Changed Due To error of Field Name
             
             
             'con.Execute "Insert Into CustomPList(CartonFrom" & _
             ",Qty,PInvoiceID,OrderItemID) Values(" & _
             Val(.Text) & "," & Val(.SubItems(1)) & _
             "," & Val(cmbCustomInvoice.List(cmbCustomInvoice.ListIndex, 1)) & _
             "," & Val(.ListSubItems(3).Tag) & ")"
             
             con.Execute "INSERT INTO CustomPList(CustomInvoice,CartonFrom,CartonTo" & _
             ",Qty,OrderItemID) VALUES('" & _
             cmbCustomInvoice & "'," & Val(.Text) & "," & Val(.SubItems(1)) & "," & Val(.ListSubItems(1).Tag) & _
             ",'" & Val(.ListSubItems(4).Tag) & "')"
