...
Begin VB.Form frmVendRcvWastage 
   ClientHeight    =   9945
   ClientLeft      =   60
   ClientTop       =   60
   ClientWidth     =   11880
   ...
   Begin VB.Frame FALL 
      ...
      Begin MSForms.Label Label2 
         ...
         ForeColor       =   4194304
         BackColor       =   12504280
         Caption         =   $"frmVendRcvWastage.frx":0000
         Size            =   "14208;397"
   ...
   End
End
Attribute VB_Name = "frmVendRcvWastage"
...
Private Sub cmbProcess_matched()
    ...
    If HicoVisible Then
        ...
        Else
            Call cmbVend.AddVals(con, "VenderName", "VVenders", "VendID", " Where (PhaseID=" & cmbProcess.ID & " Or PhaseID=-1) And VendID In(Select VendID From VVendIssItems Where ProcessID=" & cmbProcess.ID & " Group By VendID Having Sum(RemQty)>0)")
        End If
    Else
        If cmbProcess.ID = 7 Then
...
Private Sub cmbVend_matched()
    ...
    With rs
        ...
        .Open "Select ItemCode,ItemDescription,Sum(IssQty-RcvdQty)-dbo.F_AuthQtyForIss(ItemCode,null,null,null," & cmbVend.ID & ") as Quantity From VVendIssItems1 Where IssQty-RcvdQty>0 and VendID=" & cmbVend.ID & " and ProcessID=" & cmbProcess.ID & " And IsNull(ReqAuth,0)=0 Group By ItemCode,ItemDescription ", con, adOpenForwardOnly, adLockReadOnly
        LV.ListItems.Clear
        Do Until .EOF
...
Private Sub cmdSClear_Click()
    ...
    If Saved Then
        ...
        Unload Me
        Call ShowWastRcvForm(FormProcessID)
    End If
    'Call ClearFields
...
Private Sub txtQty_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        If Val(txtQty) = 0 Then
            ...
            Else
                If GotRcvID = 0 Then
                    GotRcvID = Val(con.Execute("Select Max(Cast(right(RecieptID,Len(RecieptID)-10) As Int)) From VVendRcvdItems Where Convert(varchar,DT,6)='" & Format(DTPicker1, "dd MMM yy") & "' and UPPER(Left(RecieptID,3))='WST'").Fields(0).value & "") + 1
                Else
                    GotRcvID = GotRcvID + 1
                End If
                'LV.SelectedItem.SubItems(LV.ColumnHeaders.Count - 1) = "RCV-" & _
