...
Begin VB.Form frmVendIssItemsByItem 
   ClientHeight    =   10125
   ClientLeft      =   1110
   ClientTop       =   60
   ClientWidth     =   11805
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmVendIssItemsByItem.frx":0000
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "LVBal"
      Tab(0).Control(1)=   "txtTotal"
      Tab(0).ControlCount=   2
      TabCaption(1)   =   "Vender History"
      TabPicture(1)   =   "frmVendIssItemsByItem.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "LVMaker"
      Tab(1).Control(1)=   "txtRate"
      Tab(1).ControlCount=   2
      TabCaption(2)   =   "Issued Items"
      TabPicture(2)   =   "frmVendIssItemsByItem.frx":0038
      Tab(2).ControlEnabled=   -1  'True
      Tab(2).Control(0)=   "LVIssued"
      Tab(2).Control(0).Enabled=   0   'False
      Tab(2).Control(1)=   "txtRate1"
   ...
   Begin VB.CommandButton cmdPost 
      ...
      Height          =   510
      Left            =   6495
      Picture         =   "frmVendIssItemsByItem.frx":0054
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5010
      UseMaskColor    =   -1  'True
   ...
   Begin MSComctlLib.ImageList ImageList2 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVendIssItemsByItem.frx":0156
            Key             =   "Down"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVendIssItemsByItem.frx":0312
            Key             =   "Up"
         EndProperty
      EndProperty
   End
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         NumListImages   =   2
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVendIssItemsByItem.frx":04CE
            Key             =   "Active"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmVendIssItemsByItem.frx":0922
            Key             =   "Inactive"
         EndProperty
      EndProperty
   End
   ...
   Begin MSForms.Label Label2 
      ...
      Width           =   2355
      BackColor       =   11517387
      Caption         =   $"frmVendIssItemsByItem.frx":0D76
      Size            =   "4154;423"
      BorderColor     =   16512
      BorderStyle     =   1
      FontName        =   "Tahoma"
   ...
   End
End
Attribute VB_Name = "frmVendIssItemsByItem"
...
Private Sub cmbOrderNo_Click()
    ...
    With cmd
        ...
        .CommandText = "IssueItems_SP1"
        .Parameters("@ProcessID").value = cmbProcess.ID
        .Parameters("@OrderNo").value = cmbOrderNo.Text
        Set rs = .Execute
    End With
    Dim lQty As Long
    ...
    With rs
        ...
        Do Until .EOF
            ...
            For i = 0 To cmbItems.ListCount - 1
                If Trim(cmbItems.List(i)) = Trim(![ITemID] & "") Then
                    isAdded = True
                    Exit For
                End If
            Next i
            ...
            If cmbProcess.ID = !InitialProcessID Then
                lQty = Val(!OrderQty & "") - Val(!IssQty & "")
            Else
                lQty = Val(!IssQty & "")
            End If
...
Private Sub cmbProcess_matchedOld()
    Exit Sub
    On Error GoTo err
...
Private Sub cmbVender_matched()
    ...
    Else
        cmbVender.Tag = Val(con.Execute("Select Rate From VMakerAssItems Where VendID=" & cmbVender.ID & " and ItemID='" & LV.SelectedItem.SubItems(2) & "' AND ProcessID=" & cmbProcess.ID).Fields(0).value & "")
    End If
...
Private Sub ShowVenderHistory()
    ...
    Dim ITM As ListItem
    With rs
        .Open "Select Top 10 * From VMakerIssItems Where VendID=" & cmbVender.ID & " and ProcessID=" & cmbProcess.ID & " Order By DT Desc,EntryID Desc", con, adOpenForwardOnly, adLockReadOnly
        ...
        If cmbVender.ID <> 129 Then
            .Open "Select ItemCode,ItemDescription,Sum(RemQty) As RemQty From VMakerIssItems Where VendID=" & cmbVender.ID & " And ProcessID=" & cmbProcess.ID & " AND RemQty>0 Group By ItemCode,ItemDescription Having Sum(RemQty)>0", con, adOpenForwardOnly, adLockReadOnly
...
Private Sub PrintIssSlip()

    On Error GoTo err
    Dim rpt As craxddrt.Report
    Set rpt = rptApp.OpenReport(RptPath & "\ProcessIssSlip.rpt")
    Dim strIssIds As String
    Dim rs As New ADODB.Recordset
    With rs
        .Open "SELECT DISTINCT RecieptNo FROM PrintSession", con
        strIssIds = ""
        Do Until .EOF
            strIssIds = strIssIds & "'" & .Fields(0).value & "',"
            .MoveNext
        Loop
        .Close
    End With
    If Len(strIssIds) > 0 Then
        strIssIds = Left(strIssIds, Len(strIssIds) - 1)
    End If
    
    Dim f As New frmPrevRpt
    f.ShowReport "{VendIssdDetail.RecieptID} IN [" & strIssIds & "]", rpt
    Set rs = Nothing
    Exit Sub
err:
    MsgBox err.Description
...
Private Sub LVIssued_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 46 Then
        ...
        LV.ListItems(LVIssued.SelectedItem.Tag).ListSubItems(5) = LV.ListItems(LVIssued.SelectedItem.Tag).ListSubItems(5).Tag
...
Private Sub FillVendList()
    ...
    Else
        Call cmbVender.AddVals(con, "Distinct VenderName", "VMakerAssItems", "VendID", " Where (ItemID='" & LV.SelectedItem.SubItems(2) & "' AND Active=1 and (ProcessID=" & cmbProcess.ID & ") and VendID Not In" & VendIDList & " ) OR VendID=129 ")
    End If
    'End If
    ...
    For i = 1 To LVIssued.ListItems.count
        With LVIssued.ListItems(i)
            ...
            For j = 1 To VendID.count
                If Val(LVIssued.ListItems(i).ListSubItems(1).Tag) = VendID(j) Then
            ...
            If SaveIt Then
                'Val (LVIssued.ListItems(i).ListSubItems(1).Tag)
'                con.Execute "Insert Into VendIssued(VendID,DT,RecieptID,UserID,ProcessID) Values(129" & _
'                 ",'" & ServerDate & " " & ServerTime & "',''," & _
'                 UserID & "," & cmbProcess.ID & ")"
                Dim lMakerIssNo As Long, strMakerIssNo As String
                lMakerIssNo = Val(con.Execute("SELECT Max(Cast(right(RecieptID,Len(RecieptID)-12) As Int)) From VendIssued Where Convert(varchar,DT,6)='" & Format(DTPicker2, "dd MMM yy") & "' AND ISNULL(RecieptID,'')<>''").Fields(0).value & "")
                ...
                'con.Execute "INSERT INTO VendIssued(VendID,DT,RecieptID,UserID,ProcessID,ItemID) " & _
                 "Values(" & Val(LVIssued.ListItems(i).ListSubItems(1).Tag) & ",'" & _
                 ServerDate & " " & ServerTime & "','" & strMakerIssNo & "'," & _
                 UserID & "," & cmbProcess.ID & ",'" & .SubItems(5) & "')"
                 
                con.Execute "INSERT INTO VendIssued(VendID,DT,RecieptID,UserID,ProcessID,ItemID) " & _
                 "Values(" & Val(LVIssued.ListItems(i).ListSubItems(1).Tag) & ",'" & _
                 ServerDate & " " & ServerTime & "','" & strMakerIssNo & "'," & _
                 UserID & "," & cmbProcess.ID & ",'" & .SubItems(4) & "')"
            ...
            Else
                'con.Execute "INSERT INTO VendIssdDetail(RefID,RecieptID,ItemCode" & _
                 ...
                 .SubItems(4) & "'," & .ListSubItems(3).Tag & ",'" & .SubItems(6) & "'," & .SubItems(9) & ")"
                con.Execute "Insert Into VendIssdDetail(RefID,RecieptID,ItemCode" & _
                 ",Rate,IssQty,ReqAuth,OrderNo,RcvProcessID,ReturnDT,Priority) Values(" & lRefID & ",'" & _
                 .SubItems(2) & "','" & .SubItems(4) & "'," & _
                 Val(.SubItems(6)) & "," & Val(.SubItems(7)) & "," & UserReqAuth & ",'" & .SubItems(3) & _
                 "'," & cmbProcess.ID & ",'" & DTPicker2.value & "'," & .Index & ")"
...
Private Sub txtRate1_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        'LVMaker.SelectedItem.SubItems(3) = Val(txtRate)
        Dim ITM As ListItem
        iQty = LV.ListItems(LVIssued.SelectedItem.Tag).ListSubItems(5).Tag - Val(txtRate1)
        If iQty < 0 Then
            MsgBox "Invalid Qty!", vbInformation
            Exit Sub
        End If
        ...
        LV.ListItems(LVIssued.SelectedItem.Tag).SubItems(5) = iQty
