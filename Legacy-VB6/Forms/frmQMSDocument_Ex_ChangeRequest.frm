...
Begin VB.Form frmQMSDocument_Ex_ChangeRequest 
   ...
   End
End
Attribute VB_Name = "frmQMSDocument_Ex_ChangeRequest"
...
'    Call cmbEmp.AddVals(con, "'{'+EmpID+'} '+Name", "Employees", "EmpID")
    ...
    Call AddToCombo(cmbDocumentNo, "DocumentNumber", "QMS_Documents_Ex", , , "EntryID")
End Sub
...
Private Sub cmbDocumentNo_Click()
    ...
    With rs
        .Open "SELECT DocumentName,DocumentType FROM QMS_Documents_Ex WHERE EntryID=" & Val(cmbDocumentNo.List(cmbDocumentNo.ListIndex, 1)), con, adOpenForwardOnly, adLockReadOnly
        If .EOF = False Then
            TBox(3) = !DocumentType & ""
            TBox(2) = !DocumentName & ""
        End If
        ...
        .Open "SELECT * FROM QMS_Documents_Ex_ChangeRequests WHERE QMS_RefID=" & Val(cmbDocumentNo.List(cmbDocumentNo.ListIndex, 1)) & " AND Approved IS NULL", con, adOpenForwardOnly, adLockReadOnly
...
Private Sub cmdApprove_Click()
    ...
    If MsgBox("Are you sure?", vbQuestion + vbYesNo) = vbYes Then
        ...
        Call StartTrans(con)
        con.Execute "UPDATE QMS_Documents_Ex_ChangeRequests SET Approved=1,ApprovedUserName='" & CurrentUserName & "',ApprovedMachineName='" & strComputerName & "',ApprovedEntryDT=getDate() WHERE EntryID=" & lEntryID
        con.Execute "UPDATE QMS_Documents_Ex SET Revision='" & TBox(4) & "' WHERE EntryID=" & lQMSEntryID
        
        'Update Original PDF Details in Change Request for History Purpose...
        con.Execute "UPDATE QMS_Documents_Ex_ChangeRequests SET OriginalPDF=T1.AttachmentPDF,OriginalPDFFileName=T1.AttachedPDFFileName FROM QMS_Documents_Ex_ChangeRequests " & _
         "INNER JOIN QMS_Documents_Ex T1 ON T1.EntryID=QMS_Documents_Ex_ChangeRequests.QMS_RefID WHERE QMS_Documents_Ex_ChangeRequests.EntryID=" & lEntryID
         
        
        'Update PDF Details in Document...
        con.Execute "UPDATE QMS_Documents_Ex SET AttachmentPDF=ChangedPDF,AttachedPDFFileName=ChangedPDFFileName FROM QMS_Documents_Ex " & _
         "INNER JOIN QMS_Documents_Ex_ChangeRequests ON QMS_Documents_Ex.EntryID=QMS_Documents_Ex_ChangeRequests.QMS_RefID WHERE QMS_Documents_Ex_ChangeRequests.EntryID=" & lEntryID
...
Private Sub cmdViewPDF_Click()
    ...
    Dim lCount As Long
    lCount = GetSingleLongValue("COUNT(*)", "QMS_Documents_Ex_ChangeRequests", " WHERE EntryID=" & lEntryID & " AND ChangedPDF IS NOT NULL")
    
    If lCount = 0 Then
        MsgBox "No PDF attached.", vbInformation
        Exit Sub
    Else
        Call ShowTechDrawing("QMS_Documents_Ex_ChangeRequests", "ChangedPDF", " WHERE EntryID=" & lEntryID, txtFilePath.Text, CD1)
    End If
...
Private Function SaveItem() As Boolean
    ...
    If lEntryID = 0 Then
        Sql = CreateInsertSQL("QMS_Documents_Ex_ChangeRequests")
    Else
        Sql = CreateUpdateSQL("QMS_Documents_Ex_ChangeRequests")
    ...
    If lEntryID = 0 Then
        lmyEntryID = GetSingleLongValue("MAX(EntryID)", "QMS_Documents_Ex_ChangeRequests")
    Else
        lmyEntryID = lEntryID
    End If
...
Private Sub ShowData(p_lEntryID As Long, Optional bCopy As Boolean = False)
    ...
    Dim c As Control
    With rs
        .Open "SELECT * FROM QMS_Documents_Ex WHERE EntryID=" & p_lEntryID, con, adOpenForwardOnly, adLockReadOnly
...
Private Function GetNewDocumentNo() As String
    ...
    Dim lDocNo As Long
    lDocNo = GetSingleLongValue("MAX(CAST(RIGHT(DocumentNumber,LEN(DocumentNumber)-8) AS INT))", "QMS_Documents_Ex")
    lDocNo = lDocNo + 1
...
Private Sub AttachPDF(p_lEntryID As Long)
    ...
    If strPDFPath <> "" And txtFilePath.Tag = "" Then
        ...
        Rec.Open "SELECT EntryID,ChangedPDF,ChangedPDFFileName FROM QMS_Documents_Ex_ChangeRequests WHERE EntryID=" & p_lEntryID, con, adOpenStatic, adLockOptimistic
