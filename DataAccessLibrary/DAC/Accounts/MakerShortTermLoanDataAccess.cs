...
{
    public class MakerShortTermLoanDataAccess : IMakerShortTermLoanDataAccess
    {
        private readonly IConfiguration _config;

        public MakerShortTermLoanDataAccess(IConfiguration config)
        {
            _config = config;
        }
        ...
        public async Task<decimal> GetMakerShortTermBalanceAsync(string accNo)
        {
            if (string.IsNullOrWhiteSpace(accNo)) return 0m;

            using IDbConnection db = new SqlConnection(ConnectionString);
            string sql = @"SELECT ISNULL(SUM(Amount - AmountCleared), 0) FROM MakerAdvancesShort WHERE AccNo = @AccNo";
        ...
        public async Task SaveMakerShortTermLoanAsync(MakerShortTermLoanDto dto, string vchrNo)
        {
            using IDbConnection db = new SqlConnection(ConnectionString);
            string sql = @"INSERT INTO MakerAdvancesShort(AccNo, DT, Description, Amount, AmountCleared, AccVoucherNo, MasterPONo)
                           VALUES(@AccNo, @DT, @Description, @Amount, 0, @AccVoucherNo, @MasterPONo)";
