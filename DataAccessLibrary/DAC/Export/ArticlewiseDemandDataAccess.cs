...
{
    public class ArticlewiseDemandDataAccess : IArticlewiseDemandDataAccess
    {
        private readonly string _connectionString;

        public ArticlewiseDemandDataAccess(IConfiguration configuration)
        {
        ...
        public async Task<List<ArticlewiseDemandModel>> GetArticlewiseDemandDataAsync(string itemId, string custCode, string country, bool useDateRange, DateTime dtFrom, DateTime dtTo)
        {
            ...
            {
                conditions.Add("CompItemCode = @ItemID");
            ...
            string sql = "SELECT ItemID, CompItemCode, ItemName, OrderNo, DT, Qty, CustCode, Country FROM VArticleWiseOrders";
            ...
            var result = await db.QueryAsync<ArticlewiseDemandModel>(sql, parameters);
            return result.ToList();
        }
    }
}
